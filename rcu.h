...
public:
  ...
  public:
    ...
    inline void set_pin_cpu(size_t cpu) { pin_cpu_ = cpu; }

    inline ssize_t get_pin_cpu() const { return pin_cpu_; }
  ...
  // thin forwarders
  inline void *alloc(size_t sz) { return mysync().alloc(sz); }

  inline void *alloc_static(size_t sz) { return mysync().alloc_static(sz); }

  // this releases memory back to the allocator subsystem
  // this should NOT be used to free objects!
  inline void dealloc(void *p, size_t sz) { return mysync().dealloc(p, sz); }

  void dealloc_rcu(void *p, size_t sz);

  inline bool try_release() { return mysync().try_release(); }

  inline void do_cleanup() { mysync().do_cleanup(); }
  ...
  inline bool in_rcu_region() const {
    uint64_t rcu_tick;
  ...
  inline uint64_t cleaning_rcu_tick_exclusive() const {
...
public:
  ...
  inline ticker::guard *guard() { return guard_.obj(); }

  inline rcu::sync *sync() { return sync_; }
