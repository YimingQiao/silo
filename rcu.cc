...
static event_avg_counter evt_avg_time_inbetween_allocator_releases_usec("avg_time_inbetween_allocator_releases_usec");

#ifdef MEMCHECK_MAGIC
static void report_error_and_die(const void *p, size_t alloc_size, const char *msg, const string &prefix = "",
  ...
  if (pnext) {
    ...
    if (!pmd) {
      ...
      if (((uintptr_t) pnext % pmd->unit_) == 0) {
        if (recurse) report_error_and_die(pnext, pmd->unit_, "", prefix + "    ", recurse - 1, false);
        else
          cerr << prefix << "recursion stopped" << endl;
      } else {
        cerr << prefix << "Next ptr not properly aligned" << endl;
        if (recurse)
          report_error_and_die((const void *) slow_round_down((uintptr_t) pnext, (uintptr_t) pmd->unit_), pmd->unit_,
                               "", prefix + "    ", recurse - 1, false);
        else
          cerr << prefix << "recursion stopped" << endl;
      }
    }
  ...
  for (const char *buf = (const char *) p; buf < (const char *) p + alloc_size; buf += 8) {
    cerr << prefix << hexify_buf(buf, 8) << endl;
  }
  if (first) ALWAYS_ASSERT(false);
}

static void check_pointer_or_die(void *p, size_t alloc_size) {
  ALWAYS_ASSERT(p);
  if (unlikely(((uintptr_t) p % alloc_size) != 0)) report_error_and_die(p, alloc_size, "pointer not properly aligned");
  for (size_t off = sizeof(void **); off < alloc_size; off++)
    if (unlikely((unsigned char) *((const char *) p + off) != (unsigned char) MEMCHECK_MAGIC))
...
void *rcu::sync::alloc_static(size_t sz) {
  if (pin_cpu_ == -1) return malloc(sz);
...
void rcu::sync::dealloc(void *p, size_t sz) {
  ...
  *reinterpret_cast<void **>(p) = arenas_[arena];
  ...
  NDB_MEMSET((char *) p + sizeof(void **), MEMCHECK_MAGIC, alloc_size - sizeof(void **));
...
bool rcu::sync::try_release() {
  ...
  size_t acc = 0;
  for (size_t i = 0; i < ::allocator::MAX_ARENAS; i++) acc += deallocs_[i];
  if (acc > threshold) {
    do_release();
...
void rcu::sync::do_cleanup() {
  ...
  for (auto it = q.begin(); it != q.end(); ++it, ++n) {
    try {
      it->run(*this);
    } catch (...) { cerr << "rcu::region_end: uncaught exception in free routine" << endl; }
  }
  q.clear();
  evt_rcu_deletes += n;
...
rcu::rcu() : syncs_() {
  ...
  for (size_t i = 0; i < ::allocator::MAX_ARENAS; i++) {
    evt_allocator_arena_allocations[i] = new event_counter("allocator_arena" + to_string(i) + "_allocation");
    evt_allocator_arena_deallocations[i] = new event_counter("allocator_arena" + to_string(i) + "_deallocation");
  }
}
...
static void rcu_stress_test_worker(unsigned id) {
  ...
  rcu_stress_test_rec *mypx = (rcu_stress_test_rec *) rcu::s_instance.alloc(sizeof(rcu_stress_test_rec));
  ...
  while (rcu_stress_test_keep_going.load(memory_order_acquire)) {
    ...
    if (mypx) {
      ...
      mypx = (rcu_stress_test_rec *) rcu::s_instance.alloc(sizeof(rcu_stress_test_rec));
      mypx->magic_ = rcu_stress_test_magic;
      mypx->counter_ = 0;
      rcu_stress_test_array[id].store(mypx, memory_order_release);
    }
...
static void rcu_stress_test() {
  for (size_t i = 0; i < rcu_stress_test_nthreads; i++) rcu_stress_test_array[i].store(nullptr, memory_order_release);
  vector<thread> workers;
  for (size_t i = 0; i < rcu_stress_test_nthreads; i++) workers.emplace_back(rcu_stress_test_worker, i);
  sleep(120);
