...
struct test_callback_ctr {
  ...
  inline bool operator()(const concurrent_btree::string_type &k, const string &v) const {
    (*ctr)++;
    return true;
  }
  size_t *const ctr;
...
template<typename P>
static void always_assert_cond_in_txn(const P &t, bool cond, const char *condstr, const char *func,
                                      const char *filename, int lineno) {
  if (likely(cond)) return;
  static mutex g_report_lock;
  std::lock_guard<mutex> guard(g_report_lock);
  cerr << func << " (" << filename << ":" << lineno << ") - Condition `" << condstr << "' failed!" << endl;
  t.dump_debug_info();
...
template<typename P>
static inline void AssertSuccessfulCommit(P &t) {
  ALWAYS_ASSERT_COND_IN_TXN(t, t.commit(false));
}

template<typename P>
static inline void AssertFailedCommit(P &t) {
  ALWAYS_ASSERT_COND_IN_TXN(t, !t.commit(false));
}

template<typename T>
inline void AssertByteEquality(const T &t, const uint8_t *v, size_t sz) {
...
template<typename T>
inline void AssertByteEquality(const T &t, const string &v) {
...
static inline ostream &operator<<(ostream &o, const rec &r) {
  o << "[rec=" << r.v << "]";
  return o;
}
...
static void test1() {
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
    ...
    {
      TxnType<Traits> t0(txn_flags, arena), t1(txn_flags, arena);
      string v0, v1;
...
template<template<typename> class TxnType, typename Traits>
static void test_absent_key_race() {
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
    const uint64_t txn_flags = TxnFlags[txn_flags_idx];
...
template<template<typename> class TxnType, typename Traits>
static void test_inc_value_size() {
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
    const uint64_t txn_flags = TxnFlags[txn_flags_idx];
    txn_btree<TxnType> btr;
    typename Traits::StringAllocator arena;
...
template<template<typename> class TxnType, typename Traits>
static void test_read_only_snapshot() {
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
    const uint64_t txn_flags = TxnFlags[txn_flags_idx];
    txn_btree<TxnType> btr;
    typename Traits::StringAllocator arena;
    ...
    {
      TxnType<Traits> t0(txn_flags, arena), t1(txn_flags | transaction_base::TXN_FLAG_READ_ONLY, arena);
      string v0, v1;
      ALWAYS_ASSERT_COND_IN_TXN(t0, btr.search(t0, u64_varkey(0), v0));
      ALWAYS_ASSERT_COND_IN_TXN(t0, !v0.empty());
      AssertByteEquality(rec(0), v0);
...
namespace test_long_keys_ns {
  ...
  static inline string make_long_key(int32_t a, int32_t b, int32_t c, int32_t d) {
  ...
  public:
    ...
    virtual bool invoke(const typename txn_btree<Protocol>::keystring_type &k, const string &v) {
...
namespace test_insert_same_key_ns {
  struct rec0 {
    rec0(int ch) { NDB_MEMSET(&buf[0], ch, sizeof(buf)); }
    char buf[8];
  };
  struct rec1 {
    rec1(int ch) { NDB_MEMSET(&buf[0], ch, sizeof(buf)); }
    char buf[128];
  };
  struct rec2 {
    rec2(int ch) { NDB_MEMSET(&buf[0], ch, sizeof(buf)); }
    char buf[1024];
  };
  struct rec3 {
    rec3(int ch) { NDB_MEMSET(&buf[0], ch, sizeof(buf)); }
    char buf[2048];
  };
  struct rec4 {
    rec4(int ch) { NDB_MEMSET(&buf[0], ch, sizeof(buf)); }
    char buf[4096];
  };
}// namespace test_insert_same_key_ns
...
static void test_long_keys() {
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
...

    {
      ...
      AssertSuccessfulCommit(t);
      if (c.ctr != N) cerr << "c.ctr: " << c.ctr << ", N: " << N << endl;
...
#ifdef HAVE_REVERSE_RANGE_SCANS
    {
      ...
      AssertSuccessfulCommit(t);
      if (c.ctr != (N - 1)) cerr << "c.ctr: " << c.ctr << ", N: " << N << endl;
...
template<template<typename> class TxnType, typename Traits>
static void test_long_keys2() {
  using namespace test_long_keys_ns;
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
    const uint64_t txn_flags = TxnFlags[txn_flags_idx];

    const uint8_t lowkey_cstr[] = {
        0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x45, 0x49, 0x4E, 0x47, 0x41, 0x54,
    ...
    const uint8_t highkey_cstr[] = {0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x45, 0x49, 0x4E, 0x47, 0x41, 0x54,
...
template<template<typename> class TxnType, typename Traits>
static void test_insert_same_key() {
...
#define TESTREC_KEY_FIELDS(x, y) x(int32_t, k0) y(int32_t, k1)
#define TESTREC_VALUE_FIELDS(x, y) x(int32_t, v0) y(int16_t, v1) y(inline_str_fixed<10>, v2)
...
namespace test_typed_btree_ns {
  ...
  public:
    ...
    virtual bool invoke(const testrec::key &key, const testrec::value &value) {
...
static void test_typed_btree() {
  ...
  {
    txn_type t(0, arena);
    for (size_t i = 0; i < ARRAY_NELEMS(scan_values); i++) btr.insert(t, scan_values[i].first, scan_values[i].second);
    AssertSuccessfulCommit(t);
  }
...
public:
  txn_btree_worker(txn_btree<Protocol> &btr, uint64_t txn_flags) : btr(&btr), txn_flags(txn_flags) {}
    ...
    virtual void run() {
      ...
      while (running.load()) {
        ...
        try {
          ...
          t.commit(true);
          commits++;
        } catch (transaction_abort_exception &e) { aborts++; }
      }
    }
...
template<template<typename> class TxnType, typename Traits>
static void mp_stress_test_allocator() {
  ...
  txn_btree<TxnType> btr;
  {
    ...
    for (size_t i = 0; i < nkeys; i++) btr.insert_object(t, u64_varkey(i), rec(0));
    AssertSuccessfulCommit(t);
  }
  vector<shared_ptr<worker<TxnType, Traits>>> v;
  for (size_t i = 0; i < nworkers; i++) v.emplace_back(new worker<TxnType, Traits>(i, btr, 0));
  for (auto &p: v) p->start();
  sleep(60);
  running.store(false);
  for (auto &p: v) {
    p->join();
    cerr << "worker " << p->get_id() << " commits " << p->get_commits() << " aborts " << p->get_aborts() << endl;
  }
...
namespace mp_stress_test_insert_removes_ns {
  ...
  class worker : public txn_btree_worker<TxnType> {
  public:
    worker(txn_btree<TxnType> &btr, uint64_t txn_flags) : txn_btree_worker<TxnType>(btr, txn_flags) {}
    ~worker() {}
    virtual void run() {
      fast_random r(reinterpret_cast<unsigned long>(this));
      while (running.load()) {
...
template<template<typename> class TxnType, typename Traits>
static void mp_stress_test_insert_removes() {
  using namespace mp_stress_test_insert_removes_ns;
  txn_btree<TxnType> btr;
  vector<shared_ptr<worker<TxnType, Traits>>> v;
  for (size_t i = 0; i < nworkers; i++) v.emplace_back(new worker<TxnType, Traits>(btr, 0));
  for (auto &p: v) p->start();
    ...
    virtual void run() {
      ...
      retry:
        ...
        try {
          ...
          t.commit(true);
        } catch (transaction_abort_exception &e) { goto retry; }
      }
    }
  };
}// namespace mp_test1_ns
...
static void mp_test1() {
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
...

    {
      ...
      if (rv != (niters * 4)) cerr << "v: " << rv << ", expected: " << (niters * 4) << endl;
...
namespace mp_test2_ns {
  ...
  struct control_rec {
    ...
    control_rec() : count_(0) { NDB_MEMSET(&values_[0], 0, sizeof(values_)); }
    ...
    void insert(uint32_t v) {
      if (count_ >= ARRAY_NELEMS(values_)) {
        cerr << "ERROR when trying to insert " << v << ": " << vectorize() << endl;
      }
    ...
    inline vector<uint32_t> vectorize() const {
      vector<uint32_t> v;
      for (size_t i = 0; i < count_; i++) v.push_back(values_[i]);
      return v;
    }
  };
  ...
  public:
    virtual bool invoke(const typename txn_btree<Protocol>::keystring_type &k, const string &v) {
      ...
      const uint64_t u64k = host_endian_trfm<uint64_t>()(*reinterpret_cast<const uint64_t *>(k.data()));
  ...
  public:
    mutate_worker(txn_btree<TxnType> &btr, uint64_t flags) : txn_btree_worker<TxnType>(btr, flags), naborts(0) {}
    virtual void run() {
      while (running) {
        ...
        retry:
          ...
          //  try {
          //    string v, v_ctr;
          //    ALWAYS_ASSERT_COND_IN_TXN(t, this->btr->search(t,
          //    u64_varkey(ctr_key), v_ctr)); ALWAYS_ASSERT_COND_IN_TXN(t,
          //    v_ctr.size() == sizeof(control_rec)); const bool ret =
          //    this->btr->search(t, u64_varkey(i), v); t.commit(true); if
          //    (reinterpret_cast<const rec *>(v_ctr.data())->v != did_v) {
          //      cerr << "rec.v: " << reinterpret_cast<const rec
          //      *>(v_ctr.data())->v << ", did_v: " << did_v << endl;
          //      ALWAYS_ASSERT(false);
          //    }
          //    if (did_remove && ret) {
...
static void mp_test2() {
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
...

    {
      ...
      TxnType<Traits> t(txn_flags | transaction_base::TXN_FLAG_READ_ONLY, arena);
      typename Traits::StringAllocator arena;
      string v_ctr;
...
namespace mp_test3_ns {
    ...
    virtual void run() {
      ...
      retry:
        try {
          ...
          while (unlikely(a == b)) b = r.next() % naccounts;
          string arecv, brecv;
  ...
  template<template<typename> class TxnType, typename Traits>
  class invariant_worker_scan : public txn_btree_worker<TxnType>, public txn_btree<TxnType>::search_range_callback {
  public:
    ...
    virtual void run() {
      while (running) {
        try {
          ...
          validations++;
        } catch (transaction_abort_exception &e) { naborts++; }
      }
    }
    virtual bool invoke(const typename txn_btree<TxnType>::keystring_type &k, const string &v) {
      sum += ((rec *) v.data())->v;
      return true;
    }
...
static void mp_test3() {
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
    ...
    {
      TxnType<Traits> t(txn_flags, arena);
      for (uint64_t i = 0; i < naccounts; i++) btr.insert_object(t, u64_varkey(i), rec(amount_per_person));
      AssertSuccessfulCommit(t);
    }

    txn_epoch_sync<TxnType>::sync();

    transfer_worker<TxnType, Traits> w0(btr, txn_flags, 342), w1(btr, txn_flags, 93852), w2(btr, txn_flags, 23085),
...
namespace mp_test_simple_write_skew_ns {
      ...
      while (running) {
        try {
          ...
          if ((n % 2) == 0) {
            ...
            if (ctr == 2) this->btr->insert_object(t, u64_varkey(d), rec(0));
            t.commit(true);
  ...
  template<template<typename> class TxnType, typename Traits>
  class scan_worker : public txn_btree_worker<TxnType>, public txn_btree<TxnType>::search_range_callback {
  public:
...
template<template<typename> class TxnType, typename Traits>
static void mp_test_simple_write_skew() {
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
    ...
    typename Traits::StringAllocator arena;
    {
      ...
      for (uint64_t i = 0; i < NDoctors; i++) btr.insert_object(t, u64_varkey(i), rec(i < 2 ? 1 : 0));
      AssertSuccessfulCommit(t);
    }
    ...
    for (size_t i = 0; i < NDoctors / 2; i++) workers.push_back(new get_worker<TxnType, Traits>(i, btr, txn_flags));
    for (size_t i = NDoctors / 2; i < NDoctors; i++)
      workers.push_back(new scan_worker<TxnType, Traits>(i, btr, txn_flags));
    for (size_t i = 0; i < NDoctors; i++) workers[i]->start();
    sleep(10);
    running = false;
    __sync_synchronize();
    ...
    for (size_t i = 0; i < NDoctors; i++) {
      workers[i]->join();
      if (get_worker<TxnType, Traits> *w = dynamic_cast<get_worker<TxnType, Traits> *>(workers[i])) n_get_succ += w->n;
...
namespace mp_test_batch_processing_ns {
  ...
  static inline string MakeKey(uint32_t batch_id, uint32_t receipt_id) {
  ...
  template<template<typename> class TxnType, typename Traits>
  class report_worker : public ndb_thread, public txn_btree<TxnType>::search_range_callback {
  public:
...
template<template<typename> class TxnType, typename Traits>
static void mp_test_batch_processing() {
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags); txn_flags_idx++) {
...

    {
      ...
      for (size_t i = 0; i < nkeys / nkeyspertxn; i++) {
        ...
        for (size_t j = i * nkeyspertxn; j < end; j++) btr.insert_object(t, u64_varkey(j), rec(j + 1));
        AssertSuccessfulCommit(t);
