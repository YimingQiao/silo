...
struct test_callback_ctr {
  ...
  inline bool operator()(const concurrent_btree::string_type &k,
...
template <typename P>
static void always_assert_cond_in_txn(const P &t, bool cond,
...
#define ALWAYS_ASSERT_COND_IN_TXN(t, cond)                                     \
  always_assert_cond_in_txn(t, cond, #cond, __PRETTY_FUNCTION__, __FILE__,     \
                            __LINE__)

template <typename P> static inline void AssertSuccessfulCommit(P &t) {
  ALWAYS_ASSERT_COND_IN_TXN(t, t.commit(false));
}

template <typename P> static inline void AssertFailedCommit(P &t) {
  ALWAYS_ASSERT_COND_IN_TXN(t, !t.commit(false));
}
...
inline void AssertByteEquality(const T &t, const uint8_t *v, size_t sz) {
  ...
  if (!success) {
    cerr << "expecting: " << hexify(string((const char *)&t, sizeof(T)))
         << endl;
...
static void test1() {
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags);
       txn_flags_idx++) {
...
static void test_read_only_snapshot() {
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags);
    ...
    {
      TxnType<Traits> t0(txn_flags, arena),
...
class counting_scan_callback
    : public txn_btree<Protocol>::search_range_callback {
public:
  counting_scan_callback(uint64_t expect) : ctr(0), expect(expect) {}

  virtual bool invoke(const typename txn_btree<Protocol>::keystring_type &k,
                      const string &v) {
...
static void test_long_keys2() {
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags);
    ...
    const uint8_t lowkey_cstr[] = {
        0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x45, 0x49,
        0x4E, 0x47, 0x41, 0x54, 0x49, 0x4F, 0x4E, 0x45, 0x49, 0x4E,
        0x47, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    };
    const string lowkey_s((const char *)&lowkey_cstr[0],
                          ARRAY_NELEMS(lowkey_cstr));
    const uint8_t highkey_cstr[] = {
        0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x45, 0x49,
        0x4E, 0x47, 0x41, 0x54, 0x49, 0x4F, 0x4E, 0x45, 0x49, 0x4E,
        0x47, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    const string highkey_s((const char *)&highkey_cstr[0],
                           ARRAY_NELEMS(highkey_cstr));
...
#define TESTREC_VALUE_FIELDS(x, y)                                             \
  x(int32_t, v0) y(int16_t, v1) y(inline_str_fixed<10>, v2)
...
template <template <typename> class Protocol>
class scan_callback
    : public typed_txn_btree<Protocol, schema<testrec>>::search_range_callback {
public:
...
public:
  worker(unsigned id, txn_btree<TxnType> &btr, uint64_t txn_flags)
      : txn_btree_worker<TxnType>(btr, txn_flags), id(id), commits(0),
        aborts(0) {}
  ~worker() {}
  virtual void run() {
...
static void mp_stress_test_allocator() {
  ...
  for (auto &p : v) {
    p->join();
    cerr << "worker " << p->get_id() << " commits " << p->get_commits()
         << " aborts " << p->get_aborts() << endl;
  }
...
struct control_rec {
  ...
  void insert(uint32_t v) {
    if (count_ >= ARRAY_NELEMS(values_)) {
      cerr << "ERROR when trying to insert " << v << ": " << vectorize()
           << endl;
    }
  ...
  bool remove(uint32_t v) {
    for (size_t i = 0; i < count_; i++) {
      if (values_[i] == v) {
        ...
        memmove(&values_[i], &values_[i + 1],
                (count_ - (i + 1)) * sizeof(uint32_t));
        count_ -= 1;
        sanity_check();
        INVARIANT(!contains(v));
        // no dups assumption
...
public:
  virtual bool invoke(const typename txn_btree<Protocol>::keystring_type &k,
    ...
    const uint64_t u64k = host_endian_trfm<uint64_t>()(
        *reinterpret_cast<const uint64_t *>(k.data()));
    ...
    VERBOSE(cerr << "counting_scan_callback: " << hexify(k) << " => " << r->v
                 << endl);
        ...
        // uint64_t did_v = 0;
        {
          ...
          try {
            ...
            ALWAYS_ASSERT_COND_IN_TXN(
                t, this->btr->search(t, u64_varkey(ctr_key), v_ctr));
            ALWAYS_ASSERT_COND_IN_TXN(t, v_ctr.size() == sizeof(control_rec));
            ALWAYS_ASSERT_COND_IN_TXN(
                t, ((const control_rec *)v_ctr.data())->count_ > 1);
...
public:
  reader_worker(txn_btree<TxnType> &btr, uint64_t flags, bool reverse)
      : txn_btree_worker<TxnType>(btr, flags), reverse_(reverse),
  ...
  virtual void run() {
    while (running) {
      ...
      try {
        ...
        if (c.values_ != cvec) {
          cerr << "observed (" << c.values_.size() << "): " << c.values_
               << endl;
...
static void mp_test2() {
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags);
    ...
    reader_worker<TxnType, Traits> w2(
        btr, txn_flags | transaction_base::TXN_FLAG_READ_ONLY, false);
    reader_worker<TxnType, Traits> w3(
        btr, txn_flags | transaction_base::TXN_FLAG_READ_ONLY, false);
#ifdef HAVE_REVERSE_RANGE_SCANS
    reader_worker<TxnType, Traits> w4(
        btr, txn_flags | transaction_base::TXN_FLAG_READ_ONLY, true);
#else
    reader_worker<TxnType, Traits> w4(
        btr, txn_flags | transaction_base::TXN_FLAG_READ_ONLY, false);
#endif
    ...
    retry:
      try {
        ...
        while (unlikely(a == b))
          b = r.next() % naccounts;
        string arecv, brecv;
        ALWAYS_ASSERT_COND_IN_TXN(t,
                                  this->btr->search(t, u64_varkey(a), arecv));
        ALWAYS_ASSERT_COND_IN_TXN(t,
                                  this->btr->search(t, u64_varkey(b), brecv));
...
public:
  invariant_worker_scan(txn_btree<TxnType> &btr, uint64_t flags)
      : txn_btree_worker<TxnType>(btr, flags), running(true), validations(0),
        naborts(0), sum(0) {}
  virtual void run() {
    while (running) {
      try {
    ...
    while (running) {
      try {
        ...
        if (sum != (naccounts * amount_per_person)) {
          cerr << "sum: " << sum << endl;
          cerr << "naccounts * amount_per_person: "
               << (naccounts * amount_per_person) << endl;
        }
        ALWAYS_ASSERT_COND_IN_TXN(t, sum == (naccounts * amount_per_person));
        validations++;
      } catch (transaction_abort_exception &e) {
...
static void mp_test3() {
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags);
    ...
    transfer_worker<TxnType, Traits> w0(btr, txn_flags, 342),
        w1(btr, txn_flags, 93852), w2(btr, txn_flags, 23085),
    ...
    invariant_worker_scan<TxnType, Traits> w6(
        btr, txn_flags | transaction_base::TXN_FLAG_READ_ONLY);
    invariant_worker_1by1<TxnType, Traits> w7(
        btr, txn_flags | transaction_base::TXN_FLAG_READ_ONLY);
    ...
    cerr << "scan validations: " << w4.validations
         << ", scan aborts: " << w4.naborts << endl;
    cerr << "1by1 validations: " << w5.validations
         << ", 1by1 aborts: " << w5.naborts << endl;
    cerr << "scan-readonly validations: " << w6.validations
         << ", scan-readonly aborts: " << w6.naborts << endl;
    cerr << "1by1-readonly validations: " << w7.validations
         << ", 1by1-readonly aborts: " << w7.naborts << endl;
      ...
      try {
        ...
        if ((n % 2) == 0) {
          ...
          for (unsigned int i = 0; i < NDoctors && ctr < 2; i++) {
            string v;
            ALWAYS_ASSERT_COND_IN_TXN(t,
                                      this->btr->search(t, u64_varkey(i), v));
...
static void mp_test_simple_write_skew() {
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags);
    ...
    for (size_t i = 0; i < NDoctors; i++) {
      workers[i]->join();
      if (get_worker<TxnType, Traits> *w =
              dynamic_cast<get_worker<TxnType, Traits> *>(workers[i]))
        n_get_succ += w->n;
      else if (scan_worker<TxnType, Traits> *w =
                   dynamic_cast<scan_worker<TxnType, Traits> *>(workers[i]))
        n_scan_succ += w->n;
      else
        ALWAYS_ASSERT(false);
      delete workers[i];
...
class report_worker : public ndb_thread,
                      public txn_btree<TxnType>::search_range_callback {
public:
  report_worker(txn_btree<TxnType> &ctrl, txn_btree<TxnType> &receipts,
                uint64_t txn_flags)
      : ctrl(&ctrl), receipts(&receipts), txn_flags(txn_flags), n(0), m(0),
        sum(0) {}
  virtual void run() {
    while (running) {
      try {
        ...
        sum = 0;
        const string endkey =
            MakeKey(prev_bid, numeric_limits<uint32_t>::max());
...
public:
  new_receipt_worker(txn_btree<TxnType> &ctrl, txn_btree<TxnType> &receipts,
                     uint64_t txn_flags)
      : ctrl(&ctrl), receipts(&receipts), txn_flags(txn_flags), n(0),
        last_bid(0), last_rid(0) {}
  virtual void run() {
    while (running) {
      try {
        typename Traits::StringAllocator arena;
...
public:
  incr_worker(txn_btree<TxnType> &ctrl, txn_btree<TxnType> &receipts,
              uint64_t txn_flags)
      : ctrl(&ctrl), receipts(&receipts), txn_flags(txn_flags), n(0) {}
  virtual void run() {
    struct timespec t;
  ...
  for (size_t txn_flags_idx = 0; txn_flags_idx < ARRAY_NELEMS(TxnFlags);
...

    {
      ...
      for (size_t i = 0; i < nkeys / nkeyspertxn; i++) {
        TxnType<Traits> t;
        const size_t end =
            (i == (nkeys / nkeyspertxn - 1)) ? nkeys : ((i + 1) * nkeyspertxn);
    ...
    double avg_per_core_throughput =
        agg_throughput / double(ARRAY_NELEMS(seeds));
...
void txn_btree_test() {
  ...
  mp_stress_test_insert_removes<transaction_proto2,
                                default_transaction_traits>();
