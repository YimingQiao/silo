...
public:
  testrunner_base(const lcdf::String &name) : name_(name), next_(0) {
  ...
  const lcdf::String &name() const { return name_; }

  static testrunner_base *first() { return thehead; }
...
public:
  inline testrunner(const lcdf::String &name) : testrunner_base(name) {}
...
#define MAKE_TESTRUNNER(name, text)                                            \
  ...
  public:                                                                      \
    ...
    void run(TESTRUNNER_CLIENT_TYPE client) {                                  \
      text;                                                                    \
      client.finish();                                                         \
    }                                                                          \
  };                                                                           \
