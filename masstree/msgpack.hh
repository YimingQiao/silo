...
inline bool is_null_or_bool(uint8_t x) { return in_range(x, fnull, 4); }

inline bool is_bool(uint8_t x) { return in_range(x, ffalse, 2); }

inline bool is_fixstr(uint8_t x) { return in_range(x, ffixstr, nfixstr); }

inline bool is_fixarray(uint8_t x) { return in_range(x, ffixarray, nfixarray); }

inline bool is_fixmap(uint8_t x) { return in_range(x, ffixmap, nfixmap); }
...
struct array_t {
  ...
  array_t(uint32_t s) : size(s) {}
};

inline array_t array(uint32_t size) { return array_t(size); }

struct object_t {
  uint32_t size;

  object_t(uint32_t s) : size(s) {}
};

inline object_t object(uint32_t size) { return object_t(size); }

template <typename T> class unparser {
public:
  inline unparser(T &base) : base_(base) {}

  template <typename X> inline unparser(T &base, const X &x) : base_(base) {
    *this << x;
  }
  ...
  inline unparser<T> &operator<<(const Json::null_t &) { return null(); }
  ...
  template <typename X> inline unparser<T> &write(const X &x) {
    return *this << x;
  }
...
public:
  ...
  explicit inline parser(const unsigned char *s) : s_(s), str_() {}
  ...
  template <typename T> inline parser &read_int(T &x) {
  ...
  inline parser &operator>>(int &x) { return read_int(x); }

  inline parser &operator>>(long &x) { return read_int(x); }

  inline parser &operator>>(long long &x) { return read_int(x); }

  inline parser &operator>>(unsigned &x) { return read_int(x); }

  inline parser &operator>>(unsigned long &x) { return read_int(x); }

  inline parser &operator>>(unsigned long long &x) { return read_int(x); }
  ...
  inline parser &skip_primitive() { return skip_primitives(1); }
...
template <typename T> void parser::hard_read_int(T &x) {
  switch (*s_) {
  case format::fuint8:
    x = s_[1];
    s_ += 2;
...
template <typename T> unparser<T> &unparser<T>::operator<<(const Json &j) {
  if (j.is_null())
    base_.append(char(format::fnull));
  else if (j.is_b())
...
template <typename T, typename X> inline T &unparse(T &s, const X &x) {
  unparser<T>(s) << x;
  return s;
}

template <typename T, typename X> inline T &unparse_wide(T &s, const X &x) {
  unparser<T>(s).write_wide(x);
  return s;
}

template <typename T> parser &parser::operator>>(::std::vector<T> &x) {
  uint32_t sz;
...
inline streaming_parser::streaming_parser() : state_(st_normal) {}
...
inline bool streaming_parser::done() const { return state_ < 0; }

inline bool streaming_parser::success() const { return state_ == st_final; }

inline bool streaming_parser::error() const { return state_ == st_error; }
...
inline Json &streaming_parser::result() { return json_; }

inline const Json &streaming_parser::result() const { return json_; }
