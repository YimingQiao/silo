...
namespace msgpack {
  ...
  namespace {
    const uint8_t nbytes[] = {
        ...
        0,
        0,
        /* 0xD9-0xDB fstr8-fstr32 */ 2,
        3,
        5,
        /* 0xDC-0xDD farray16-farray32 */ 3,
        5,
        /* 0xDE-0xDF fmap16-fmap32 */ 3,
        5};
  }

  const uint8_t *streaming_parser::consume(const uint8_t *first, const uint8_t *last, const String &str) {
    using std::swap;
    Json *jx;
    int n = 0;

    if (state_ < 0) return first;
    if (state_ == st_partial || state_ == st_string) {
      int nneed;
      if (state_ == st_partial) nneed = nbytes[str_.udata()[0] - 0xC0];
      else
        nneed = stack_.back().size;
      const uint8_t *next;
      if (last - first < nneed - str_.length()) next = last;
      else
        next = first + (nneed - str_.length());
      str_.append(first, next);
      if (str_.length() != nneed) return next;
      first = next;
      if (state_ == st_string) {
        stack_.pop_back();
        ...
        *jx = str_;
        ...
        if (state_ != st_normal) return next;
      }
    }
    ...
    while (first != last) {
      ...
      map:
        ...
        ++first;
      array:
        if (!jx->is_a()) *jx = Json::make_array_reserve(n);
        jx->resize(n);
      ...
      raw:
        ...
        if (first < str.ubegin() || first + n >= str.uend()) *jx = String(first, n);
        else {
    ...
    next:
      ...
      if (jx->is_a() && n != 0) stack_.push_back(selem{&jx->at_insert(0), n - 1});
      else if (jx->is_o() && n != 0)
        stack_.push_back(selem{&jokey_, n - 1});
      else {
        while (!stack_.empty() && stack_.back().size == 0) stack_.pop_back();
        if (stack_.empty()) {
          state_ = st_final;
          return first;
        }
  ...
  parser &parser::operator>>(String &x) {
    Str s;
    *this >> s;
    if (str_) x = str_.substring(s.begin(), s.end());
    else
