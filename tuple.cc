...
event_avg_counter
    dbtuple::g_evt_avg_dbtuple_read_retries("avg_dbtuple_read_retries");
...
event_counter
    dbtuple::g_evt_dbtuple_physical_deletes("dbtuple_physical_deletes");
...
event_counter dbtuple::g_evt_dbtuple_inplace_buf_insufficient(
    "dbtuple_inplace_buf_insufficient");
event_counter dbtuple::g_evt_dbtuple_inplace_buf_insufficient_on_spill(
    "dbtuple_inplace_buf_insufficient_on_spill");
...
dbtuple::~dbtuple() {
  ...
  VERBOSE(cerr << "dbtuple: " << hexify(intptr_t(this)) << " is being deleted"
               << endl);
...
void dbtuple::gc_this() {
  INVARIANT(rcu::s_instance.in_rcu_region());
  INVARIANT(!is_latest());
  release(this);
}

string dbtuple::VersionInfoStr(version_t v) {
  ostringstream buf;
  buf << "[";
...
static ostream &format_tuple(ostream &o, const dbtuple &t) {
  string truncated_contents((const char *)&t.value_start[0],
                            min(static_cast<size_t>(t.size), 16UL));
  o << &t << " [tid=" << g_proto_version_str(t.version) << ", size=" << t.size
    << ", contents=0x" << hexify(truncated_contents)
    << (t.size > 16 ? "..." : "") << ", next=" << t.next << "]";
  return o;
}

void dbtuple::print(ostream &o, unsigned len) const {
  o << "dbtuple:" << endl
  ...
  size_t n = 0;
  for (const dbtuple *p = this; p && n < len; p = p->get_next(), ++n) {
    o << "  ";
    format_tuple(o, *p);
    o << endl;
  }
...
ostream &operator<<(ostream &o, const dbtuple &t) {
  t.print(o, 1);
  return o;
}
