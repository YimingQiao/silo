...
event_avg_counter dbtuple::g_evt_avg_dbtuple_stable_version_spins("avg_dbtuple_stable_version_spins");
event_avg_counter dbtuple::g_evt_avg_dbtuple_lock_acquire_spins("avg_dbtuple_lock_acquire_spins");
event_avg_counter dbtuple::g_evt_avg_dbtuple_read_retries("avg_dbtuple_read_retries");
...
static ostream &format_tuple(ostream &o, const dbtuple &t) {
  string truncated_contents((const char *) &t.value_start[0], min(static_cast<size_t>(t.size), 16UL));
  o << &t << " [tid=" << g_proto_version_str(t.version) << ", size=" << t.size << ", contents=0x"
    << hexify(truncated_contents) << (t.size > 16 ? "..." : "") << ", next=" << t.next << "]";
  return o;
}
