...
static inline constexpr std::size_t ceil_log2_const(std::size_t x,
                                                    bool exact = true) {
  return (x == 0) ? (1 / x)
         : (x == 1)
             ? (exact ? 0 : 1)
...
static inline std::size_t round_up_to_pow2(std::size_t x) {
  auto bits = sizeof(long long) * 8 - __builtin_clzll(x);
  if (x == (std::size_t)1 << (bits - 1))
    return x;
...
static inline constexpr std::size_t round_up_to_pow2_const(std::size_t x) {
