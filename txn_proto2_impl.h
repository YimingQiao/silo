...
public:
  ...
  static void Init(size_t nworkers, const std::vector<std::string> &logfiles,
                   const std::vector<std::vector<unsigned>> &assignments_given,
                   std::vector<std::vector<unsigned>> *assignments_used = nullptr, bool call_fsync = true,
                   bool use_compression = false, bool fake_writes = false);
  ...
  struct pbuffer {
    ...
    pbuffer(unsigned core_id, unsigned buf_sz) : core_id_(core_id), buf_sz_(buf_sz) {
  ...
  static bool AssignmentsValid(const std::vector<std::vector<unsigned>> &assignments, unsigned nfds,
                               unsigned nworkers) {
...
private:
  ...
  struct persist_stats {
    ...
    // how many txns have been pushed to the logger (but not necessarily
    // persisted)
  ...
  static inline persist_ctx &persist_ctx_for(uint64_t core_id, InitMode imode) {
    ...
    if (unlikely(!ctx.init_ && imode != INITMODE_NONE)) {
      ...
      if (IsCompressionEnabled()) needed += size_t(LZ4_create_size()) + sizeof(pbuffer) + g_horizon_buffer_size;
...
public:
  ...
  static inline uint64_t constexpr to_read_only_tick(uint64_t epoch_tick) {
  ...
  static void wait_an_epoch() {
    ...
    if (!e) {
      std::cerr << "wait_an_epoch(): consistent reads happening in e-1, but "
                   "e=0 so special case"
                << std::endl;
    } else {
      std::cerr << "wait_an_epoch(): consistent reads happening in e-1: " << (e - 1) << std::endl;
    }
...
protected:
  struct delete_entry {
    ...
    delete_entry(dbtuple *tuple_ahead, uint64_t trigger_tid, dbtuple *tuple, const marked_ptr<std::string> &key,
  ...
  static inline size_t push_horizon_to_buffer(txn_logger::pbuffer *horizon, void *lz4ctx,
    ...
    util::timer tt;
#endif
    const int ret = LZ4_compress_heap_limitedOutput(lz4ctx, (const char *) horizon->datastart(),
                                                    (char *) px->pointer() + sizeof(uint32_t), horizon->datasize(),
...
#if defined(CHECK_INVARIANTS) && defined(PARANOID_CHECKING)
    {
      ...
      const int decode_ret =
          LZ4_decompress_safe_partial((const char *) px->pointer() + sizeof(uint32_t), (char *) &decode_buf[0], ret,
                                      txn_logger::g_horizon_buffer_size, txn_logger::g_horizon_buffer_size);
      INVARIANT(decode_ret >= 0);
      INVARIANT(size_t(decode_ret) == horizon->datasize());
      INVARIANT(memcmp(horizon->datastart(), &decode_buf[0], decode_ret) == 0);
    }
#endif
...
bool txn_logger::pbuffer::can_hold_tid(uint64_t tid) const {
  return !header()->nentries_ ||
         (transaction_proto2_static::EpochId(header()->last_tid_) == transaction_proto2_static::EpochId(tid));
}

// protocol 2 - no global consistent TIDs
template<typename Traits>
class transaction_proto2 : public transaction<transaction_proto2, Traits>, private transaction_proto2_static {
...
public:
  ...
  transaction_proto2(uint64_t flags, typename Traits::StringAllocator &sa)
      : transaction<transaction_proto2, Traits>(flags, sa) {
    if (this->get_flags() & transaction_base::TXN_FLAG_READ_ONLY) {
      const uint64_t global_tick_ex = this->rcu_guard_->guard()->impl().global_last_tick_exclusive();
  ...
  inline bool can_overwrite_record_tid(tid_t prev, tid_t cur) const {
    INVARIANT(prev <= cur);

#ifdef PROTO2_CAN_DISABLE_SNAPSHOTS
    if (!IsSnapshotsEnabled()) return true;
#endif
    ...
    return (to_read_only_tick(EpochId(prev)) == to_read_only_tick(EpochId(cur))) || !prev;
  }

  // can only read elements in this epoch or previous epochs
  inline bool can_read_tid(tid_t t) const { return true; }

  inline void on_tid_finish(tid_t commit_tid) {
    if (!txn_logger::IsPersistenceEnabled() || this->state != transaction_base::TXN_COMMITED) return;
    ...
    for (unsigned idx = 0; idx < nwrites; idx++) {
      ...
      const uint32_t v_nbytes =
          rec.get_value() ? rec.get_writer()(dbtuple::TUPLE_WRITER_COMPUTE_DELTA_NEEDED, rec.get_value(), nullptr, 0)
                          : 0;
    ...
    txn_logger::persist_ctx &ctx = txn_logger::persist_ctx_for(my_core_id, txn_logger::INITMODE_REG);
    txn_logger::persist_stats &stats = txn_logger::g_persist_stats[my_core_id];
    ...
    if (do_compress) {
      ...
      if (ctx.horizon_->space_remaining() < space_needed || (horizon_cond = !ctx.horizon_->can_hold_tid(commit_tid))) {
        ...
        const uint64_t npushed = push_horizon_to_buffer(ctx.horizon_, ctx.lz4ctx_, pull_buf, push_buf);
        if (npushed) util::non_atomic_fetch_add(stats.ntxns_pushed_, npushed);
      }

      INVARIANT(ctx.horizon_->space_remaining() >= space_needed);
      const uint64_t written = write_current_txn_into_buffer(ctx.horizon_, commit_tid, value_sizes);
      if (written != space_needed) INVARIANT(false);
    ...
    retry:
      ...
      bool cond = false;
      if (px->space_remaining() < space_needed || (cond = !px->can_hold_tid(commit_tid))) {
        INVARIANT(px->header()->nentries_);
        ...
        push_buf.enq(px0);
        if (cond) ++txn_logger::g_evt_log_buffer_epoch_boundary;
        else
          ++txn_logger::g_evt_log_buffer_out_of_space;
        goto retry;
      }

      const uint64_t written = write_current_txn_into_buffer(px, commit_tid, value_sizes);
      if (written != space_needed) INVARIANT(false);
    }
  }
...
private:
  ...
  inline uint64_t write_current_txn_into_buffer(txn_logger::pbuffer *px, uint64_t commit_tid,
                                                const write_set_u32_vec &value_sizes) {
    INVARIANT(px->can_hold_tid(commit_tid));

    if (unlikely(!px->header()->nentries_)) px->earliest_start_us_ = this->rcu_guard_->guard()->start_us();
...
public:
  inline ALWAYS_INLINE bool is_snapshot() const { return this->get_flags() & transaction_base::TXN_FLAG_READ_ONLY; }

  inline transaction_base::tid_t snapshot_tid() const {
#ifdef PROTO2_CAN_DISABLE_SNAPSHOTS
  ...
  void dump_debug_info() const {
    ...
    if (this->is_snapshot())
      std::cerr << "  last_consistent_tid: " << g_proto_version_str(u_.last_consistent_tid) << std::endl;
  }

  transaction_base::tid_t gen_commit_tid(const dbtuple_write_info_vec &write_tuples) {
    ...
    if (u_.commit_epoch != EpochId(ret)) ret = MakeTid(0, 0, u_.commit_epoch);

    // What is this? Is txn_proto1_impl used?
    if (g_hack->status_.load(std::memory_order_acquire)) g_hack->global_tid_.fetch_add(1, std::memory_order_acq_rel);
    ...
    // tid_t ret = 0;
    {
      ...
      for (; it != it_end; ++it) {
        if (it->get_tid() > ret) ret = it->get_tid();
      }
    }

    {
      ...
      for (; it != it_end; ++it) {
        ...
        // abort isn't necessary (we really should just write the value in the
        // correct position)
        ...
        if (t != dbtuple::MAX_TID && t > ret) ret = t;
      }
  ...
  inline ALWAYS_INLINE void on_dbtuple_spill(dbtuple *tuple_ahead, dbtuple *tuple) {
#ifdef PROTO2_CAN_DISABLE_GC
    ...
    ctx.queue_.enqueue(delete_entry(tuple_ahead, tuple_ahead->version, tuple, marked_ptr<std::string>(), nullptr),
                       ro_tick);
  }

  inline ALWAYS_INLINE void on_logical_delete(dbtuple *tuple, const std::string &key, concurrent_btree *btr) {
#ifdef PROTO2_CAN_DISABLE_GC
    if (!IsGCEnabled()) return;
#endif
    ...
    if (likely(key.size() <= tuple->alloc_size)) {
      ...
      ctx.queue_.enqueue(delete_entry(nullptr, tuple->version, tuple, mpx, btr), ro_tick);
    } else {
  ...
  void on_post_rcu_region_completion() {
    ...
    if (unlikely(!last_tick_ex)) return;
...
struct base_txn_btree_handler<transaction_proto2> {
  static inline void on_construct() {
...
struct txn_epoch_sync<transaction_proto2> : public transaction_proto2_static {
  static void sync() {
    wait_an_epoch();
    if (txn_logger::IsPersistenceEnabled()) txn_logger::wait_until_current_point_persisted();
  }
  static void finish() {
    if (txn_logger::IsPersistenceEnabled()) txn_logger::wait_until_current_point_persisted();
  }
  static void thread_init(bool loader) {
    if (!txn_logger::IsPersistenceEnabled()) return;
    const unsigned long my_core_id = coreid::core_id();
    // try to initialize using numa allocator
    txn_logger::persist_ctx_for(my_core_id, loader ? txn_logger::INITMODE_REG : txn_logger::INITMODE_RCU);
  }
  static void thread_end() {
    if (!txn_logger::IsPersistenceEnabled()) return;
    const unsigned long my_core_id = coreid::core_id();
    txn_logger::persist_ctx &ctx = txn_logger::persist_ctx_for(my_core_id, txn_logger::INITMODE_NONE);
    if (unlikely(!ctx.init_)) return;
    txn_logger::persist_stats &stats = txn_logger::g_persist_stats[my_core_id];
    txn_logger::pbuffer_circbuf &pull_buf = ctx.all_buffers_;
    txn_logger::pbuffer_circbuf &push_buf = ctx.persist_buffers_;
    if (txn_logger::IsCompressionEnabled() && ctx.horizon_->header()->nentries_) {
      INVARIANT(ctx.horizon_->datasize());
      const uint64_t npushed = push_horizon_to_buffer(ctx.horizon_, ctx.lz4ctx_, pull_buf, push_buf);
      if (npushed) util::non_atomic_fetch_add(stats.ntxns_pushed_, npushed);
    }
  ...
  static std::tuple<uint64_t, uint64_t, double> compute_ntxn_persisted() {
    if (!txn_logger::IsPersistenceEnabled()) return std::make_tuple(0, 0, 0.0);
