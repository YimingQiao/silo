...
private:
  ...
  struct persist_ctx {
    ...
    circbuf<pbuffer, g_perthread_buffers>
        persist_buffers_; // core pushes to logger
  ...
  struct persist_stats {
    ...
    persist_stats()
        : ntxns_persisted_(0), ntxns_pushed_(0), ntxns_committed_(0),
          latency_numer_(0) {}
  };

  // helpers

  static void advance_system_sync_epoch(
  ...
  static inline persist_ctx &persist_ctx_for(uint64_t core_id, InitMode imode) {
    ...
    if (unlikely(!ctx.init_ && imode != INITMODE_NONE)) {
      ...
      if (IsCompressionEnabled())
        needed +=
            size_t(LZ4_create_size()) + sizeof(pbuffer) + g_horizon_buffer_size;
      char *mem = (imode == INITMODE_REG)
                      ? (char *)malloc(needed)
...
static inline std::ostream &operator<<(std::ostream &o,
                                       txn_logger::logbuf_header &hdr) {
  o << "{nentries_=" << hdr.nentries_
    << ", last_tid_=" << g_proto_version_str(hdr.last_tid_) << "}";
  return o;
}
...
public:
  ...
  static inline ALWAYS_INLINE uint64_t CoreId(uint64_t v) {
    return v & CoreMask;
  }

  static inline ALWAYS_INLINE uint64_t NumId(uint64_t v) {
    return (v & NumIdMask) >> NumIdShift;
  }

  static inline ALWAYS_INLINE uint64_t EpochId(uint64_t v) {
  ...
  static void wait_an_epoch() {
    ...
    const uint64_t e =
        to_read_only_tick(ticker::s_instance.global_last_tick_exclusive());
    if (!e) {
    ...
    while (to_read_only_tick(ticker::s_instance.global_last_tick_exclusive()) ==
           e)
      nop_pause();
  ...
  static inline ALWAYS_INLINE uint64_t MakeTid(uint64_t core_id,
                                               uint64_t num_id,
                                               uint64_t epoch_id) {
  ...
  static inline void set_hack_status(bool hack_status) {
...
protected:
  struct delete_entry {
    ...
    delete_entry(dbtuple *tuple_ahead, uint64_t trigger_tid, dbtuple *tuple,
                 const marked_ptr<std::string> &key, concurrent_btree *btr)
        :
#ifdef CHECK_INVARIANTS
  ...
  push_horizon_to_buffer(txn_logger::pbuffer *horizon, void *lz4ctx,
    ...
    const int ret = LZ4_compress_heap_limitedOutput(
        lz4ctx, (const char *)horizon->datastart(),
...
#if defined(CHECK_INVARIANTS) && defined(PARANOID_CHECKING)
    {
      ...
      const int decode_ret = LZ4_decompress_safe_partial(
          (const char *)px->pointer() + sizeof(uint32_t),
          (char *)&decode_buf[0], ret, txn_logger::g_horizon_buffer_size,
  ...
  inline void on_tid_finish(tid_t commit_tid) {
    ...
    for (unsigned idx = 0; idx < nwrites; idx++) {
      ...
      const uint32_t v_nbytes =
          rec.get_value()
              ? rec.get_writer()(dbtuple::TUPLE_WRITER_COMPUTE_DELTA_NEEDED,
                                 rec.get_value(), nullptr, 0)
              : 0;
    ...
    if (do_compress) {
      ...
      if (ctx.horizon_->space_remaining() < space_needed ||
        ...
        if (!ctx.horizon_->datasize()) {
          ...
          std::cerr << "space_remaining: " << ctx.horizon_->space_remaining()
                    << std::endl;
          std::cerr << "can_hold_tid: "
                    << ctx.horizon_->can_hold_tid(commit_tid) << std::endl;
        }
        INVARIANT(ctx.horizon_->datasize());
        // horizon out of space, so we push it
        const uint64_t npushed = push_horizon_to_buffer(
            ctx.horizon_, ctx.lz4ctx_, pull_buf, push_buf);
        if (npushed)
    ...
    retry:
      ...
      if (px->space_remaining() < space_needed ||
        ...
        util::non_atomic_fetch_add(stats.ntxns_pushed_,
                                   px0->header()->nentries_);
        push_buf.enq(px0);
        if (cond)
          ++txn_logger::g_evt_log_buffer_epoch_boundary;
        else
...
private:
  ...
  inline uint64_t
  write_current_txn_into_buffer(txn_logger::pbuffer *px, uint64_t commit_tid,
    ...
    for (unsigned idx = 0; idx < nwrites; idx++) {
      ...
      if (v_nbytes) {
        rec.get_writer()(dbtuple::TUPLE_WRITER_DO_DELTA_WRITE, rec.get_value(),
                         p, v_nbytes);
        p += v_nbytes;
      }
    }
...
public:
  ...
  gen_commit_tid(const dbtuple_write_info_vec &write_tuples) {
...

    {
      ...
      typename dbtuple_write_info_vec::const_iterator it_end =
          write_tuples.end();
      for (; it != it_end; ++it) {
  ...
  inline ALWAYS_INLINE void on_dbtuple_spill(dbtuple *tuple_ahead,
                                             dbtuple *tuple) {
#ifdef PROTO2_CAN_DISABLE_GC
    if (!IsGCEnabled())
      return;
...
#ifdef CHECK_INVARIANTS
    uint64_t exp = 0;
    INVARIANT(tuple->opaque.compare_exchange_strong(exp, 1,
    ...
    ctx.queue_.enqueue(delete_entry(tuple_ahead, tuple_ahead->version, tuple,
                                    marked_ptr<std::string>(), nullptr),
                       ro_tick);
  }

  inline ALWAYS_INLINE void on_logical_delete(dbtuple *tuple,
    ...
    if (likely(key.size() <= tuple->alloc_size)) {
      ...
      ctx.queue_.enqueue(delete_entry(nullptr, tuple->version, tuple, mpx, btr),
                         ro_tick);
    } else {
...
template <> struct base_txn_btree_handler<transaction_proto2> {
...
struct txn_epoch_sync<transaction_proto2> : public transaction_proto2_static {
  ...
  static void thread_init(bool loader) {
    ...
    txn_logger::persist_ctx_for(my_core_id, loader ? txn_logger::INITMODE_REG
                                                   : txn_logger::INITMODE_RCU);
  }
