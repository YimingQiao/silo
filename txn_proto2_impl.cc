...
txn_logger::epoch_array txn_logger::per_thread_sync_epochs_[txn_logger::g_nmax_loggers];
...
event_counter txn_logger::g_evt_log_buffer_epoch_boundary("log_buffer_epoch_boundary");
event_counter txn_logger::g_evt_log_buffer_out_of_space("log_buffer_out_of_space");
event_counter txn_logger::g_evt_log_buffer_bytes_before_compress("log_buffer_bytes_before_compress");
event_counter txn_logger::g_evt_log_buffer_bytes_after_compress("log_buffer_bytes_after_compress");
event_counter txn_logger::g_evt_logger_writev_limit_met("logger_writev_limit_met");
event_counter txn_logger::g_evt_logger_max_lag_wait("logger_max_lag_wait");
event_avg_counter txn_logger::g_evt_avg_log_buffer_compress_time_us("avg_log_buffer_compress_time_us");
event_avg_counter txn_logger::g_evt_avg_log_entry_ntxns("avg_log_entry_ntxns_per_entry");
event_avg_counter txn_logger::g_evt_avg_logger_bytes_per_writev("avg_logger_bytes_per_writev");
event_avg_counter txn_logger::g_evt_avg_logger_bytes_per_sec("avg_logger_bytes_per_sec");
...
void txn_logger::Init(size_t nworkers, const vector<string> &logfiles,
                      const vector<vector<unsigned>> &assignments_given, vector<vector<unsigned>> *assignments_used,
  ...
  if (assignments_used) *assignments_used = assignments;
}
...
void txn_logger::advance_system_sync_epoch(const vector<vector<unsigned>> &assignments) {
  ...
  for (size_t i = 0; i < assignments.size(); i++)
    for (auto j: assignments[i])
      for (size_t k = j; k < NMAXCORES; k += g_nworkers) {
          ...
          if (!l.is_locked()) {
            ...
            if (did_lock) {
              if (!ctx.persist_buffers_.peek()) {
                ...
                per_thread_sync_epochs_[i].epochs_[k].store(best_tick_inc, memory_order_release);
                l.unlock();
                continue;
              }
              l.unlock();
        ...
        min_so_far = min(per_thread_sync_epochs_[i].epochs_[k].load(memory_order_acquire), min_so_far);
      }
  ...
  for (size_t i = 0; i < g_persist_stats.size(); i++) {
    ...
    for (uint64_t e = syssync + 1; e <= min_so_far; e++) {
      ...
      non_atomic_fetch_add(ps.latency_numer_, (now_us - start_us) * ntxns_in_epoch);
...
void txn_logger::writer(unsigned id, int fd, vector<unsigned> assignment) {
  ...
  for (;;) {
    ...
    const uint64_t cur_sync_epoch_ex = system_sync_epoch_->load(memory_order_acquire) + 1;
    ...
    for (auto idx: assignment) {
      ...
      for (size_t k = idx; k < NMAXCORES; k += g_nworkers) {
        ...
        for (auto px: pxs) {
          ...
          if (transaction_proto2_static::EpochId(px->header()->last_tid_) >= cur_sync_epoch_ex + g_max_lag_epochs) {
...
#define PXLEN(px) ((px)->curoff_)
          ...
          const uint64_t px_epoch = transaction_proto2_static::EpochId(px->header()->last_tid_);
          INVARIANT(transaction_proto2_static::CoreId(px->header()->last_tid_) == px->core_id_);
  ...
  process:
    ...
    if (!g_fake_writes) {
...
#ifdef ENABLE_EVENT_COUNTERS
      {
        ...
        const double bytes_per_sec = double(nbyteswritten) / (write_timer.lap_ms() / 1000.0);
    ...
    for (auto idx: assignment) {
      for (size_t k = idx; k < NMAXCORES; k += g_nworkers) {
        ...
        if (x1 > x0) ea.epochs_[k].store(x1, memory_order_release);
        ...
        while ((px = ctx.persist_buffers_.peek()) && px->io_scheduled_) {
#ifdef LOGGER_STRIDE_OVER_BUFFER
          {
            ...
            for (size_t p = 0; p < pxlen; p += stridelen)
              if ((&px->buf_start_[0])[p] & 0xF) non_atomic_fetch_add(ea.dummy_work_, 1UL);
          }
#endif
...
tuple<uint64_t, uint64_t, double> txn_logger::compute_ntxns_persisted_statistics() {
  ...
  INVARIANT(acc <= acc1);
  INVARIANT(acc1 <= acc2);
  if (acc == 0) return make_tuple(0, acc1, 0.0);
...
void txn_logger::wait_for_idle_state() {
  for (size_t i = 0; i < NMAXCORES; i++) {
    ...
    if (!ctx.init_) continue;
    pbuffer *px;
    while (!(px = ctx.all_buffers_.peek()) || px->header()->nentries_) nop_pause();
    while (ctx.persist_buffers_.peek()) nop_pause();
  }
}

void txn_logger::wait_until_current_point_persisted() {
  const uint64_t e = ticker::s_instance.global_current_tick();
  cerr << "waiting for system_sync_epoch_=" << system_sync_epoch_->load(memory_order_acquire) << " to be < e=" << e
       << endl;
  while (system_sync_epoch_->load(memory_order_acquire) < e) nop_pause();
}
/*}}}*/
...
static event_avg_counter evt_avg_time_inbetween_ro_epochs_usec("avg_time_inbetween_ro_epochs_usec");

void transaction_proto2_static::InitGC() { g_flags->g_gc_init.store(true, memory_order_release); }
...
void transaction_proto2_static::PurgeThreadOutstandingGCTasks() {
  ...
  uint64_t e;
  if (!ctx.queue_.get_latest_epoch(e)) return;
...
void transaction_proto2_static::clean_up_to_including(threadctx &ctx, uint64_t ro_tick_geq) {
  INVARIANT(!rcu::s_instance.in_rcu_region());
  INVARIANT(ctx.last_reaped_epoch_ <= ro_tick_geq);
  INVARIANT(ctx.scratch_.empty());
  if (ctx.last_reaped_epoch_ == ro_tick_geq) return;
...
#define ENTER_RCU() \
  do { new (&rcu_guard[0]) scoped_rcu_base<false>(); } while (0)
#define EXIT_RCU()                                                         \
  do {                                                                     \
  ...
  for (auto it = q.begin(); it != q.end(); ++it, ++n, ++niters_with_rcu) {
    ...
    if (!delent.key_.get_flags()) {
...
#ifdef CHECK_INVARIANTS
      if (delent.trigger_tid_ >
          last_consistent_tid /*|| !IsBlocked(delent.tuple_ahead_, delent.tuple(), last_consistent_tid) */) {
      ...
      if (unlikely(!delent.tuple()->is_latest())) {
        ...
        if (unlikely(spx)) ctx.pool_.emplace_back(spx);
        continue;
      }
#ifdef CHECK_INVARIANTS
      delent.tuple()->opaque.store(0, std::memory_order_release);
...
aligned_padded_elem<transaction_proto2_static::flags> transaction_proto2_static::g_flags;
percore_lazy<transaction_proto2_static::threadctx> transaction_proto2_static::g_threadctxs;
event_counter transaction_proto2_static::g_evt_worker_thread_wait_log_buffer("worker_thread_wait_log_buffer");
event_counter transaction_proto2_static::g_evt_dbtuple_no_space_for_delkey("dbtuple_no_space_for_delkey");
event_counter transaction_proto2_static::g_evt_proto_gc_delete_requeue("proto_gc_delete_requeue");
event_avg_counter transaction_proto2_static::g_evt_avg_log_entry_size("avg_log_entry_size");
event_avg_counter transaction_proto2_static::g_evt_avg_proto_gc_queue_len("avg_proto_gc_queue_len");
