...
static inline int sched_getcpu() { return mycpu()->id; }
#endif
...
template <class T> viter<T> begin(const vector<T> &v) {
  return viter<T>(&v, 0);
}

template <class T> viter<T> end(const vector<T> &v) {
...
template <class T, int Nbits> inline uintptr_t log2r(T v) {
  if (Nbits == 8) {
    return log2table[v];
  } else {
    T hi = v >> (Nbits / 2);
...
template <class T> uintptr_t log2(T v) { return log2r<T, sizeof(T) * 8>(v); }

/*
 * ctrgroup: a group of performance counters.
 */

template <typename... Counters> class ctrgroup_chain;

template <> class ctrgroup_chain<> {
public:
  ctrgroup_chain() {}
...
ctrgroup_chain<Counters...> ctrgroup(Counters *...args) {
...
protected:
  ...
  static void print_row(const char *rowname, const Row &r, int w0, int w,
                        Callback f) {
...
public:
  perfsum_tmpl(const ctrgroup_chain<Counters...> *c, const char *n,
...
public:
  perfsum_ctr(const ctrgroup_chain<Counters...> *c, const char *n,
              perfsum_base::display_opt d)
      : perfsum_tmpl<Enabler, Counters...>(c, n, d), base(0) {
    reset();
  }

  perfsum_ctr(const char *n, const perfsum_ctr<Enabler, Counters...> *basesum,
              perfsum_base::display_opt d)
  ...
  void print(int w0, int w) const /* override */ {
    ...
    for (uint i = 0; i < cg->cg_nctr; i++) {
      uint64_t b =
          base ? this->addcpus(base->stat,
                               [&](const stats *s) { return s->sum[i]; })
               : this->addcpus(stat, [&](const stats *s) { return s->count; });
      p.push_back(make_pair(
          b, this->addcpus(stat, [i](const stats *s) { return s->sum[i]; })));
    }

    this->print_row(perfsum_base::name, cg->get_names(), w0, w,
                    [](const char *name) { return name; });
    this->print_row("  avg", p, w0, w,
                    [](const pair<uint64_t, uint64_t> &e)
#if !defined(XV6)
    ...
    this->print_row("  total", p, w0, w,
                    [](const pair<uint64_t, uint64_t> &e) { return e.second; });
    this->print_row("  count", p, w0, w,
                    [](const pair<uint64_t, uint64_t> &e) { return e.first; });
  }

  void reset() /* override */ { memset(stat, 0, sizeof(stat)); }
...
template <typename Enabler, typename... Counters>
class perfsum_ctr_inlinegroup : public ctrgroup_chain<Counters...>,
...
template <typename Enabler = default_enabler, typename... Counters>
perfsum_ctr_inlinegroup<Enabler, Counters...> perfsum_group(const char *name,
                                                            Counters *...c) {
  return perfsum_ctr_inlinegroup<Enabler, Counters...>(name, perfsum_base::show,
                                                       c...);
}

template <typename Enabler, typename... Counters>
perfsum_ctr<Enabler, Counters...>
perfsum_frac(const char *name, const perfsum_ctr<Enabler, Counters...> *base) {
...
public:
  perfsum_hist_tmpl(const ctrgroup_chain<Counters...> *c, const char *n,
                    perfsum_base::display_opt d)
  ...
  void print(int w0, int w) const /* override */ {
    ...
    for (uint i = 0; i < cg->cg_nctr; i++) {
      ...
      for (uint j = 0; j < nbuckets; j++) {
        v.count[j] = this->addcpus(
            stat, [&](const stats *s) { return s->hist[i].count[j]; });
        if (v.count[j]) {
          if (j < first)
            first = j;
          if (j > last)
    ...
    for (uint i = first; i <= last; i++) {
      ...
      this->print_row(n, p, w0, w,
                      [&](const buckets &b) { return b.count[i]; });
    }
 ...
 */
template <uint64_t CounterWidth> class namedctr {
public:
  namedctr(const char *n) : name(n) {}
  void setup() {}
  const char *name;
  static const uint64_t mask = ((1ULL << (CounterWidth - 1)) - 1) << 1 | 1;
};
...
template <uint64_t CounterWidth> class pmc_ctr : public namedctr<CounterWidth> {
public:
...
public:
  pmc_setup(uint64_t v, const char *nm) : pmc_ctr<CounterWidth>(nm), pmc_v(v) {}
 ...
 */
template <typename Perfsum> class base_perf_region {
public:
...
template <typename Perfsum>
scoped_perf_region<Perfsum> perf_region(Perfsum *ps,
                                        getcpu_fn getcpu = sched_getcpu) {
  return scoped_perf_region<Perfsum>(ps, getcpu);
}

template <typename Perfsum>
killable_perf_region<Perfsum> killable_region(Perfsum *ps,
                                              getcpu_fn getcpu = sched_getcpu) {
