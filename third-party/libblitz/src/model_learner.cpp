...
namespace db_compress {
    ...
    namespace {
        ...
        bool CreateModel(const Schema &schema, const std::vector <size_t> &predictors, size_t target_var,
            ...
            std::unique_ptr <SquIDModel> model(creator->CreateModel(schema.attr_type_, predictors, target_var, err));
            if (model != nullptr) {
    ...
    void RelationModelLearner::FeedTuple(const AttrVector &tuple) {
        for (auto &active_model: active_model_list_) { active_model->FeedAttrs(tuple, 1); }
    }
    ...
    void RelationModelLearner::EndOfData() {
        switch (learner_stage_) {
            ...
            case 1:
                ...
                if (inactive_attr_.size() == schema_.attr_type_.size()) { learner_stage_ = 2; }
        }
        ...
        if (learner_stage_ == 0) {
            ...
            for (size_t i = 0; i < schema_.attr_type_.size(); ++i) {
                ...
                for (size_t attr: model_predictor_list_[i]) {
                    if (inactive_attr_.count(attr) == 0) { learnable = false; }
                }
