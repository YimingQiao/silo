...
namespace db_compress {
  ...
  void RelationDecompressor::InitWithoutIndex() {
    // Number of tuples
    num_total_tuples_ = byte_reader_.Read32Bit();
    // Ordering of attributes
    for (size_t i = 0; i < schema_.attr_type_.size(); ++i) { attr_order_.push_back(byte_reader_.Read16Bit()); }
  ...
  void RelationDecompressor::TransformBytesToTuple(std::vector<uint8_t> *bytes, AttrVector *tuple, int stop_idx) {
    byte_reader_.FillStream(bytes);
    decoder_.InitProbInterval();
    ...
    for (int i = 0; i < stop_idx; ++i) {
      ...
      switch (schema_.attr_type_[attr_index]) {
        case 0: {
          ...
          CategoricalSquID *squid = model->GetSquID(*tuple);
          squid->Decompress(&decoder_, &byte_reader_);
          tuple->attr_[attr_index].value_ = squid->GetResultAttrValue();
          break;
        }
        case 1: {
          auto *model = static_cast<TableNumerical *>(model_[attr_index].get());
