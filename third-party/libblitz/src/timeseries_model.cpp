...
namespace db_compress {
    ...
    const db_compress::AttrValue &db_compress::TimeSeriesSquID::GetResultAttr() {
        ...
        if (target_int_) ans.value_ = (static_cast<int>(res + GetEstValue()));
        else
    ...
    void
    db_compress::TimeSeriesSquID::GetProbIntervals(std::vector<Branch *> &prob_intervals, int &prob_intervals_index,
        ...
        for (int i = 0; i < length; ++i) {
            ...
            numerical_squid_.GetProbIntervals(prob_intervals, prob_intervals_index, AttrValue(time_series[i] - est));
    ...
    TimeSeriesSquID::TimeSeriesSquID(double bin_size, bool target_int)
            : target_int_(target_int),
              numerical_squid_(bin_size, false) {
        if (!target_int_) {
            if (bin_size >= 1 || bin_size == 0) std::cout << "Bin size of double cannot be zero or larger than 1.\n";
            decimal_places_ = 0;
            while (bin_size * 10 < 1) {
                bin_size *= 10;
                decimal_places_++;
    ...
    void TimeSeriesSquID::Decompress(Decoder *decoder, ByteReader *byte_reader, std::vector<double> &time_series,
                                     int length) {
    ...
    void TimeSeriesSquID::WriteARParams(std::vector<Branch *> &prob_intervals, int &prob_intervals_index) {
    ...
    db_compress::TimeSeriesSquID *db_compress::TableTimeSeries::GetSquID(const db_compress::AttrVector &tuple) {
    ...
    db_compress::TableTimeSeries *db_compress::TableTimeSeries::ReadModel(db_compress::ByteReader *byte_reader,
                                                                          const db_compress::Schema &schema,
                                                                          size_t index,
                                                                          bool target_int) {
        ...
        TableTimeSeries *model = new TableTimeSeries(schema.attr_type_, predictor_list, index, bin_size, target_int);
    ...
    SquIDModel *TableTimeSeriesCreator::ReadModel(ByteReader *byte_reader, const Schema &schema, size_t index) {
