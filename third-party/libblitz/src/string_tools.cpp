...
namespace db_compress {
  ...
  void StringSplitter::ParseString(const std::string &str) {
    ...
    if (num_words_ >= words_.size()) throw BufferOverflowException("StringSplitter::ParseString: Vector Overflow.\n");
  }
  ...
  void GlobalDictionary::GetProbIntervals(std::vector<Branch *> &prob_intervals, int &prob_intervals_index,
    ...
    if (term_to_id_.find(word) == term_to_id_.end()) std::cerr << "word " << word << " not in dictionary" << std::endl;
