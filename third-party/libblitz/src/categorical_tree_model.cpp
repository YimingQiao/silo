...
namespace db_compress {
    void db_compress::CategoricalTreeSquid::Init(TableCategorical &group_table, std::vector <TableCategorical> &groups,
                                                 int group_size_bits) {
    ...
    void db_compress::CategoricalTreeSquid::GetProbIntervals(std::vector<Branch *> &prob_intervals,
        ...
        squid_groups_[group_id]->GetProbIntervals(prob_intervals, prob_intervals_index, AttrValue(term_id));
    }
    ...
    void db_compress::TableCategoricalTree::FeedAttrs(const db_compress::AttrValue &attr_val, int count) {
