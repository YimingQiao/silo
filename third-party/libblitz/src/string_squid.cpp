...
namespace db_compress {
  ...
  void MarkovCharDist::EndOfData() {
    ...
    for (int i = 0; i < num_markov_table_; ++i) {
      ...
      if (sum_count == 0) zero_weight_exist = true;
      else {
  ...
  void MarkovCharDist::ReadMarkov(ByteReader *byte_reader) {
    ...
    for (int i = 0; i < num_markov_table_; ++i) {
      ...
      if (stats.weight_[stats.weight_.size() - 1] != 0) stats.rare_branch_handler_.Init(stats.weight_);
  ...
  void MarkovCharDist::GetMarkovProbInterval(std::vector<Branch *> &prob_intervals, int &prob_intervals_index,
  ...
  void MarkovCharDist::MarkovDecompress(Decoder *decoder, ByteReader *byte_reader, std::string &word, int word_length) {
    Reset();
  ...
  void StringSquID::GetProbIntervals(std::vector<Branch *> &prob_intervals, int &prob_intervals_index,
    ...
    stats_.num_terms_squid_->GetProbIntervals(prob_intervals, prob_intervals_index, AttrValue(num_words));
    ...
    for (int word_idx = 0; word_idx < num_words; ++word_idx) {
      ...
      monitor_.UpdateIndex(prob_intervals_index);
      stats_.encoding_method_squid_->GetProbIntervals(prob_intervals, prob_intervals_index, AttrValue(method));
      ...
      monitor_.UpdateIndex(prob_intervals_index);
      stats_.delimiter_type_squid_->GetProbIntervals(prob_intervals, prob_intervals_index, AttrValue(delimiter));
      monitor_.AddInfo(prob_intervals, prob_intervals_index, 3);
      start = end + static_cast<int>(delimiter != splitter_.empty_);
    }
  }

  std::string StringSquID::CheckLocalDict(std::vector<Branch *> &prob_intervals, int &prob_intervals_index,
    ...
    monitor_.UpdateIndex(prob_intervals_index);
    stats_.delta_encoding_->GetProbIntervals(prob_intervals, prob_intervals_index, AttrValue(delta_idx));
    if (delta_idx != 0) {
  ...
  void StringSquID::NormalCompress(std::vector<Branch *> &prob_intervals, int &prob_intervals_index,
    ...
    stats_.word_length_squid_->GetProbIntervals(prob_intervals, prob_intervals_index, AttrValue(branch));
  ...
  void StringSquID::Decompress(Decoder *decoder, ByteReader *byte_reader) {
    ...
    for (int word_idx = 0; word_idx < num_words; ++word_idx) {
      ...
      if (delimiter_id != splitter_.empty_) attr_.String().push_back(splitter_.id2delimiters_[delimiter_id]);
    }
