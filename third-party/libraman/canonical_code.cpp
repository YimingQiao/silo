...
namespace raman {
    ...
    CanonicalCode::CanonicalCode(const std::vector <uint32_t> &codeLens) {
        ...
        if (codeLens.size() < 2) throw std::invalid_argument("At least 2 symbols needed");
        if (codeLens.size() > UINT32_MAX) throw std::length_error("Too many symbols");
        ...
        for (uint32_t cl: codeLengths) {
            ...
            while (cl < currentLevel) {
                if (numNodesAtLevel % 2 != 0) throw std::invalid_argument("Under-full Huffman code tree");
                numNodesAtLevel /= 2;
                currentLevel--;
            }
            numNodesAtLevel++;
        ...
        if (numNodesAtLevel < 1) throw std::invalid_argument("Under-full Huffman code tree");
        if (numNodesAtLevel > 1) throw std::invalid_argument("Over-full Huffman code tree");
    ...
    CanonicalCode::CanonicalCode(const CodeTree &tree, uint32_t symbolLimit) {
        if (symbolLimit < 2) throw std::invalid_argument("At least 2 symbols needed");
        codeLengths = std::vector<uint32_t>(symbolLimit, 0);
        buildCodeLengths(&tree.root_, 0);
    }
    ...
    void CanonicalCode::buildCodeLengths(const Node *node, uint32_t depth) {
        if (dynamic_cast<const InternalNode *>(node) != nullptr) {
            ...
            buildCodeLengths(internalNode->l_.get(), depth + 1);
            buildCodeLengths(internalNode->r_.get(), depth + 1);
        } else if (dynamic_cast<const Leaf *>(node) != nullptr) {
            uint32_t symbol = dynamic_cast<const Leaf *>(node)->sym_;
            if (symbol >= codeLengths.size()) throw std::invalid_argument("Symbol exceeds symbol limit");
    ...
    uint32_t CanonicalCode::getSymbolLimit() const { return static_cast<uint32_t>(codeLengths.size()); }

    uint32_t CanonicalCode::getCodeLength(uint32_t symbol) const {
        if (symbol >= codeLengths.size()) throw std::domain_error("Symbol out of range");
        return codeLengths.at(symbol);
    }

    CodeTree CanonicalCode::toCodeTree() const {
        std::vector <std::unique_ptr<Node>> nodes;
        for (uint32_t i = *std::max_element(codeLengths.cbegin(), codeLengths.cend());;
             i--) {  // Descend through code lengths
            if (nodes.size() % 2 != 0)
            ...
            if (i > 0) {
                uint32_t j = 0;
                for (uint32_t cl: codeLengths) {
                    if (cl == i) newNodes.push_back(std::unique_ptr<Node>(new Leaf(j)));
                    j++;
                }
            }
            ...
            for (std::size_t j = 0; j < nodes.size(); j += 2) {
                newNodes.push_back(std::unique_ptr<Node>(
                        new InternalNode(std::move(nodes.at(j)), std::move(nodes.at(j + 1)))));
            }
        ...
        static_cast<uint32_t>(codeLengths.size()));
        delete root;
        return result;
    }
}  // namespace raman
