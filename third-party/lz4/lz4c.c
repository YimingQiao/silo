...
/*
  ...
  It is not part of LZ4 compression library, it is a user program of LZ4
  library. The license of LZ4 library is BSD. The license of xxHash library is
  BSD. The license of this compression CLI program is GPLv2.
*/
...
#define WELCOME_MESSAGE                                                        \
  "*** %s %s, by %s (%s) ***\n", COMPRESSOR_NAME, COMPRESSOR_VERSION, AUTHOR,  \
      COMPILED
...
static unsigned int LZ4S_GetCheckBits_FromXXH(unsigned int xxh) {
  return (xxh >> 8) & _8BITS;
}
static int LZ4S_isSkippableMagicNumber(unsigned int magic) {
  return (magic & LZ4S_SKIPPABLEMASK) == LZ4S_SKIPPABLE0;
}

int get_fileHandle(char *input_filename, char *output_filename, FILE **pfinput,
                   FILE **pfoutput) {

  if (!strcmp(input_filename, stdinmark)) {
    DISPLAY("Using stdin for input\n");
    *pfinput = stdin;
#ifdef _WIN32 // Need to set stdin/stdout to binary mode specifically for
              // windows
    ...
    *pfoutput = 0;
    ...
    if (*pfoutput != 0) {
      ...
      if (!overwrite) {
        ...
        ch = (char)getchar();
        if (ch != 'Y')
          EXM_THROW(11, "Operation aborted : %s already exists",
                    output_filename);
      }
    }
  ...
  if (*pfinput == 0)
    EXM_THROW(12, "Pb opening %s", input_filename);
  if (*pfoutput == 0)
    EXM_THROW(13, "Pb opening %s", output_filename);

  return 0;
}

int legacy_compress_file(char *input_filename, char *output_filename,
  ...
  *(unsigned int *)out_buff = LITTLE_ENDIAN_32(LEGACY_MAGICNUMBER);
  ...
  while (1) {
    ...
    // Read Block
    int inSize =
        (int)fread(in_buff, (size_t)1, (size_t)LEGACY_BLOCKSIZE, finput);
    if (inSize <= 0)
      break;
    filesize += inSize;
    if (displayLevel)
    ...
    if (displayLevel)
      DISPLAY("Read : %i MB  ==> %.2f%%\r", (int)(filesize >> 20),
    ...
    *(unsigned int *)out_buff = LITTLE_ENDIAN_32(outSize);
    ...
    if (sizeCheck != (size_t)(outSize + 4))
      EXM_THROW(23, "Write error : cannot write compressed block");
  }
...
int compress_file_blockDependency(char *input_filename, char *output_filename,
                                  int compressionlevel) {
  ...
  in_start = in_buff;
  in_end = in_buff + inputBufferSize;
  if (streamChecksum)
    streamChecksumState = XXH32_init(LZ4S_CHECKSUM_SEED);
  ctx = initFunction(in_buff);

  // Write Archive Header
  *(unsigned int *)out_buff = LITTLE_ENDIAN_32(
      LZ4S_MAGICNUMBER); // Magic Number, in Little Endian convention
  ...
  *(out_buff + 6) = (unsigned char)checkbits;
  ...
  if (sizeCheck != header_size)
    EXM_THROW(32, "Write error : cannot write header");
  ...
  while (1) {
    ...
    inSize =
        (unsigned int)fread(in_start, (size_t)1, (size_t)blockSize, finput);
    if (inSize <= 0)
    ...
    // Compress Block
    outSize =
        compressionFunction(ctx, in_start, out_buff + 4, inSize, inSize - 1);
    if (outSize > 0)
      compressedfilesize += outSize + 4;
    else
    ...
    if (outSize > 0) {
      ...
      *(unsigned int *)out_buff = LITTLE_ENDIAN_32(outSize);
      ...
      if (sizeCheck != (size_t)(sizeToWrite))
        EXM_THROW(33, "Write error : cannot write compressed block");

    } else // Copy Original
    {
      unsigned int checksum;
      *(unsigned int *)out_buff =
          LITTLE_ENDIAN_32(inSize | 0x80000000); // Add Uncompressed flag
      sizeCheck = fwrite(out_buff, 1, 4, foutput);
      if (sizeCheck != (size_t)(4))
        EXM_THROW(34, "Write error : cannot write block header");
      sizeCheck = fwrite(in_start, 1, inSize, foutput);
      if (sizeCheck != (size_t)(inSize))
        EXM_THROW(35, "Write error : cannot write block");
      if (blockChecksum) {
        checksum = XXH32(in_start, inSize, LZ4S_CHECKSUM_SEED);
        *(unsigned int *)out_buff = LITTLE_ENDIAN_32(checksum);
        sizeCheck = fwrite(out_buff, 1, 4, foutput);
        if (sizeCheck != (size_t)(4))
          EXM_THROW(36, "Write error : cannot write block checksum");
      }
    }
    in_start += inSize;
  }
  ...
  *(unsigned int *)out_buff = LZ4S_EOS;
  ...
  if (streamChecksum) {
    ...
    *(unsigned int *)out_buff = LITTLE_ENDIAN_32(checksum);
    sizeCheck = fwrite(out_buff, 1, 4, foutput);
    if (sizeCheck != (size_t)(4))
      EXM_THROW(37, "Write error : cannot write stream checksum");
    compressedfilesize += 4;
  }
...
int compress_file(char *input_filename, char *output_filename,
  ...
  // Branch out
  if (blockIndependence == 0)
    return compress_file_blockDependency(input_filename, output_filename,
                                         compressionlevel);
  ...
  switch (compressionlevel) {
  case 0:
    compressionFunction = LZ4_compress_limitedOutput;
    break;
  case 1:
    compressionFunction = LZ4_compressHC_limitedOutput;
    break;
  default:
    compressionFunction = LZ4_compress_limitedOutput;
  }
  ...
  *(out_buff + 6) = (unsigned char)checkbits;
  ...
  while (1) {
    ...
    // Read Block
    unsigned int inSize =
        (unsigned int)fread(in_buff, (size_t)1, (size_t)blockSize, finput);
    if (inSize <= 0)
      break; // No more input : end of compression
    filesize += inSize;
    if (displayLevel)
      DISPLAY("Read : %i MB  \r", (int)(filesize >> 20));
    if (streamChecksum)
      XXH32_update(streamChecksumState, in_buff, inSize);
...
unsigned long long decodeLegacyStream(FILE *finput, FILE *foutput) {
  ...
  if (!in_buff || !out_buff)
    EXM_THROW(51, "Allocation error : not enough memory");
  ...
  while (1) {
    ...
    if (blockSize >
        LZ4_COMPRESSBOUND(
            LEGACY_BLOCKSIZE)) { // Cannot read next block : maybe new stream ?
    ...
    if (sinkint < 0)
      EXM_THROW(52, "Decoding Failed ! Corrupted input detected !");
    filesize += sinkint;

    // Write Block
    sizeCheck = fwrite(out_buff, 1, sinkint, foutput);
    if (sizeCheck != (size_t)sinkint)
      EXM_THROW(53, "Write error : cannot write decoded block into output\n");
  }
...
unsigned long long decodeLZ4S(FILE *finput, FILE *foutput) {
  ...
  if (nbReadBytes != 3)
    EXM_THROW(61, "Unreadable header");
  {
    ...
    if (version != 1)
      EXM_THROW(62, "Wrong version number");
    if (streamSize == 1)
      EXM_THROW(64, "Does not support stream size");
    if (reserved1 != 0)
      EXM_THROW(65, "Wrong value for reserved bits");
    if (dictionary == 1)
      EXM_THROW(66, "Does not support dictionary");
    if (reserved2 != 0)
      EXM_THROW(67, "Wrong value for reserved bits");
    if (blockSizeId < 4)
      EXM_THROW(68, "Unsupported block size");
    if (reserved3 != 0)
      EXM_THROW(67, "Wrong value for reserved bits");
    ...
    if (checkBits != checkBits_xxh32)
      EXM_THROW(69, "Stream descriptor error detected");
  }
  ...
  // Allocate Memory
  {
    ...
    if (outbuffSize < MIN_STREAM_BUFSIZE)
      outbuffSize = MIN_STREAM_BUFSIZE;
    out_buff = (char *)malloc(outbuffSize);
    out_end = out_buff + outbuffSize;
    out_start = out_buff + prefix64k;
    if (!in_buff || !out_buff)
      EXM_THROW(70, "Allocation error : not enough memory");
  }
  ...
  while (1) {
    ...
    if (nbReadBytes != 4)
      EXM_THROW(71, "Read error : cannot read next block size");
    if (blockSize == LZ4S_EOS)
      break; // End of Stream Mark : stream is completed
    blockSize = LITTLE_ENDIAN_32(blockSize); // Convert to little endian
    uncompressedFlag = blockSize >> 31;
    blockSize &= 0x7FFFFFFF;
    if (blockSize > maxBlockSize)
      EXM_THROW(72, "Error : invalid block size");

    // Read Block
    nbReadBytes = fread(in_buff, 1, blockSize, finput);
    if (nbReadBytes != blockSize)
      EXM_THROW(73, "Read error : cannot read data block");
    ...
    if (blockChecksumFlag) {
      ...
      if (sizeCheck != 4)
        EXM_THROW(74, "Read error : cannot read next block size");
      readChecksum = LITTLE_ENDIAN_32(readChecksum); // Convert to little endian
      if (checksum != readChecksum)
        EXM_THROW(75, "Error : invalid block checksum detected");
    }

    if (uncompressedFlag) {
      // Write uncompressed Block
      sizeCheck = fwrite(in_buff, 1, blockSize, foutput);
      if (sizeCheck != (size_t)blockSize)
        EXM_THROW(76, "Write error : cannot write data block");
      filesize += blockSize;
      if (streamChecksumFlag)
        XXH32_update(streamChecksumState, in_buff, blockSize);
      if (!blockIndependenceFlag) {
        if (blockSize >= prefix64k) {
          memcpy(out_buff, in_buff + (blockSize - prefix64k),
                 prefix64k); // Required for reference for next blocks
          out_start = out_buff + prefix64k;
          continue;
        } else {
      ...
      // Decode Block
      decodedBytes =
          decompressionFunction(in_buff, out_start, blockSize, maxBlockSize);
      if (decodedBytes < 0)
        EXM_THROW(77, "Decoding Failed ! Corrupted input detected !");
      filesize += decodedBytes;
      if (streamChecksumFlag)
        XXH32_update(streamChecksumState, out_start, decodedBytes);

      // Write Block
      sizeCheck = fwrite(out_start, 1, decodedBytes, foutput);
      if (sizeCheck != (size_t)decodedBytes)
        EXM_THROW(78, "Write error : cannot write decoded block\n");
    }
  ...
  if (streamChecksumFlag) {
    ...
    sizeCheck = fread(&readChecksum, 1, 4, finput);
    if (sizeCheck != 4)
      EXM_THROW(74, "Read error : cannot read stream checksum");
    readChecksum = LITTLE_ENDIAN_32(readChecksum); // Convert to little endian
    if (checksum != readChecksum)
      EXM_THROW(75, "Error : invalid stream checksum detected");
  }
...
unsigned long long selectDecoder(FILE *finput, FILE *foutput) {
  ...
  if (nbReadBytes != MAGICNUMBER_SIZE)
    EXM_THROW(41, "Unrecognized header : Magic Number unreadable");
  magicNumber =
      LITTLE_ENDIAN_32(magicNumber); // Convert to Little Endian format
  if (LZ4S_isSkippableMagicNumber(magicNumber))
    magicNumber = LZ4S_SKIPPABLE0; // fold skippable magic numbers
  ...
  default:
    if (ftell(finput) == MAGICNUMBER_SIZE)
      EXM_THROW(
          44,
          "Unrecognized header : file cannot be decoded"); // Wrong magic number
                                                           // at the beginning
                                                           // of 1st stream
...
int main(int argc, char **argv) {
      ...
      while (argument[1] != 0) {
        ...
        case 'B':
          while (argument[1] != 0)
            ...
            case 'D':
              blockIndependence = 0, argument++;
              break;
            case 'X':
              blockChecksum = 1, argument++;
              break;
            default:
  ...
  if (!output_filename) {
    ...
    {
      int i = 0, l = 0;
      while (input_filename[l] != 0)
        l++;
      output_filename = (char *)calloc(1, l + 5);
      for (i = 0; i < l; i++)
        output_filename[i] = input_filename[i];
      for (i = l; i < l + 4; i++)
        output_filename[i] = extension[i - l];
    } else // decompression (input file must respect format extension ".lz4")
    {
      int inl = 0, outl;
      while (input_filename[inl] != 0)
        inl++;
      output_filename = (char *)calloc(1, inl + 1);
      for (outl = 0; outl < inl; outl++)
        output_filename[outl] = input_filename[outl];
      if (inl > 4)
        while ((outl >= inl - 4) &&
               (input_filename[outl] == extension[outl - inl + 4]))
  ...
  if (decode)
    return decodeFile(input_filename, output_filename);
