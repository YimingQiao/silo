...
static inline int local_LZ4_compress_limitedOutput(const char *in, char *out,
                                                   int inSize) {
  return LZ4_compress_limitedOutput(in, out, inSize, LZ4_compressBound(inSize));
}

static inline int local_LZ4_compressHC_limitedOutput(const char *in, char *out,
                                                     int inSize) {
  return LZ4_compressHC_limitedOutput(in, out, inSize,
                                      LZ4_compressBound(inSize));
}

static inline int local_LZ4_decompress_fast(const char *in, char *out,
...
static inline int local_LZ4_decompress_fast_withPrefix64k(const char *in,
                                                          char *out, int inSize,
                                                          int outSize) {
  (void)inSize;
...
static inline int local_LZ4_decompress_safe_partial(const char *in, char *out,
                                                    int inSize, int outSize) {
...
int fullSpeedBench(char **fileNamesTable, int nbFiles) {
  ...
  int compressed_buff_size;
  struct chunkParameters *chunkP;
  U32 crcc, crcd = 0;
#define NB_COMPRESSION_ALGORITHMS 4
  static char *compressionNames[] = {
      "LZ4_compress", "LZ4_compress_limitedOutput", "LZ4_compressHC",
...
#define NB_DECOMPRESSION_ALGORITHMS 5
  static char *decompressionNames[] = {
      ...
      "LZ4_decompress_safe", "LZ4_decompress_safe_withPrefix64k",
  ...
  while (fileIdx < nbFiles) {
    ...
    // Alloc
    chunkP = (struct chunkParameters *)malloc(((benchedSize / chunkSize) + 1) *
                                              sizeof(struct chunkParameters));
    ...
    // Bench
    {
      ...
      for (cAlgNb = 0;
           (cAlgNb < NB_COMPRESSION_ALGORITHMS) && (compressionTest);
           cAlgNb++) {
        ...
        for (loopNb = 1; loopNb <= nbIterations; loopNb++) {
          ...
          while (BMK_GetMilliSpan(milliTime) < TIMELOOP) {
            for (chunkNb = 0; chunkNb < nbChunks; chunkNb++) {
              chunkP[chunkNb].compressedSize = compressionFunction(
                  chunkP[chunkNb].origBuffer, chunkP[chunkNb].compressedBuffer,
          ...
          cSize = 0;
          for (chunkNb = 0; chunkNb < nbChunks; chunkNb++)
            cSize += chunkP[chunkNb].compressedSize;
          ratio = (double)cSize / (double)benchedSize * 100.;
          DISPLAY("%1i-%-19.19s : %9i -> %9i (%5.2f%%),%7.1f MB/s\r", loopNb,
                  cName, (int)benchedSize, (int)cSize, ratio,
                  (double)benchedSize / bestTime / 1000.);
        }

        if (ratio < 100.)
          DISPLAY("%-21.21s : %9i -> %9i (%5.2f%%),%7.1f MB/s\n", cName,
                  (int)benchedSize, (int)cSize, ratio,
                  (double)benchedSize / bestTime / 1000.);
        else
      ...
      for (chunkNb = 0; chunkNb < nbChunks; chunkNb++) {
        chunkP[chunkNb].compressedSize = LZ4_compress(
            chunkP[chunkNb].origBuffer, chunkP[chunkNb].compressedBuffer,
            chunkP[chunkNb].origSize);
        if (chunkP[chunkNb].compressedSize == 0)
      ...
      for (dAlgNb = 0;
           (dAlgNb < NB_DECOMPRESSION_ALGORITHMS) && (decompressionTest);
           dAlgNb++) {
        ...
        for (loopNb = 1; loopNb <= nbIterations; loopNb++) {
          ...
          while (BMK_GetMilliSpan(milliTime) < TIMELOOP) {
            for (chunkNb = 0; chunkNb < nbChunks; chunkNb++) {
              int decodedSize = decompressionFunction(
                  chunkP[chunkNb].compressedBuffer, chunkP[chunkNb].origBuffer,
          ...
          DISPLAY("%1i-%-19.19s : %9i -> %7.1f MB/s\r", loopNb, dName,
                  (int)benchedSize, (double)benchedSize / bestTime / 1000.);
        }
  ...
  if (nbFiles > 1) {
    ...
    DISPLAY(" ** TOTAL ** : \n");
    for (AlgNb = 0; (AlgNb < NB_COMPRESSION_ALGORITHMS) && (compressionTest);
         AlgNb++) {
      char *cName = compressionNames[AlgNb];
      DISPLAY("%-21.21s :%10llu ->%10llu (%5.2f%%), %6.1f MB/s\n", cName,
              (long long unsigned int)totals,
              (long long unsigned int)totalCSize[AlgNb],
              (double)totalCSize[AlgNb] / (double)totals * 100.,
              (double)totals / totalCTime[AlgNb] / 1000.);
    }
    for (AlgNb = 0;
         (AlgNb < NB_DECOMPRESSION_ALGORITHMS) && (decompressionTest);
         AlgNb++) {
      char *dName = decompressionNames[AlgNb];
      DISPLAY("%-21.21s :%10llu -> %6.1f MB/s\n", dName,
              (long long unsigned int)totals,
              (double)totals / totalDTime[AlgNb] / 1000.);
    }
  }
