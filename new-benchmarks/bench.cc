...
static void map_agg(map<K, V> &agg, const map<K, V> &m) {
  for (typename map<K, V>::const_iterator it = m.begin(); it != m.end(); ++it) agg[it->first] += it->second;
}
...
void bench_runner::run() {
  ...
  {
    const auto persisted_info = db->get_ntxn_persisted();
    if (get<0>(persisted_info) != 0 || get<1>(persisted_info) != 0 || get<2>(persisted_info) != 0.0) {
  ...
  if (verbose) {
    ...
    cerr << "--- txn counter statistics ---" << endl;
    {
      ...
      for (auto &p: agg) {
        cerr << p.first << ":" << endl;
        for (auto &q: p.second) cerr << "  " << q.first << " : " << q.second << endl;
      }
    }
#endif
  ...
  if (!slow_exit) exit(0);// exit() instead of returning, so we don't call a bunch of dtors

  map<string, uint64_t> agg_stats;
  for (auto &p: open_tables) map_agg(agg_stats, p.second->clear());
  if (verbose)
...
map<string, size_t> bench_worker::get_txn_counts() const {
  map<string, size_t> m;
  const workload_desc_vec workload = get_workload();
  for (size_t i = 0; i < txn_counts.size(); i++) m[workload[i].name] = txn_counts[i];
  return m;
}
