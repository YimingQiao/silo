...
static constexpr inline ALWAYS_INLINE size_t NumDistrictsPerWarehouse() { return 10; }

static constexpr inline ALWAYS_INLINE size_t NumCustomersPerDistrict() { return 3000; }
...
public:
  ...
  static void PinToWarehouseId(unsigned int wid) {
    ...
    if (verbose)
      cerr << "PinToWarehouseId(): coreid=" << coreid::core_id() << " pinned to whse=" << wid << " (partid=" << partid
           << ")" << endl;
  ...
  static inline ALWAYS_INLINE int GetItemId(fast_random &r) {
    return CheckBetweenInclusive(
        g_uniform_item_dist ? RandomNumber(r, 1, NumItems()) : NonUniformRandom(r, 8191, 7911, 1, NumItems()), 1,
        NumItems());
  }
...
public:
  tpcc_worker(unsigned int worker_id, unsigned long seed, Database *db, const tpcc_tables<Database> &tables,
              spin_barrier *barrier_a, spin_barrier *barrier_b, uint warehouse_id_start, uint warehouse_id_end)
    ...
    if (verbose) {
      cerr << "tpcc: worker id " << worker_id << " => warehouses [" << warehouse_id_start << ", " << warehouse_id_end
           << ")" << endl;
    }
  }
  ...
  virtual workload_desc_vec get_workload() const OVERRIDE {
    ...
    if (g_txn_workload_mix[1]) w.push_back(workload_desc("Payment", double(g_txn_workload_mix[1]) / 100.0, TxnPayment));
...
class tpcc_warehouse_loader : public typed_bench_loader<Database>, public tpcc_worker_mixin {
public:
  tpcc_warehouse_loader(unsigned long seed, Database *db, const tpcc_tables<Database> &tables)
...
template<typename Database>
class tpcc_item_loader : public typed_bench_loader<Database>, public tpcc_worker_mixin {
public:
  tpcc_item_loader(unsigned long seed, Database *db, const tpcc_tables<Database> &tables)
      : typed_bench_loader<Database>(seed, db),
        tpcc_worker_mixin(),
        tables(tables) {}
    ...
    try {
      for (uint i = 1; i <= NumItems(); i++) {
        ...
        if (RandomNumber(this->r, 1, 100) > 10) {
          ...
          const string i_data =
              RandomStr(this->r, startOriginal + 1) + "ORIGINAL" + RandomStr(this->r, len - startOriginal - 7);
          v.i_data.assign(i_data);
        }
...
template<typename Database>
class tpcc_stock_loader : public typed_bench_loader<Database>, public tpcc_worker_mixin {
public:
  tpcc_stock_loader(unsigned long seed, Database *db, const tpcc_tables<Database> &tables, ssize_t warehouse_id)
      : typed_bench_loader<Database>(seed, db),
        tpcc_worker_mixin(),
        tables(tables),
  ...
  virtual void load() {
    ...
    for (uint w = w_start; w <= w_end; w++) {
      const size_t batchsize =
          (this->typed_db()->txn_max_batch_size() == -1) ? NumItems() : this->typed_db()->txn_max_batch_size();
...
template<typename Database>
class tpcc_district_loader : public typed_bench_loader<Database>, public tpcc_worker_mixin {
public:
  tpcc_district_loader(unsigned long seed, Database *db, const tpcc_tables<Database> &tables)
      : typed_bench_loader<Database>(seed, db),
        tpcc_worker_mixin(),
        tables(tables) {}
...
template<typename Database>
class tpcc_customer_loader : public typed_bench_loader<Database>, public tpcc_worker_mixin {
public:
  tpcc_customer_loader(unsigned long seed, Database *db, const tpcc_tables<Database> &tables, ssize_t warehouse_id)
      : typed_bench_loader<Database>(seed, db),
        tpcc_worker_mixin(),
        tables(tables),
        warehouse_id(warehouse_id) {
...
protected:
  virtual void load() {
    const uint w_start = (warehouse_id == -1) ? 1 : static_cast<uint>(warehouse_id);
    const uint w_end = (warehouse_id == -1) ? NumWarehouses() : static_cast<uint>(warehouse_id);
    const size_t batchsize = (this->typed_db()->txn_max_batch_size() == -1) ? NumCustomersPerDistrict()
                                                                            : this->typed_db()->txn_max_batch_size();
        ...
        for (uint batch = 0; batch < nbatches;) {
          ...
          try {
            for (uint cidx0 = cstart; cidx0 < cend; cidx0++) {
              ...
              if (RandomNumber(this->r, 1, 100) <= 10) v.c_credit.assign("BC");
              else
                v.c_credit.assign("GC");

              if (c <= 1000) v.c_last.assign(GetCustomerLastName(this->r, c - 1));
              else
...
template<typename Database>
class tpcc_order_loader : public typed_bench_loader<Database>, public tpcc_worker_mixin {
public:
  tpcc_order_loader(unsigned long seed, Database *db, const tpcc_tables<Database> &tables, ssize_t warehouse_id)
      : typed_bench_loader<Database>(seed, db),
        tpcc_worker_mixin(),
        tables(tables),
        warehouse_id(warehouse_id) {
      ...
      for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
        ...
        for (uint c = 1; c <= NumCustomersPerDistrict();) {
          ...
          try {
            ...
            if (k_oo.o_id < 2101) v_oo.o_carrier_id = RandomNumber(this->r, 1, 10);
            else
              v_oo.o_carrier_id = 0;
...
tpcc_worker<Database, AllowReadOnlyScans>::txn_new_order() {
  ...
  try {
    ...
    customer::value v_c;
    ALWAYS_ASSERT(tables.tbl_customer(warehouse_id)
                      ->search(txn, k_c, v_c,
                               GUARDED_FIELDS(customer::value::c_discount_field, customer::value::c_last_field,
    ...
    warehouse::value v_w;
    ALWAYS_ASSERT(
        tables.tbl_warehouse(warehouse_id)->search(txn, k_w, v_w, GUARDED_FIELDS(warehouse::value::w_tax_field)));
    checker::SanityCheckWarehouse(&k_w, &v_w);

    const district::key k_d(warehouse_id, districtID);
    district::value v_d;
    ALWAYS_ASSERT(
        tables.tbl_district(warehouse_id)
            ->search(txn, k_d, v_d, GUARDED_FIELDS(district::value::d_next_o_id_field, district::value::d_tax_field)));
    ...
    if (!g_new_order_fast_id_gen) {
      v_d.d_next_o_id++;
      tables.tbl_district(warehouse_id)->put(txn, k_d, v_d, GUARDED_FIELDS(district::value::d_next_o_id_field));
    }
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      item::value v_i;
      ALWAYS_ASSERT(tables.tbl_item(1)->search(
          txn, k_i, v_i,
          GUARDED_FIELDS(item::value::i_price_field, item::value::i_name_field, item::value::i_data_field)));
    ...
    if (likely(txn.commit())) return txn_result(true, ret);
  } catch (typename Database::abort_exception_type &e) { txn.abort(); }
  return txn_result(false, 0);
}

template<typename Database>
class new_order_scan_callback : public Database::template IndexType<schema<new_order>>::type::search_range_callback {
public:
  new_order_scan_callback() : invoked(false) {}

  virtual bool invoke(const new_order::key &key, const new_order::value &value) OVERRIDE {
    INVARIANT(!invoked);
    k_no = key;
    invoked = true;
#ifdef CHECK_INVARIANTS
  ...
  try {
    ...
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      ...
      new_order_scan_callback<Database> new_order_c;
      {
        ...
        tables.tbl_new_order(warehouse_id)->search_range_call(txn, k_no_0, &k_no_1, new_order_c);
      }

      const new_order::key &k_no = new_order_c.get_key();
      if (unlikely(!new_order_c.was_invoked())) continue;
      last_no_o_ids[d - 1] = k_no.no_o_id + 1;// XXX: update last seen

      const oorder::key k_oo(warehouse_id, d, k_no.no_o_id);
      oorder::value v_oo;
      if (unlikely(!tables.tbl_oorder(warehouse_id)
                        ->search(txn, k_oo, v_oo,
                                 GUARDED_FIELDS(oorder::value::o_c_id_field, oorder::value::o_carrier_id_field)))) {
      ...
      tables.tbl_order_line(warehouse_id)
          ->search_range_call(
              txn, k_oo_0, &k_oo_1, c, false,
              GUARDED_FIELDS(order_line::value::ol_amount_field, order_line::value::ol_delivery_d_field));
      float sum = 0.0;
      for (size_t i = 0; i < c.size(); i++) {
        ...
        tables.tbl_order_line(warehouse_id)
            ->put(txn, c.key(i), c.value(i), GUARDED_FIELDS(order_line::value::ol_delivery_d_field));
      }
      ...
      tables.tbl_oorder(warehouse_id)->put(txn, k_oo, v_oo, GUARDED_FIELDS(oorder::value::o_carrier_id_field));
      ...
      customer::value v_c;
      ALWAYS_ASSERT(
          tables.tbl_customer(warehouse_id)->search(txn, k_c, v_c, GUARDED_FIELDS(customer::value::c_balance_field)));
      v_c.c_balance += ol_total;
      tables.tbl_customer(warehouse_id)->put(txn, k_c, v_c, GUARDED_FIELDS(customer::value::c_balance_field));
    }
...
tpcc_worker<Database, AllowReadOnlyScans>::txn_payment() {
  ...
  if (likely(g_disable_xpartition_txn || NumWarehouses() == 1 || RandomNumber(this->r, 1, 100) <= 85)) {
    customerDistrictID = districtID;
    customerWarehouseID = warehouse_id;
  } else {
    customerDistrictID = RandomNumber(this->r, 1, NumDistrictsPerWarehouse());
    do { customerWarehouseID = RandomNumber(this->r, 1, NumWarehouses()); } while (customerWarehouseID == warehouse_id);
  }
  ...
  if (g_enable_partition_locks) {
    mlock.enq(LockForPartition(warehouse_id));
    if (PartitionId(customerWarehouseID) != PartitionId(warehouse_id)) mlock.enq(LockForPartition(customerWarehouseID));
    mlock.multilock();
  }
  ...
  try {
    ...
    const warehouse::key k_w(warehouse_id);
    warehouse::value v_w;
    ALWAYS_ASSERT(tables.tbl_warehouse(warehouse_id)
                      ->search(txn, k_w, v_w,
                               GUARDED_FIELDS(warehouse::value::w_ytd_field, warehouse::value::w_street_1_field,
                                              warehouse::value::w_street_1_field, warehouse::value::w_city_field,
                                              warehouse::value::w_state_field, warehouse::value::w_zip_field,
    ...
    const district::key k_d(warehouse_id, districtID);
    district::value v_d;
    ALWAYS_ASSERT(tables.tbl_district(warehouse_id)
                      ->search(txn, k_d, v_d,
                               GUARDED_FIELDS(district::value::d_ytd_field, district::value::d_street_1_field,
                                              district::value::d_street_1_field, district::value::d_city_field,
                                              district::value::d_state_field, district::value::d_zip_field,
    ...
    if (RandomNumber(this->r, 1, 100) <= 60) {
      ...
      bytes_static_limit_callback<typename Database::template IndexType<schema<customer_name_idx>>::type,
                                  NMaxCustomerIdxScanElems, true>
          c(s_arena.get());
      tables.tbl_customer_name_idx(customerWarehouseID)->bytes_search_range_call(txn, k_c_idx_0, &k_c_idx_1, c);
...
class order_line_nop_callback
    : public Database::template IndexType<schema<order_line>>::type::bytes_search_range_callback {

public:
  order_line_nop_callback() : n(0) {}

  virtual bool invoke(const string &key, const string &value) {
...
tpcc_worker<Database, AllowReadOnlyScans>::txn_order_status() {
  ...
  //   num_txn_contexts : 4
  const uint64_t read_only_mask = !AllowReadOnlyScans ? 0 : transaction_base::TXN_FLAG_READ_ONLY;
  typename Database::template TransactionType<AllowReadOnlyScans ? abstract_db::HINT_TPCC_ORDER_STATUS_READ_ONLY
                                                                 : abstract_db::HINT_TPCC_ORDER_STATUS>::type
      txn(txn_flags | read_only_mask, this->arena);
  scoped_str_arena s_arena(this->arena);
  ...
  try {
    ...
    if (RandomNumber(this->r, 1, 100) <= 60) {
      ...
      bytes_static_limit_callback<typename Database::template IndexType<schema<customer_name_idx>>::type,
                                  NMaxCustomerIdxScanElems, true>
          c(s_arena.get());
      tables.tbl_customer_name_idx(warehouse_id)->bytes_search_range_call(txn, k_c_idx_0, &k_c_idx_1, c);
      ALWAYS_ASSERT(c.size() > 0);
      INVARIANT(c.size() < NMaxCustomerIdxScanElems);// we should detect this
      int index = c.size() / 2;
      if (c.size() % 2 == 0) index--;
    ...
    latest_key_callback<typename Database::template IndexType<schema<oorder_c_id_idx>>::type> c_oorder(
    ...
    const oorder_c_id_idx::key k_oo_idx_1(warehouse_id, districtID, k_c.c_id, numeric_limits<int32_t>::max());
    {
      ...
      tables.tbl_oorder_c_id_idx(warehouse_id)->bytes_search_range_call(txn, k_oo_idx_0, &k_oo_idx_1, c_oorder);
    }
    ...
    tables.tbl_order_line(warehouse_id)->bytes_search_range_call(txn, k_ol_0, &k_ol_1, c_order_line);
    ALWAYS_ASSERT(c_order_line.n >= 5 && c_order_line.n <= 15);

    // measure_txn_counters(txn, "txn_order_status");
    if (likely(txn.commit())) return txn_result(true, 0);
  } catch (typename Database::abort_exception_type &e) { txn.abort(); }
  return txn_result(false, 0);
}

template<typename Database>
class order_line_scan_callback
    : public Database::template IndexType<schema<order_line>>::type::bytes_search_range_callback {
public:
  ...
  virtual bool invoke(const string &key, const string &value) {
    ...
    typed_txn_btree_<schema<order_line>>::do_record_read((const uint8_t *) value.data(), value.size(), mask, &v_ol);
...
tpcc_worker<Database, AllowReadOnlyScans>::txn_stock_level() {
  ...
  //   num_txn_contexts : 3
  const uint64_t read_only_mask = !AllowReadOnlyScans ? 0 : transaction_base::TXN_FLAG_READ_ONLY;
  typename Database::template TransactionType<AllowReadOnlyScans ? abstract_db::HINT_TPCC_STOCK_LEVEL_READ_ONLY
                                                                 : abstract_db::HINT_TPCC_STOCK_LEVEL>::type
      txn(txn_flags | read_only_mask, this->arena);
  scoped_str_arena s_arena(this->arena);
  ...
  try {
    ...
    const order_line::key k_ol_1(warehouse_id, districtID, cur_next_o_id, 0);
    {
      ...
      tables.tbl_order_line(warehouse_id)->bytes_search_range_call(txn, k_ol_0, &k_ol_1, c, nfields);
    }
    {
      ...
      for (auto &p: c.s_i_ids) {
        ...
        INVARIANT(p.first >= 1 && p.first <= NumItems());
        {
          ...
          ALWAYS_ASSERT(
              tables.tbl_stock(warehouse_id)->search(txn, k_s, v_s, GUARDED_FIELDS(stock::value::s_quantity_field)));
        }
        if (v_s.s_quantity < int(threshold)) s_i_ids_distinct[p.first] = 1;
      }
...
private:
  ...
  static vector<shared_ptr<typename Database::template IndexType<Schema>::type>> OpenTablesForTablespace(
      Database *db, const char *name, size_t expected_size) {
    ...
    if (g_enable_separate_tree_per_partition && !is_read_only) {
      if (NumWarehouses() <= nthreads) {
        ...
        for (size_t partid = 0; partid < nthreads; partid++) {
          ...
          const unsigned wend = (partid + 1 == nthreads) ? NumWarehouses() : (partid + 1) * nwhse_per_partition;
          auto idx = db->template open_index<Schema>(s_name + "_" + to_string(partid), expected_size, is_append_only);
...
public:
  tpcc_bench_runner(Database *db) : typed_bench_runner<Database>(db) {
...
#define OPEN_TABLESPACE_X(x)                                                                         \
  do {                                                                                               \
    ...
    for (size_t i = 0; i < v.size(); i++) this->open_tables[string(#x) + "_" + to_string(i)] = v[i]; \
  } while (0);
...
#undef OPEN_TABLESPACE_X
    ...
    if (g_new_order_fast_id_gen) {
      void *const px = memalign(
          CACHELINE_SIZE, sizeof(aligned_padded_elem<atomic<uint64_t>>) * NumWarehouses() * NumDistrictsPerWarehouse());
...
protected:
  virtual vector<unique_ptr<bench_loader>> make_loaders() {
...
private:
  template<bool RO>
  vector<unique_ptr<bench_worker>> make_workers_impl() {
    ...
    if (NumWarehouses() <= nthreads) {
      for (size_t i = 0; i < nthreads; i++)
        ret.emplace_back(new tpcc_worker<Database, RO>(blockstart + i, r.next(), this->typed_db(), tables,
                                                       &this->barrier_a, &this->barrier_b, (i % NumWarehouses()) + 1,
                                                       (i % NumWarehouses()) + 2));
    } else {
      const unsigned nwhse_per_partition = NumWarehouses() / nthreads;
      for (size_t i = 0; i < nthreads; i++) {
...
protected:
  virtual vector<unique_ptr<bench_worker>> make_workers() {
...
template<typename Database>
static unique_ptr<bench_runner> MakeBenchRunner(Database *db) {
  return unique_ptr<bench_runner>(g_disable_read_only_scans
...
void tpcc_do_test(const string &dbtype, const persistconfig &cfg, int argc, char **argv) {
  // parse options
  optind = 1;
  ...
  if (dbtype == "ndb-proto2") {
    if (!cfg.logfiles_.empty()) {
      ...
      txn_logger::Init(nthreads, cfg.logfiles_, cfg.assignments_, &assignments_used, !cfg.nofsync_, cfg.do_compress_,
                       cfg.fake_writes_);
      if (verbose) {
    ...
    if (!cfg.disable_gc_) transaction_proto2_static::InitGC();
#endif
#ifdef PROTO2_CAN_DISABLE_SNAPSHOTS
    if (cfg.disable_snapshots_) transaction_proto2_static::DisableSnapshots();
#endif
