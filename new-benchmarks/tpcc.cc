        ...
        try {
            for (uint i = 1; i <= NumItems(); i++) {
                ...
                if (RandomNumber(this->r, 1, 100) > 10) {
                    ...
                    const string i_data = RandomStr(this->r, startOriginal + 1) + "ORIGINAL" +
        ...
        }
        {
            ...
            for (auto &p: c.s_i_ids) {
                ...
                INVARIANT(p.first >= 1 && p.first <= NumItems());
                {
                    ...
                    ALWAYS_ASSERT(tables.tbl_stock(warehouse_id)->search(txn, k_s, v_s, GUARDED_FIELDS(
                            stock::value::s_quantity_field)));
                }
        ...
        if (g_enable_separate_tree_per_partition && !is_read_only) {
            if (NumWarehouses() <= nthreads) {
                for (size_t i = 0; i < NumWarehouses(); i++)
                    ret[i] = db->template open_index<Schema>(s_name + "_" + to_string(i), expected_size,
                                                             is_append_only);
            } else {
                const unsigned nwhse_per_partition = NumWarehouses() / nthreads;
                for (size_t partid = 0; partid < nthreads; partid++) {
                    ...
                    auto idx =
                            db->template open_index<Schema>(s_name + "_" + to_string(partid), expected_size,
                                                            is_append_only);
