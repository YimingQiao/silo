...
public:
  tpcc_worker(unsigned int worker_id, unsigned long seed, Database *db,
              const tpcc_tables<Database> &tables, spin_barrier *barrier_a,
              spin_barrier *barrier_b, uint warehouse_id_start,
              uint warehouse_id_end)
      : bench_worker(worker_id, true, seed, db, barrier_a, barrier_b),
        tpcc_worker_mixin(), tables(tables),
...
public:
  tpcc_warehouse_loader(unsigned long seed, Database *db,
                        const tpcc_tables<Database> &tables)
      : typed_bench_loader<Database>(seed, db), tpcc_worker_mixin(),
        tables(tables) {}
    ...
    try {
      vector<warehouse::value> warehouses;
      {
        ...
        typename Database::template TransactionType<
            abstract_db::HINT_DEFAULT>::type txn(txn_flags, this->arena);
        for (uint i = 1; i <= NumWarehouses(); i++) {
          const warehouse::key k(i);

          const string w_name =
              RandomStr(this->r, RandomNumber(this->r, 6, 10));
          const string w_street_1 =
              RandomStr(this->r, RandomNumber(this->r, 10, 20));
          const string w_street_2 =
              RandomStr(this->r, RandomNumber(this->r, 10, 20));
          const string w_city =
              RandomStr(this->r, RandomNumber(this->r, 10, 20));
...
class tpcc_item_loader : public typed_bench_loader<Database>,
                         public tpcc_worker_mixin {
public:
  tpcc_item_loader(unsigned long seed, Database *db,
                   const tpcc_tables<Database> &tables)
      : typed_bench_loader<Database>(seed, db), tpcc_worker_mixin(),
        tables(tables) {}
...
protected:
  virtual void load() {
    ...
    auto txn = this->typed_db()->template new_txn<abstract_db::HINT_DEFAULT>(
        txn_flags, this->arena);
    uint64_t total_sz = 0;
    try {
      for (uint i = 1; i <= NumItems(); i++) {
        ...
        if (RandomNumber(this->r, 1, 100) > 10) {
          ...
          const string i_data = RandomStr(this->r, startOriginal + 1) +
                                "ORIGINAL" +
                                RandomStr(this->r, len - startOriginal - 7);
          v.i_data.assign(i_data);
        }
        ...
        tables.tbl_item(1)->insert(
            *txn, k, v); // this table is shared, so any partition is OK

        if (bsize != -1 && !(i % bsize)) {
          ALWAYS_ASSERT(txn->commit());
          txn = this->typed_db()->template new_txn<abstract_db::HINT_DEFAULT>(
              txn_flags, this->arena);
          this->arena.reset();
        }
      }
...
class tpcc_stock_loader : public typed_bench_loader<Database>,
                          public tpcc_worker_mixin {
public:
  tpcc_stock_loader(unsigned long seed, Database *db,
                    const tpcc_tables<Database> &tables, ssize_t warehouse_id)
      : typed_bench_loader<Database>(seed, db), tpcc_worker_mixin(),
        tables(tables), warehouse_id(warehouse_id) {
...
protected:
  virtual void load() {
    ...
    for (uint w = w_start; w <= w_end; w++) {
      const size_t batchsize = (this->typed_db()->txn_max_batch_size() == -1)
                                   ? NumItems()
      ...
      for (uint b = 0; b < nbatches;) {
        scoped_str_arena s_arena(this->arena);
        auto txn =
            this->typed_db()->template new_txn<abstract_db::HINT_DEFAULT>(
                txn_flags, this->arena);
        try {
...
class tpcc_district_loader : public typed_bench_loader<Database>,
                             public tpcc_worker_mixin {
public:
  tpcc_district_loader(unsigned long seed, Database *db,
                       const tpcc_tables<Database> &tables)
      : typed_bench_loader<Database>(seed, db), tpcc_worker_mixin(),
        tables(tables) {}
...
class tpcc_customer_loader : public typed_bench_loader<Database>,
                             public tpcc_worker_mixin {
public:
  tpcc_customer_loader(unsigned long seed, Database *db,
                       const tpcc_tables<Database> &tables,
                       ssize_t warehouse_id)
      : typed_bench_loader<Database>(seed, db), tpcc_worker_mixin(),
        tables(tables), warehouse_id(warehouse_id) {
        ...
        for (uint batch = 0; batch < nbatches;) {
          ...
          try {
            for (uint cidx0 = cstart; cidx0 < cend; cidx0++) {
              ...
              v.c_first.assign(
                  RandomStr(this->r, RandomNumber(this->r, 8, 16)));
              ...
              v.c_data.assign(
                  RandomStr(this->r, RandomNumber(this->r, 300, 500)));
              ...
              v_hist.h_data.assign(
                  RandomStr(this->r, RandomNumber(this->r, 10, 24)));
...
class tpcc_order_loader : public typed_bench_loader<Database>,
                          public tpcc_worker_mixin {
public:
  tpcc_order_loader(unsigned long seed, Database *db,
                    const tpcc_tables<Database> &tables, ssize_t warehouse_id)
      : typed_bench_loader<Database>(seed, db), tpcc_worker_mixin(),
        tables(tables), warehouse_id(warehouse_id) {
    ALWAYS_ASSERT(warehouse_id == -1 ||
          ...
          try {
            ...
            for (uint l = 1; l <= uint(v_oo.o_ol_cnt); l++) {
              ...
              if (k_ol.ol_o_id < 2101) {
                ...
                v_ol.ol_amount =
                    (float)(RandomNumber(this->r, 1, 999999) / 100.0);
              }
...
tpcc_worker<Database, AllowReadOnlyScans>::txn_new_order() {
  const uint warehouse_id =
      PickWarehouseId(this->r, warehouse_id_start, warehouse_id_end);
  ...
  //   num_txn_contexts : 9
  typename Database::template TransactionType<
      abstract_db::HINT_TPCC_NEW_ORDER>::type txn(txn_flags, this->arena);
  scoped_str_arena s_arena(this->arena);
  ...
  try {
    ...
    customer::value v_c;
    ALWAYS_ASSERT(
        tables.tbl_customer(warehouse_id)
            ->search(txn, k_c, v_c,
                     GUARDED_FIELDS(customer::value::c_discount_field,
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      ALWAYS_ASSERT(tables.tbl_item(1)->search(
          txn, k_i, v_i,
          GUARDED_FIELDS(item::value::i_price_field, item::value::i_name_field,
...
template <typename Database>
class new_order_scan_callback
    : public Database::template IndexType<
          schema<new_order>>::type::search_range_callback {
public:
  new_order_scan_callback() : invoked(false) {}

  virtual bool invoke(const new_order::key &key,
                      const new_order::value &value) OVERRIDE {
    INVARIANT(!invoked);
    k_no = key;
    invoked = true;
#ifdef CHECK_INVARIANTS
...
tpcc_worker<Database, AllowReadOnlyScans>::txn_delivery() {
  ...
  const uint o_carrier_id =
      RandomNumber(this->r, 1, NumDistrictsPerWarehouse());
  ...
  //   num_txn_contexts : 4
  typename Database::template TransactionType<
      abstract_db::HINT_TPCC_DELIVERY>::type txn(txn_flags, this->arena);
  scoped_str_arena s_arena(this->arena);
  ...
  try {
    ...
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      ...
      new_order_scan_callback<Database> new_order_c;
      {
        ...
        tables.tbl_new_order(warehouse_id)
            ->search_range_call(txn, k_no_0, &k_no_1, new_order_c);
      }
      ...
      static_limit_callback<
          typename Database::template IndexType<schema<order_line>>::type, 15,
          false>
          c;
      ...
      tables.tbl_order_line(warehouse_id)
          ->search_range_call(
              ...
              GUARDED_FIELDS(order_line::value::ol_amount_field,
      ...
      tables.tbl_oorder(warehouse_id)
          ->put(txn, k_oo, v_oo,
...
tpcc_worker<Database, AllowReadOnlyScans>::txn_payment() {
  ...
  const float paymentAmount =
      (float)(RandomNumber(this->r, 100, 500000) / 100.0);
  const uint32_t ts = GetCurrentTimeMillis();
  ...
  //   num_txn_contexts : 5
  typename Database::template TransactionType<
      abstract_db::HINT_TPCC_PAYMENT>::type txn(txn_flags, this->arena);
  scoped_str_arena s_arena(this->arena);
  scoped_multilock<spinlock> mlock;
  if (g_enable_partition_locks) {
    mlock.enq(LockForPartition(warehouse_id));
  ...
  try {
    ...
    warehouse::value v_w;
    ALWAYS_ASSERT(
        tables.tbl_warehouse(warehouse_id)
            ->search(txn, k_w, v_w,
                     GUARDED_FIELDS(warehouse::value::w_ytd_field,
    ...
    tables.tbl_warehouse(warehouse_id)
        ->put(txn, k_w, v_w, GUARDED_FIELDS(warehouse::value::w_ytd_field));

    const district::key k_d(warehouse_id, districtID);
    district::value v_d;
    ALWAYS_ASSERT(tables.tbl_district(warehouse_id)
                      ->search(txn, k_d, v_d,
                               GUARDED_FIELDS(district::value::d_ytd_field,
    ...
    tables.tbl_district(warehouse_id)
        ->put(txn, k_d, v_d, GUARDED_FIELDS(district::value::d_ytd_field));
    ...
    if (RandomNumber(this->r, 1, 100) <= 60) {
      ...
      bytes_static_limit_callback<typename Database::template IndexType<
                                      schema<customer_name_idx>>::type,
                                  NMaxCustomerIdxScanElems, true>
          c(s_arena.get());
      tables.tbl_customer_name_idx(customerWarehouseID)
          ->bytes_search_range_call(txn, k_c_idx_0, &k_c_idx_1, c);
      ...
      const customer_name_idx::value *v_c_idx =
          Decode(c.value(index), v_c_idx_temp);

      k_c.c_w_id = customerWarehouseID;
      k_c.c_d_id = customerDistrictID;
      k_c.c_id = v_c_idx->c_id;
      ALWAYS_ASSERT(
          tables.tbl_customer(customerWarehouseID)->search(txn, k_c, v_c));
    } else {
      // cust by ID
      const uint customerID = GetCustomerId(r);
      k_c.c_w_id = customerWarehouseID;
...
class order_line_nop_callback
    : public Database::template IndexType<
          schema<order_line>>::type::bytes_search_range_callback {
...
tpcc_worker<Database, AllowReadOnlyScans>::txn_order_status() {
  ...
  typename Database::template TransactionType<
      AllowReadOnlyScans ? abstract_db::HINT_TPCC_ORDER_STATUS_READ_ONLY
  ...
  try {
    ...
    if (RandomNumber(this->r, 1, 100) <= 60) {
      ...
      bytes_static_limit_callback<typename Database::template IndexType<
                                  ...
                                  NMaxCustomerIdxScanElems, true>
          c(s_arena.get());
      tables.tbl_customer_name_idx(warehouse_id)
          ->bytes_search_range_call(txn, k_c_idx_0, &k_c_idx_1, c);
      ALWAYS_ASSERT(c.size() > 0);
      INVARIANT(c.size() < NMaxCustomerIdxScanElems); // we should detect this
      int index = c.size() / 2;
    ...
    latest_key_callback<
        typename Database::template IndexType<schema<oorder_c_id_idx>>::type>
        c_oorder(*this->arena.next(), (r.next() % 15) + 1);
                                          ...
                                          numeric_limits<int32_t>::max());
    {
      ...
      tables.tbl_oorder_c_id_idx(warehouse_id)
          ->bytes_search_range_call(txn, k_oo_idx_0, &k_oo_idx_1, c_oorder);
    }
    ALWAYS_ASSERT(c_oorder.size());

    oorder_c_id_idx::key k_oo_idx_temp;
    const oorder_c_id_idx::key *k_oo_idx =
        Decode(c_oorder.kstr(), k_oo_idx_temp);
    ...
    const order_line::key k_ol_1(warehouse_id, districtID, o_id,
                                 numeric_limits<int32_t>::max());
    tables.tbl_order_line(warehouse_id)
        ->bytes_search_range_call(txn, k_ol_0, &k_ol_1, c_order_line);
...
tpcc_worker<Database, AllowReadOnlyScans>::txn_stock_level() {
  ...
  const uint64_t read_only_mask =
      !AllowReadOnlyScans ? 0 : transaction_base::TXN_FLAG_READ_ONLY;
  typename Database::template TransactionType<
      AllowReadOnlyScans ? abstract_db::HINT_TPCC_STOCK_LEVEL_READ_ONLY
  ...
  try {
    ...
    const order_line::key k_ol_1(warehouse_id, districtID, cur_next_o_id, 0);
    {
      ...
      tables.tbl_order_line(warehouse_id)
          ->bytes_search_range_call(txn, k_ol_0, &k_ol_1, c, nfields);
    }
    {
      ...
      for (auto &p : c.s_i_ids) {
        ...
        INVARIANT(p.first >= 1 && p.first <= NumItems());
        {
          ...
          ALWAYS_ASSERT(
              tables.tbl_stock(warehouse_id)
                  ->search(txn, k_s, v_s,
                           GUARDED_FIELDS(stock::value::s_quantity_field)));
        }
...
private:
  ...
  OpenTablesForTablespace(Database *db, const char *name,
                          size_t expected_size) {
    const bool is_read_only = IsTableReadOnly(name);
    const bool is_append_only = IsTableAppendOnly(name);
    const string s_name(name);
    vector<typename Database::template IndexType<Schema>::ptr_type> ret(
        NumWarehouses());
    if (g_enable_separate_tree_per_partition && !is_read_only) {
      if (NumWarehouses() <= nthreads) {
        for (size_t i = 0; i < NumWarehouses(); i++)
          ret[i] = db->template open_index<Schema>(
              s_name + "_" + to_string(i), expected_size, is_append_only);
      } else {
        const unsigned nwhse_per_partition = NumWarehouses() / nthreads;
        for (size_t partid = 0; partid < nthreads; partid++) {
          ...
          auto idx = db->template open_index<Schema>(
              s_name + "_" + to_string(partid), expected_size, is_append_only);
          for (size_t i = wstart; i < wend; i++)
            ret[i] = idx;
        }
      }
    } else {
      auto idx = db->template open_index<Schema>(s_name, expected_size,
                                                 is_append_only);
      for (size_t i = 0; i < NumWarehouses(); i++)
        ret[i] = idx;
    }
...
#define OPEN_TABLESPACE_X(x)                                                   \
  do {                                                                         \
    tables.tbl_##x##_vec =                                                     \
        OpenTablesForTablespace<schema<x>>(db, #x, sizeof(x::value));          \
...
#undef OPEN_TABLESPACE_X
    ...
    if (g_enable_partition_locks) {
      ...
      void *const px = memalign(
          CACHELINE_SIZE, sizeof(aligned_padded_elem<spinlock>) * nthreads);
      ALWAYS_ASSERT(px);
...
protected:
  virtual vector<unique_ptr<bench_loader>> make_loaders() {
    ...
    ret.emplace_back(
        new tpcc_warehouse_loader<Database>(9324, this->typed_db(), tables));
    ret.emplace_back(
        new tpcc_item_loader<Database>(235443, this->typed_db(), tables));
    if (enable_parallel_loading) {
      fast_random r(89785943);
      for (uint i = 1; i <= NumWarehouses(); i++)
        ret.emplace_back(new tpcc_stock_loader<Database>(
            r.next(), this->typed_db(), tables, i));
    } else {
      ret.emplace_back(new tpcc_stock_loader<Database>(
          89785943, this->typed_db(), tables, -1));
    }
    ret.emplace_back(new tpcc_district_loader<Database>(
        129856349, this->typed_db(), tables));
    if (enable_parallel_loading) {
      fast_random r(923587856425);
      for (uint i = 1; i <= NumWarehouses(); i++)
        ret.emplace_back(new tpcc_customer_loader<Database>(
            r.next(), this->typed_db(), tables, i));
    } else {
      ret.emplace_back(new tpcc_customer_loader<Database>(
          923587856425, this->typed_db(), tables, -1));
    }
    if (enable_parallel_loading) {
      fast_random r(2343352);
      for (uint i = 1; i <= NumWarehouses(); i++)
        ret.emplace_back(new tpcc_order_loader<Database>(
            r.next(), this->typed_db(), tables, i));
    } else {
      ret.emplace_back(new tpcc_order_loader<Database>(
          2343352, this->typed_db(), tables, -1));
    }
    return ret;
  }

private:
  template <bool RO> vector<unique_ptr<bench_worker>> make_workers_impl() {
    ...
    if (NumWarehouses() <= nthreads) {
      for (size_t i = 0; i < nthreads; i++)
        ret.emplace_back(new tpcc_worker<Database, RO>(
            blockstart + i, r.next(), this->typed_db(), tables,
...
protected:
  virtual vector<unique_ptr<bench_worker>> make_workers() {
    return g_disable_read_only_scans ? make_workers_impl<false>()
                                     : make_workers_impl<true>();
  }
...
static unique_ptr<bench_runner> MakeBenchRunner(Database *db) {
  return unique_ptr<bench_runner>(
      g_disable_read_only_scans
          ? static_cast<bench_runner *>(
                new tpcc_bench_runner<Database, false>(db))
          : static_cast<bench_runner *>(
                new tpcc_bench_runner<Database, true>(db)));
}

void tpcc_do_test(const string &dbtype, const persistconfig &cfg, int argc,
                  char **argv) {
  // parse options
  optind = 1;
  ...
  if (dbtype == "ndb-proto2") {
    if (!cfg.logfiles_.empty()) {
      ...
      txn_logger::Init(nthreads, cfg.logfiles_, cfg.assignments_,
                       &assignments_used, !cfg.nofsync_, cfg.do_compress_,
                       cfg.fake_writes_);
      if (verbose) {
