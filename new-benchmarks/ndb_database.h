...
public:
  ndb_index(size_t value_size_hint, bool mostly_append, const std::string &name)
      : super_type(value_size_hint, mostly_append, name), name(name) {}
...
#define TXN_PROFILE_HINT_OP(x)                                                 \
  x(abstract_db::HINT_DEFAULT, default_traits) x(abstract_db::HINT_KV_GET_PUT, \
                                                 hint_kv_get_put_traits)       \
      x(abstract_db::HINT_KV_RMW,                                              \
        hint_kv_rmw_traits) x(abstract_db::HINT_KV_SCAN, hint_kv_scan_traits)  \
...
public:
  ...
  void thread_init(bool loader) OVERRIDE {
