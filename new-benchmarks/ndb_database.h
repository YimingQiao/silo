...
class ndb_index : public abstract_ordered_index, public typed_txn_btree<Transaction, Schema> {
  typedef typed_txn_btree<Transaction, Schema> super_type;

public:
  ndb_index(size_t value_size_hint, bool mostly_append, const std::string &name)
      : super_type(value_size_hint, mostly_append, name),
        name(name) {}

  virtual size_t size() const OVERRIDE { return this->size_estimate(); }
...
namespace private_ {
  ...
  template<enum abstract_db::TxnProfileHint>
  struct ndb_txn_type {};
...
#define TXN_PROFILE_HINT_OP(x)                                                                                         \
  x(abstract_db::HINT_DEFAULT, default_traits) x(abstract_db::HINT_KV_GET_PUT, hint_kv_get_put_traits)                 \
...
public:
  ...
  void do_txn_epoch_sync() const OVERRIDE { txn_epoch_sync<Transaction>::sync(); }

  void do_txn_finish() const OVERRIDE { txn_epoch_sync<Transaction>::finish(); }

  void thread_init(bool loader) OVERRIDE { txn_epoch_sync<Transaction>::thread_init(loader); }

  void thread_end() OVERRIDE { txn_epoch_sync<Transaction>::thread_end(); }

  std::tuple<uint64_t, uint64_t, double> get_ntxn_persisted() const OVERRIDE {
    return txn_epoch_sync<Transaction>::compute_ntxn_persisted();
  }

  void reset_ntxn_persisted() OVERRIDE { txn_epoch_sync<Transaction>::reset_ntxn_persisted(); }
