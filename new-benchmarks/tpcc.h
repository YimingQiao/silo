...
#define CUSTOMER_VALUE_FIELDS(x, y)                                            \
  x(float, c_discount) y(inline_str_fixed<2>, c_credit)                        \
...
template <typename Database, bool AllowReadOnlyScans> class tpcc_bench_runner;

template <typename Database> struct tpcc_tables {
  template <typename D, bool B> friend class tpcc_bench_runner;

#define DEFN_TBL_ACCESSOR_X(name)                                              \
private:                                                                       \
  std::vector<std::shared_ptr<                                                 \
      typename Database::template IndexType<schema<name>>::type>>              \
      tbl_##name##_vec;                                                        \
                                                                               \
public:                                                                        \
  inline ALWAYS_INLINE                                                         \
      typename Database::template IndexType<schema<name>>::type *tbl_##name(   \
          unsigned int wid) {                                                  \
    INVARIANT(wid >= 1);                                                       \
