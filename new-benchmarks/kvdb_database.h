...
public:
  ...
  template <typename Writer> inline bool do_write(Writer &writer) {
    INVARIANT(!UseConcurrencyControl || this->is_locked());
    const size_t new_sz =
        writer.compute_needed((const uint8_t *)&this->data[0], this->size());
  ...
  static basic_kvdb_record *alloc(size_t alloc_sz) {
    ...
    const size_t actual_alloc_sz =
        std::min(util::round_up<size_t, allocator::LgAllocAlignment>(
                     sizeof(basic_kvdb_record) + alloc_sz),
                 max_alloc_sz);
    char *const p =
        reinterpret_cast<char *>(rcu::s_instance.alloc(actual_alloc_sz));
    INVARIANT(p);
    return new (p)
        basic_kvdb_record(actual_alloc_sz - sizeof(basic_kvdb_record));
  }
...
private:
  ...
  typedef typename typed_txn_btree_<Schema>::single_value_reader
      single_value_reader;
  typedef typename typed_txn_btree_<Schema>::value_reader value_reader;

  typedef basic_kvdb_record<UseConcurrencyControl> kvdb_record;

  template <typename Btree, typename Callback, typename KeyReader,
            typename ValueReader>
  class kvdb_wrapper_search_range_callback
      : public Btree::search_range_callback {
  public:
    kvdb_wrapper_search_range_callback(Callback &upcall, KeyReader &kr,
                                       ValueReader &vr, str_arena &arena)
        : upcall(&upcall), kr(&kr), vr(&vr), arena(&arena) {}
...
public:
  kvdb_index(size_t value_size_hint, bool mostly_append,
  ...
  inline bool search(kvdb_txn &t, const key_type &k, value_type &v,
                     FieldsMask fm = FieldsMask());

  template <typename FieldsMask = AllFields>
  inline void
  search_range_call(kvdb_txn &t, const key_type &lower, const key_type *upper,
  ...
  template <typename FieldsMask = AllFields>
  inline void put(kvdb_txn &t, const key_type &k, const value_type &v,
                  FieldsMask fm = FieldsMask());
...
private:
  ...
  inline void do_search_range_call(kvdb_txn &t, const key_type &lower,
...
template <bool UseConcurrencyControl> class kvdb_database : public abstract_db {
public:
  template <typename Schema> struct IndexType {
    typedef kvdb_index<Schema, UseConcurrencyControl> type;
    typedef std::shared_ptr<type> ptr_type;
  };

  template <enum abstract_db::TxnProfileHint hint> struct TransactionType {
...
bool kvdb_index<Schema, UseConcurrencyControl>::search(kvdb_txn &t,
                                                       const key_type &k,
                                                       value_type &v,
                                                       FieldsMask fm) {
  key_writer kw(&k);
...
template <typename Schema, bool UseConcurrencyControl>
template <typename FieldsMask>
void kvdb_index<Schema, UseConcurrencyControl>::search_range_call(
    kvdb_txn &t, const key_type &lower, const key_type *upper,
    search_range_callback &callback, bool no_key_results, FieldsMask fm) {
...
template <typename Schema, bool UseConcurrencyControl>
void kvdb_index<Schema, UseConcurrencyControl>::bytes_search_range_call(
    kvdb_txn &t, const key_type &lower, const key_type *upper,
    bytes_search_range_callback &callback, size_t value_fields_prefix) {
...
template <typename Schema, bool UseConcurrencyControl>
template <typename FieldsMask>
void kvdb_index<Schema, UseConcurrencyControl>::put(kvdb_txn &t,
                                                    const key_type &key,
  ...
  kvdb_record *const rnew =
      kvdb_record::alloc(*vw.fully_materialize(false, t.string_allocator()));
  if (!btr.insert(varkey(*keypx), (typename my_btree::value_type)rnew, &v_old,
                  0)) {
...
template <typename Schema, bool UseConcurrencyControl>
void kvdb_index<Schema, UseConcurrencyControl>::insert(kvdb_txn &t,
                                                       const key_type &k,
                                                       const value_type &v) {
  key_writer kw(&k);
  const std::string *const keypx =
  ...
  rec->set_size(sz);
  if (likely(btr.insert_if_absent(varkey(*keypx),
                                  (typename my_btree::value_type)rec, nullptr)))
    return;
...
template <typename Schema, bool UseConcurrencyControl>
void kvdb_index<Schema, UseConcurrencyControl>::remove(kvdb_txn &t,
                                                       const key_type &k) {
  key_writer kw(&k);
  const std::string *const keypx =
      kw.fully_materialize(false, t.string_allocator());
...
template <typename Schema, bool UseConcurrencyControl>
template <typename Callback, typename KeyReader, typename ValueReader>
void kvdb_index<Schema, UseConcurrencyControl>::do_search_range_call(
    kvdb_txn &t, const key_type &lower, const key_type *upper,
  ...
  kvdb_wrapper_search_range_callback<my_btree, Callback, KeyReader, ValueReader>
      c(callback, kr, vr, t.string_allocator());

  varkey uppervk;
  if (upper_str)
    uppervk = varkey(*upper_str);
  btr.search_range_call(varkey(*lower_str), upper_str ? &uppervk : nullptr, c,
                        t.string_allocator()());
}
