...
struct record_version {
  ...
  static inline ALWAYS_INLINE size_t Size(uint32_t v) { return 0; }

  inline ALWAYS_INLINE size_t size() const { return sz; }

  inline ALWAYS_INLINE void set_size(size_t s) {
...
struct record_version<true> {
  ...
  static inline bool IsLocked(uint32_t v) { return v & HDR_LOCKED_MASK; }

  inline bool is_locked() const { return IsLocked(hdr); }
  ...
  inline bool check_version(uint32_t version) const {
    COMPILER_MEMORY_FENCE;
    return hdr == version;
  }
};
...
struct basic_kvdb_record : public record_version<UseConcurrencyControl> {
  ...
  basic_kvdb_record(uint16_t alloc_size) : record_version<UseConcurrencyControl>(), alloc_size(alloc_size) {
    this->set_size(0);
  }
...
private:
  ...
  inline bool do_guarded_read(Reader &reader, StringAllocator &sa) const {
...
public:
  template<typename Reader, typename StringAllocator>
  inline void do_read(Reader &reader, StringAllocator &sa) const {
    ...
    retry:
      ...
      if (unlikely(!do_guarded_read(reader, sa) || !this->check_version(v))) {
#ifdef ENABLE_EVENT_COUNTERS
        ++nretries;
#endif
        goto retry;
      ...
      if (!ret) INVARIANT(false);
    }
  }

  template<typename Writer>
  inline bool do_write(Writer &writer) {
    ...
    if (unlikely(new_sz > alloc_size)) return false;
    writer((uint8_t *) &this->data[0], this->size());
    this->set_size(new_sz);
    return true;
  }

  static basic_kvdb_record *alloc(size_t alloc_sz) {
    ...
    const size_t actual_alloc_sz = std::min(
        util::round_up<size_t, allocator::LgAllocAlignment>(sizeof(basic_kvdb_record) + alloc_sz), max_alloc_sz);
...
private:
  static inline void deleter(void *r) {
    basic_kvdb_record *const px = reinterpret_cast<basic_kvdb_record *>(r);
...
struct purge_tree_walker : public Btree::tree_walk_callback {
  ...
  virtual void on_node_begin(const typename Btree::node_opaque_t *n) {
  ...
  virtual void on_node_failure() { spec_values.clear(); }
...
public:
  ...
  inline bool commit() { return true; }
...
private:
  ...
  typedef typename typed_txn_btree_<Schema>::key_writer key_writer;
  typedef typename typed_txn_btree_<Schema>::key_reader key_reader;

  typedef typename typed_txn_btree_<Schema>::value_writer value_writer;
  typedef typename typed_txn_btree_<Schema>::single_value_reader single_value_reader;
  typedef typename typed_txn_btree_<Schema>::value_reader value_reader;
  ...
  public:
    kvdb_wrapper_search_range_callback(Callback &upcall, KeyReader &kr, ValueReader &vr, str_arena &arena)
        : upcall(&upcall),
          kr(&kr),
          vr(&vr),
          arena(&arena) {}
    ...
    virtual bool invoke(const typename Btree::string_type &k, typename Btree::value_type v) {
      const kvdb_record *const r = reinterpret_cast<const kvdb_record *>(v);
      r->prefetch();
      r->do_read(*vr, *arena);
...
public:
  kvdb_index(size_t value_size_hint, bool mostly_append, const std::string &name) : name(name) {}

  // virtual interface

  virtual size_t size() const OVERRIDE { return btr.size(); }

  virtual std::map<std::string, uint64_t> clear() OVERRIDE {
  ...
  inline bool search(kvdb_txn &t, const key_type &k, value_type &v, FieldsMask fm = FieldsMask());

  template<typename FieldsMask = AllFields>
  inline void search_range_call(kvdb_txn &t, const key_type &lower, const key_type *upper,
                                search_range_callback &callback,
                                bool no_key_results = false /* skip decoding of keys? */, FieldsMask fm = FieldsMask());

  // a lower-level variant which does not bother to decode the key/values
  inline void bytes_search_range_call(kvdb_txn &t, const key_type &lower, const key_type *upper,
                                      bytes_search_range_callback &callback,
                                      size_t value_fields_prefix = std::numeric_limits<size_t>::max());

  template<typename FieldsMask = AllFields>
  inline void put(kvdb_txn &t, const key_type &k, const value_type &v, FieldsMask fm = FieldsMask());

  inline void insert(kvdb_txn &t, const key_type &k, const value_type &v);

  inline void remove(kvdb_txn &t, const key_type &k);

private:
  template<typename Callback, typename KeyReader, typename ValueReader>
  inline void do_search_range_call(kvdb_txn &t, const key_type &lower, const key_type *upper, Callback &callback,
                                   KeyReader &kr, ValueReader &vr);
...
public:
  ...
  template<enum abstract_db::TxnProfileHint hint>
  inline typename TransactionType<hint>::ptr_type new_txn(uint64_t txn_flags, str_arena &arena) const {
  ...
  template<typename Schema>
  inline typename IndexType<Schema>::ptr_type open_index(const std::string &name, size_t value_size_hint,
                                                         bool mostly_append) {
    return std::make_shared<typename IndexType<Schema>::type>(value_size_hint, mostly_append, name);
  }
};

template<typename Schema, bool UseConcurrencyControl>
template<typename FieldsMask>
bool kvdb_index<Schema, UseConcurrencyControl>::search(kvdb_txn &t, const key_type &k, value_type &v, FieldsMask fm) {
  key_writer kw(&k);
  const std::string *const keypx = kw.fully_materialize(false, t.string_allocator());
...
template<typename Schema, bool UseConcurrencyControl>
template<typename FieldsMask>
void kvdb_index<Schema, UseConcurrencyControl>::search_range_call(kvdb_txn &t, const key_type &lower,
                                                                  const key_type *upper,
                                                                  search_range_callback &callback, bool no_key_results,
                                                                  FieldsMask fm) {
...
template<typename Schema, bool UseConcurrencyControl>
void kvdb_index<Schema, UseConcurrencyControl>::bytes_search_range_call(kvdb_txn &t, const key_type &lower,
                                                                        const key_type *upper,
                                                                        bytes_search_range_callback &callback,
...
template<typename Schema, bool UseConcurrencyControl>
template<typename FieldsMask>
void kvdb_index<Schema, UseConcurrencyControl>::put(kvdb_txn &t, const key_type &key, const value_type &value,
                                                    FieldsMask fm) {
  key_writer kw(&key);
  const std::string *const keypx = kw.fully_materialize(false, t.string_allocator());
  if (UseConcurrencyControl)
  ...
  if (btr.search(varkey(*keypx), v)) {
    ...
    if (r->do_write(vw)) return;
    // replace - slow-path
    kvdb_record *const rnew = kvdb_record::alloc(*vw.fully_materialize(false, t.string_allocator()));
...
template<typename Schema, bool UseConcurrencyControl>
void kvdb_index<Schema, UseConcurrencyControl>::insert(kvdb_txn &t, const key_type &k, const value_type &v) {
  key_writer kw(&k);
  const std::string *const keypx = kw.fully_materialize(false, t.string_allocator());
  if (UseConcurrencyControl)
  ...
  rec->set_size(sz);
  if (likely(btr.insert_if_absent(varkey(*keypx), (typename my_btree::value_type) rec, nullptr))) return;
  kvdb_record::release_no_rcu(rec);
  put(t, k, v);
}

template<typename Schema, bool UseConcurrencyControl>
void kvdb_index<Schema, UseConcurrencyControl>::remove(kvdb_txn &t, const key_type &k) {
  key_writer kw(&k);
  const std::string *const keypx = kw.fully_materialize(false, t.string_allocator());
...
template<typename Schema, bool UseConcurrencyControl>
template<typename Callback, typename KeyReader, typename ValueReader>
void kvdb_index<Schema, UseConcurrencyControl>::do_search_range_call(kvdb_txn &t, const key_type &lower,
                                                                     const key_type *upper, Callback &callback,
                                                                     KeyReader &kr, ValueReader &vr) {
  key_writer lower_key_writer(&lower);
  key_writer upper_key_writer(upper);
  const std::string *const lower_str = lower_key_writer.fully_materialize(false, t.string_allocator());
  const std::string *const upper_str = upper_key_writer.fully_materialize(false, t.string_allocator());

  kvdb_wrapper_search_range_callback<my_btree, Callback, KeyReader, ValueReader> c(callback, kr, vr,
                                                                                   t.string_allocator());

  varkey uppervk;
  if (upper_str) uppervk = varkey(*upper_str);
