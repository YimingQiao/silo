...
static vector<string> split_ws(const string &s) {
  vector<string> r;
  istringstream iss(s);
  copy(istream_iterator<string>(iss), istream_iterator<string>(), back_inserter<vector<string>>(r));
  return r;
}

static size_t parse_memory_spec(const string &s) {
  string x(s);
  size_t mult = 1;
  if (x.back() == 'G') {
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    switch (c) {
      ...
      case 'a':
        cfg.assignments_.emplace_back(ParseCSVString<unsigned, RangeAwareParser<unsigned>>(optarg));
        break;
  ...
  if (bench_type == "tpcc") test_fn = tpcc_do_test;
  else
    ALWAYS_ASSERT(false);
  ...
  if (cfg.fake_writes_ && cfg.nofsync_) {
    cerr << "[WARNING] --log-nofsync has no effect with --log-fake-writes "
            "enabled"
         << endl;
  }
  ...
  if (!stats_server_sockfile.empty()) {
    cerr << "[WARNING] --stats-server-sockfile with no event counters enabled "
            "is useless"
         << endl;
  }
#endif
  ...
  if (numa_memory > 0) {
    const size_t maxpercpu = iceil_special(numa_memory / nthreads, ::allocator::GetHugepageSize());
  ...
  if (!cfg.logfiles_.empty() && !can_persist.count(db_type)) {
    cerr << "[ERROR] benchmark " << db_type << " does not have persistence implemented" << endl;
    return 1;
  }
  ...
  if (cfg.disable_gc_ && !has_gc.count(db_type)) {
    cerr << "[ERROR] benchmark " << db_type << " does not have gc to disable" << endl;
    return 1;
  }
#else
  if (cfg.disable_gc_) {
  ...
  if (cfg.disable_snapshots_ && !has_snapshots.count(db_type)) {
    cerr << "[ERROR] benchmark " << db_type << " does not have snapshots to disable" << endl;
    return 1;
  }
#else
  if (cfg.disable_snapshots_) {
    cerr << "[ERROR] macro PROTO2_CAN_DISABLE_SNAPSHOTS was not set, cannot "
            "disable snapshots"
         << endl;
    return 1;
  }
  ...
  if (verbose) {
    ...
    if (run_mode == RUNMODE_TIME) cerr << "  runtime     : " << runtime << endl;
    else
  ...
  int argc_ = 1 + bench_toks.size();
  char *argv_[argc];
  argv[0] = (char *) bench_type.c_str();
  for (size_t i = 1; i <= bench_toks.size(); i++) argv[i] = (char *) bench_toks[i - 1].c_str();
  test_fn(db_type, cfg, argc_, argv_);
  return 0;
}
