...
int main(int argc, char **argv) {
  ...
  while (1) {
    static struct option long_options[] = {
        ...
        {"retry-aborted-transactions", no_argument, &retry_aborted_transaction,
         1},
        {"backoff-aborted-transactions", no_argument,
         &backoff_aborted_transaction, 1},
        ...
        {0, 0, 0, 0}};
    int option_index = 0;
    int c = getopt_long(argc, argv, "b:s:t:d:B:f:r:n:o:m:l:a:x:", long_options,
                        &option_index);
    if (c == -1)
      break;
  ...
  if (cfg.fake_writes_ && cfg.logfiles_.empty()) {
    cerr << "[ERROR] --log-fake-writes specified without logging enabled"
         << endl;
    return 1;
  }
  ...
  if (numa_memory > 0) {
    const size_t maxpercpu =
        iceil_special(numa_memory / nthreads, ::allocator::GetHugepageSize());
  ...
  if (cfg.disable_gc_ && !has_gc.count(db_type)) {
    cerr << "[ERROR] benchmark " << db_type << " does not have gc to disable"
         << endl;
    return 1;
  }
#else
  if (cfg.disable_gc_) {
    cerr << "[ERROR] macro PROTO2_CAN_DISABLE_GC was not set, cannot disable gc"
         << endl;
    return 1;
  }
#endif
  ...
  cerr
      << "WARNING: invariant checking is enabled - should disable for benchmark"
      << endl;
  ...
  if (verbose) {
    ...
    cerr << "  btree_internal_node_size: "
         << concurrent_btree::InternalNodeSize() << endl;
