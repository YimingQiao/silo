...
protected:
  ...
  virtual bool do_compression() const = 0;

  pbuffer *getbuffer(unsigned id) {
...
protected:
  void worker(unsigned id) OVERRIDE {
    ...
    uint64_t lasttid = 0, ncommits_currentepoch = 0, ncommits_synced = 0;
    ...
    if (curbuf) {
      ...
      const uint64_t waitfor = tidhelpers::EpochId(curbuf->header()->last_tid_);
      INVARIANT(per_thread_epochs_[id]->load(memory_order_acquire) == waitfor);
      ALWAYS_ASSERT(waitfor < FutureEpoch);
      curbuf->header()->last_tid_ = tidhelpers::MakeTid(id, 0, FutureEpoch);
...
private:
  void fsyncer(unsigned id, int fd, one_way_post<int> &channel) {
    for (;;) {
      int ret;
      channel.peek(ret);
      if (ret == -1) return;
  ...
  void writer(unsigned id, int fd, const vector<unsigned> &assignment) {
    ...
    while (keep_going_->load(memory_order_acquire)) {
      ...
      const ssize_t ret = nwritten ? writev(fd, &iovs[0], nwritten) : 0;
      if (ret == -1) {
        perror("writev");
        exit(1);
      }
...
protected:
  ...
  uint64_t compute_log_record_space() const OVERRIDE {
  ...
  void write_log_record(uint8_t *p, uint64_t tidcommit, const vector<uint64_t> &readset,
    ...
    for (auto &pr: writeset) {
      ...
      memcpy(p, pr.second.data(), pr.second.size());
      p += pr.second.size();
    }
  }
    ...
    while (changed) {
      ...
      for (size_t i = 0; i < NMAXCORES; i++) {
        ...
        for (auto px: pxs_) {
          ...
          // cerr << "processing buffer " << px << " with curoff_=" <<
          // px->curoff_ << endl
          //      << "  p=" << intptr_t(p) << endl;
          ...
          while (px->remaining_ && allsat) {
            ...
            if (allsat) {
              ...
              //      << ", g_persistence_vc=" <<
              //      tidhelpers::Str(g_persistence_vc[i])
              //      << endl;
