...
public:
  ...
  small_vector(const small_vector &that) : n(0), large_elems(0) { assignFrom(that); }
  ...
  inline reference front() {
    if (unlikely(large_elems)) return large_elems->front();
    INVARIANT(n > 0);
    INVARIANT(n <= SmallSize);
    return *ptr();
  }

  inline const_reference front() const { return const_cast<small_vector *>(this)->front(); }

  inline reference back() {
    if (unlikely(large_elems)) return large_elems->back();
    INVARIANT(n > 0);
    INVARIANT(n <= SmallSize);
    return ptr()[n - 1];
  }

  inline const_reference back() const { return const_cast<small_vector *>(this)->back(); }
  ...
  inline reference operator[](int i) {
    if (unlikely(large_elems)) return large_elems->operator[](i);
    return ptr()[i];
  }

  inline const_reference operator[](int i) const { return const_cast<small_vector *>(this)->operator[](i); }
  ...
  inline void reserve(size_t n) {
    if (unlikely(large_elems)) large_elems->reserve(n);
  }
  ...
  inline void sort(Compare c = Compare()) {
    if (unlikely(large_elems)) std::sort(large_elems->begin(), large_elems->end(), c);
    else
...
private:
  ...
  public:
    ...
    inline bool operator!=(const small_iterator_ &o) const { return !operator==(o); }
    ...
    inline intptr_t operator-(const small_iterator_ &o) const { return p - o.p; }
  ...
  public:
    ...
    inline bool operator==(const iterator_ &o) const {
      if (unlikely(large)) return large_it == o.large_it;
  ...
  protected:
    iterator_(SmallTypeIter small_it) : large(false), small_it(small_it), large_it() {}
    iterator_(LargeTypeIter large_it) : large(true), small_it(), large_it(large_it) {}
...
private:
  ...
  inline ALWAYS_INLINE T *ptr() { return reinterpret_cast<T *>(&small_elems_buf[0]); }

  inline ALWAYS_INLINE const T *ptr() const { return reinterpret_cast<const T *>(&small_elems_buf[0]); }
