...
// ------------------------- These operator definitions should be put h
...
namespace private_ {
template <size_t Idx, bool Enable, class... Types> struct helper {
...
template <size_t Idx, class... Types> struct helper<Idx, false, Types...> {
  static inline void apply(std::ostream &o, const std::tuple<Types...> &t) {}
};
} // namespace private_
...
template <typename T, bool Pedantic = true> class aligned_padded_elem {
public:
  template <class... Args>
...
template <typename T> struct host_endian_trfm {
  inline ALWAYS_INLINE T operator()(const T &t) const { return t; }
};

template <> struct host_endian_trfm<uint16_t> {
  inline ALWAYS_INLINE uint16_t operator()(uint16_t t) const {
    return be16toh(t);
  }
};

template <> struct host_endian_trfm<int16_t> {
  inline ALWAYS_INLINE int16_t operator()(int16_t t) const {
    return be16toh(t);
  }
};

template <> struct host_endian_trfm<int32_t> {
  inline ALWAYS_INLINE int32_t operator()(int32_t t) const {
    return be32toh(t);
  }
};

template <> struct host_endian_trfm<uint32_t> {
  inline ALWAYS_INLINE uint32_t operator()(uint32_t t) const {
    return be32toh(t);
  }
};

template <> struct host_endian_trfm<int64_t> {
  inline ALWAYS_INLINE int64_t operator()(int64_t t) const {
    return be64toh(t);
  }
};

template <> struct host_endian_trfm<uint64_t> {
  inline ALWAYS_INLINE uint64_t operator()(uint64_t t) const {
    return be64toh(t);
  }
};

template <typename T> struct big_endian_trfm {
  inline ALWAYS_INLINE T operator()(const T &t) const { return t; }
};

template <> struct big_endian_trfm<uint16_t> {
  inline ALWAYS_INLINE uint16_t operator()(uint16_t t) const {
    return htobe16(t);
  }
};

template <> struct big_endian_trfm<int16_t> {
  inline ALWAYS_INLINE int16_t operator()(int16_t t) const {
    return htobe16(t);
  }
};

template <> struct big_endian_trfm<int32_t> {
  inline ALWAYS_INLINE int32_t operator()(int32_t t) const {
    return htobe32(t);
  }
};

template <> struct big_endian_trfm<uint32_t> {
  inline ALWAYS_INLINE uint32_t operator()(uint32_t t) const {
    return htobe32(t);
  }
};

template <> struct big_endian_trfm<int64_t> {
  inline ALWAYS_INLINE int64_t operator()(int64_t t) const {
    return htobe64(t);
  }
};

template <> struct big_endian_trfm<uint64_t> {
  inline ALWAYS_INLINE uint64_t operator()(uint64_t t) const {
    return htobe64(t);
  }
};
...
template <typename T> inline std::string hexify(const T &t) {
  std::ostringstream buf;
  buf << std::hex << t;
  return buf.str();
}

template <> inline std::string hexify(const std::string &input) {
  return hexify_buf(input.data(), input.size());
}

template <typename T, unsigned int lgbase> struct mask_ {
...
template <typename T, typename U>
static inline ALWAYS_INLINE T iceil_special(T x, U y) {
  U mod = x % y;
  return x + (mod ? y - mod : 0);
}

template <typename T> static inline T slow_round_up(T x, T q) {
  const T r = x % q;
  if (!r)
    return x;
  return x + (q - r);
...
template <typename T> static inline T slow_round_down(T x, T q) {
  const T r = x % q;
  if (!r)
    return x;
  return x - r;
...
inline size_t first_pos_diff(const char *p0, size_t sz0, const char *p1,
  ...
  size_t n = 0;
  while (p0 != p0end && p1 != p1end && p0[n] == p1[n])
    n++;
  return n;
}
...
template <typename PairType, typename FirstComp> struct std_pair_first_cmp {
...
template <typename T, size_t SmallSize = SMALL_SIZE_VEC> struct vec {
...
template <uint64_t Mask> struct Fields {
...
template <typename T> struct cxx_typename {
...
template <typename T> static std::vector<T> MakeRange(T start, T end) {
  std::vector<T> ret;
...
template <typename T> struct RangeAwareParser {
