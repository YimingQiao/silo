...
namespace private_ {
    ...
    struct helper {
        static inline void apply(std::ostream &o, const std::tuple<Types...> &t) {
            ...
            helper<Idx + 1, (Idx + 1) < std::tuple_size < std::tuple < Types...>>
            ::value, Types...>::apply(o, t);
        }
    };
...
static inline std::ostream &operator<<(std::ostream &o, const std::tuple<Types...> &t) {
    o << "[";
    private_::helper<0, 0 < std::tuple_size < std::tuple < Types...>>
    ::value, Types...>::apply(o, t);
    o << "]";
    return o;
}
...
namespace util {
    ...
    private:
        inline void __cl_asserter() const { static_assert((sizeof(*this) % CACHELINE_SIZE) == 0, "xx"); }
    } CACHE_ALIGNED;
...
public:
    ...
    /** [0.0, 1.0) */
    inline double next_uniform() { return (((unsigned long) next(26) << 27) + next(27)) / (double) (1L << 53); }
...
private:
    inline void set_seed0(unsigned long seed) { this->seed = (seed ^ 0x5DEECE66DL) & ((1L << 48) - 1); }
...
*/
inline size_t first_pos_diff(const char *p0, size_t sz0, const char *p1, size_t sz1) {
...
public:
    scoped_timer(const std::string &region, bool enabled = true, double *time = nullptr)
            : region(region), enabled(enabled), time(time) {}

    ~scoped_timer() {
        if (enabled) {
            const double x = t.lap() / 1000.0;// ms
            if (time != nullptr) *time = x;
...
static constexpr uint64_t
compute_fields_mask(First
f,
Rest... rest
) {
return (1UL << f) |
compute_fields_mask(rest
...);
}
...
#define GUARDED_FIELDS(args...) ::util::Fields<::std::numeric_limits<uint64_t>::max()>()
#else
...
struct cxx_typename {
    static std::string value() {
        ...
        if (unlikely(st)) return std::string(typeid(T).name()) + "<demangle failed>";
        std::string ret(name);
        free(name);
        return ret;
    }
...
struct timespec_utils {
    // thanks austin
    static void subtract(const struct timespec *x, const struct timespec *y, struct timespec *out) {
...
static std::vector <T> ParseCSVString(const std::string &s, Parser p = Parser()) {
    std::vector <T> ret;
    std::vector <std::string> toks(split(s, ','));
    for (auto &s: toks) {
        auto values = p(s);
