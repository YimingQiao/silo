...
public:
  ...
  inline bool operator!=(const varkey &that) const { return !operator==(that); }
  ...
  inline varkey shift() const {
    INVARIANT(l >= 8);
    return varkey(p + 8, l - 8);
  }

  inline varkey shift_many(size_t n) const {
    INVARIANT(l >= 8 * n);
...
inline std::ostream &operator<<(std::ostream &o, const varkey &k) {
  o << util::hexify(k.str());
  return o;
}
