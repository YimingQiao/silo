...
public:
  ...
  ~scoped_rate_timer() {
    ...
    cerr << "timed region `" << region << "' took " << x << " ms (" << rate << " events/sec)" << endl;
  }
};
...
static void test1() {
  ...
  // induce a split
  btr.insert(u64_varkey(testing_concurrent_btree::NKeysPerNode),
             (typename testing_concurrent_btree::value_type)(testing_concurrent_btree::NKeysPerNode));
  btr.invariant_checker();
  ...
  const size_t n = (testing_concurrent_btree::NKeysPerNode +
                    testing_concurrent_btree::NKeysPerNode * (testing_concurrent_btree::NMinKeysPerNode));
...
static void test5() {
  ...
  unsigned int seeds[] = {54321, 2013883780, 3028985725, 3058602342, 256561598, 2895653051};
...
namespace test6_ns {
  struct scan_callback {
    typedef vector<pair<std::string,// we want to make copies of keys
                        typename testing_concurrent_btree::value_type>>
        kv_vec;
    scan_callback(kv_vec *data, bool reverse = false) : data(data), reverse_(reverse) {}
    inline bool operator()(const typename testing_concurrent_btree::string_type &k,
                           typename testing_concurrent_btree::value_type v) const {
      if (!data->empty()) {
        const bool geq = typename testing_concurrent_btree::string_type(data->back().first) >= k;
        const bool leq = typename testing_concurrent_btree::string_type(data->back().first) <= k;
...
static void test6() {
  ...
  for (size_t i = 0; i < nkeys; i++) btr.insert(u64_varkey(i), (typename testing_concurrent_btree::value_type) i);
  btr.invariant_checker();
  ALWAYS_ASSERT(btr.size() == nkeys);
...
static void test_varlen_single_layer() {
  testing_concurrent_btree btr;
...
static void test_varlen_multi_layer() {
  testing_concurrent_btree btr;
...
public:
  struct expect {
    ...
    expect(size_t expected_size) : tag(0), expected_size(expected_size) {}
    expect(const set<string> &expected_keys) : tag(1), expected_keys(expected_keys) {}
    uint8_t tag;
  ...
  test_range_scan_helper(testing_concurrent_btree &btr, const testing_concurrent_btree::key_type &begin,
                         const testing_concurrent_btree::key_type *end, bool reverse, const expect &expectation,
  ...
  virtual bool invoke(const typename testing_concurrent_btree::string_type &k,
                      typename testing_concurrent_btree::value_type v) {
    VERBOSE(cerr << "test_range_scan_helper::invoke(): received key(size=" << k.size() << "): " << hexify(k) << endl);
    if (!keys.empty()) {
      if (!reverse_) ALWAYS_ASSERT(typename testing_concurrent_btree::string_type(keys.back()) < k);
      else
  ...
  void test() {
    keys.clear();
    if (!reverse_) btr->search_range_call(begin, end, *this);
    else
    ...
    if (expectation.tag == 0) {
      ...
      switch (ex_type) {
        case EXPECT_EXACT: {
          ...
          vector<string> cmp;
          if (!reverse_) cmp.assign(expectation.expected_keys.begin(), expectation.expected_keys.end());
          else
        ...
        case EXPECT_ATLEAST: {
          ...
          for (auto it = expectation.expected_keys.begin(); it != expectation.expected_keys.end(); ++it)
...
static void test_two_layer_range_scan() {
  const char *keys[] = {
      "a", "aaaaaaaa", "aaaaaaaaa", "aaaaaaaaaa", "aaaaaaaaaaa", "b", "c", "d", "e", "f", "g", "h",
...
static void test_multi_layer_scan() {
  const uint8_t lokey_cstr[] = {0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x45, 0x49, 0x4E, 0x47, 0x41, 0x54,
                                0x49, 0x4F, 0x4E, 0x45, 0x49, 0x4E, 0x47, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t hikey_cstr[] = {0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x45, 0x49, 0x4E, 0x47, 0x41, 0x54,
                                0x49, 0x4F, 0x4E, 0x45, 0x49, 0x4E, 0x47, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF,
...
static void test_null_keys_2() {
  ...
  retry:
    ...
    if (prefixes.count(k) == 1) goto retry;
    prefixes.insert(k);
  }
...
static inline string maxkey(unsigned size) { return string(size, 255); }

static void test_random_keys() {
  testing_concurrent_btree btr;
  fast_random r(43698);
  ...
  retry:
    ...
    if (keyset.count(k) == 1) goto retry;
    keyset.insert(k);
    swap(keys[i], k);
...
static void test_insert_remove_mix() {
  testing_concurrent_btree btr;
  ...
  retry:
    ...
    if (start_keys.count(k) == 1) goto retry;
  ...
  retry1:
    string k = r.next_readable_string(r.next() % 200);
    if (start_keys.count(k) == 1 || insert_keys.count(k) == 1) goto retry1;
  ...
  for (size_t i = 0; i < nkeys_start; i++) {
    ...
    ALWAYS_ASSERT(
        btr.insert(varkey(insert_keys_v[i]), (typename testing_concurrent_btree::value_type) insert_keys_v[i].data()));
  }
  btr.invariant_checker();
...
namespace mp_test2_ns {
  ...
  public:
    ...
    virtual void run() {
      for (size_t i = 0; i < nkeys / 2; i++) btr->remove(u64_varkey(i));
    }
  };
  ...
  public:
    ...
    virtual void run() {
      for (size_t i = nkeys / 2; i < nkeys; i++) btr->remove(u64_varkey(i));
    }
  };
}// namespace mp_test2_ns

static void mp_test2() {
...
namespace mp_test3_ns {
  ...
  public:
    ...
    virtual void run() {
      ...
      for (size_t i = 0; i < nkeys; i += 2) btr->remove(u64_varkey(i));
    }
  };
  ...
  public:
    ...
    virtual void run() {
      ...
      for (unsigned mode = 0; running.load(); mode++) {
        for (size_t i = thread * keys_per_thread; running.load() && i < (thread + 1) * keys_per_thread; i++) {
          if (mode % 2) {
            // remove
            btr->remove(u64_varkey(i));
          } else {
...
static void mp_test_pinning() {
  using namespace mp_test_pinning_ns;
  testing_concurrent_btree btr;
  vector<unique_ptr<worker>> workers;
  for (size_t i = 0; i < nthreads; i++) workers.emplace_back(new worker(i, btr));
  for (auto &p: workers) p->start();
  sleep(5);
  running.store(false);
...
namespace mp_test_inserts_removes_ns {
  ...
  class worker : public btree_worker {
  public:
    worker(bool inserts, unsigned int thread, testing_concurrent_btree &btr)
        : btree_worker(btr),
          inserts(inserts),
          thread(thread) {}
    virtual void run() {
      for (size_t i = thread * keys_per_thread; i < (thread + 1) * keys_per_thread; i++) {
        if (inserts)
          // insert
          btr->insert(u64_varkey(i), (typename testing_concurrent_btree::value_type) i);
        else
...
static void mp_test_inserts_removes() {
  using namespace mp_test_inserts_removes_ns;
  for (size_t iter = 0; iter < 3; iter++) {
    testing_concurrent_btree btr;
    vector<unique_ptr<worker>> workers;

    for (size_t i = 0; i < nthreads; i++) workers.emplace_back(new worker(true, i, btr));
    for (auto &p: workers) p->start();
    for (auto &p: workers) p->join();
    btr.invariant_checker();
    workers.clear();

    for (size_t i = 0; i < nthreads; i++) workers.emplace_back(new worker(false, i, btr));
    for (auto &p: workers) p->start();
    for (auto &p: workers) p->join();
    btr.invariant_checker();
    workers.clear();
  ...
  public:
    ...
    virtual void run() {
      ...
      for (size_t i = 0; i < niters; i++) {
        ...
        if (choice < 0.6) {
          typename testing_concurrent_btree::value_type v = 0;
          if (btr->search(u64_varkey(k), v)) ALWAYS_ASSERT(v == (typename testing_concurrent_btree::value_type) k);
        } else if (choice < 0.9) {
          btr->insert(u64_varkey(k), (typename testing_concurrent_btree::value_type) k);
...
static void mp_test5() {
  ...
  for (key_set::iterator it = inserts.begin(); it != inserts.end(); ++it) {
    if (removes.count(*it) == 1) continue;
    typename testing_concurrent_btree::value_type v = 0;
  ...
  public:
    remove_worker(const vector<typename testing_concurrent_btree::key_slice> &keys, testing_concurrent_btree &btr)
        : btree_worker(btr),
          keys(keys) {}
    virtual void run() {
      for (size_t i = 0; i < keys.size(); i++) btr->remove(u64_varkey(keys[i]));
    }
...
static void mp_test6() {
  ...
  for (size_t i = nthreads / 2; i < nthreads; i++) {
    ...
    for (size_t j = 0; j < nremovekeys_perthread;) {
      unsigned long k = r.next();
      if (insert_keys.count(k) == 1) continue;
      btr.insert(u64_varkey(k), (typename testing_concurrent_btree::value_type) k);
  ...
  for (size_t i = 0; i < nthreads / 2; i++) workers.push_back(new insert_worker(inps[i], btr));
  for (size_t i = nthreads / 2; i < nthreads; i++) workers.push_back(new remove_worker(inps[i], btr));
  for (size_t i = 0; i < nthreads; i++) workers[i]->start();
  for (size_t i = 0; i < nthreads; i++) workers[i]->join();

  btr.invariant_checker();

  ALWAYS_ASSERT(btr.size() == insert_keys.size());
  for (set<unsigned long>::iterator it = insert_keys.begin(); it != insert_keys.end(); ++it) {
    typename testing_concurrent_btree::value_type v = 0;
    ALWAYS_ASSERT(btr.search(u64_varkey(*it), v));
    ALWAYS_ASSERT(v == (typename testing_concurrent_btree::value_type) * it);
  }
  for (set<unsigned long>::iterator it = remove_keys.begin(); it != remove_keys.end(); ++it) {
    typename testing_concurrent_btree::value_type v = 0;
    ALWAYS_ASSERT(!btr.search(u64_varkey(*it), v));
  }

  for (size_t i = 0; i < nthreads; i++) delete workers[i];
}
...
namespace mp_test7_ns {
  ...
  struct scan_callback {
    typedef vector<pair<std::string, typename testing_concurrent_btree::value_type>> kv_vec;
  ...
  public:
    scan_worker(const key_vec &keys, testing_concurrent_btree &btr) : btree_worker(btr), keys(keys) {}
    virtual void run() {
      while (running) {
        scan_callback::kv_vec data;
        scan_callback cb(&data);
  ...
  public:
    mod_worker(const key_vec &keys, testing_concurrent_btree &btr) : btree_worker(btr), keys(keys) {}
    virtual void run() {
      bool insert = true;
      for (size_t i = 0; running; i = (i + 1) % keys.size(), insert = !insert) {
        if (insert) btr->insert(u64_varkey(keys[i]), (typename testing_concurrent_btree::value_type) keys[i]);
        else
...
static void mp_test7() {
  using namespace mp_test7_ns;
  fast_random r(904380439);
  key_vec lookup_keys;
  key_vec mod_keys;
  for (size_t i = 0; i < nkeys; i++) {
    if (r.next() % 2) mod_keys.push_back(i);
    else
...
namespace mp_test8_ns {
  ...
  class insert_worker : public btree_worker {
  public:
    insert_worker(const vector<string> &keys, testing_concurrent_btree &btr) : btree_worker(btr), keys(keys) {}
    virtual void run() {
      for (size_t i = 0; i < keys.size(); i++)
  ...
  class remove_worker : public btree_worker {
  public:
    remove_worker(const vector<string> &keys, testing_concurrent_btree &btr) : btree_worker(btr), keys(keys) {}
    virtual void run() {
      for (size_t i = 0; i < keys.size(); i++) ALWAYS_ASSERT(btr->remove(varkey(keys[i])));
    }
...
static void mp_test8() {
  ...
  for (size_t i = 0; i < nthreads / 2; i++) {
    ...
    retry:
      ...
      if (insert_keys.count(k) == 1) goto retry;
      insert_keys.insert(k);
      inp.push_back(k);
    }
    inps.push_back(inp);
  ...
  for (size_t i = nthreads / 2; i < nthreads; i++) {
    ...
    for (size_t j = 0; j < nremovekeys_perthread;) {
      string k = r.next_string(r.next() % 200);
      if (insert_keys.count(k) == 1 || remove_keys.count(k) == 1) continue;
      ALWAYS_ASSERT(btr.insert(varkey(k), (typename testing_concurrent_btree::value_type) k.data()));
      remove_keys.insert(k);
      inp.push_back(k);
      j++;
  ...
  ALWAYS_ASSERT(btr.size() == insert_keys.size());
  for (set<string>::iterator it = insert_keys.begin(); it != insert_keys.end(); ++it) {
    typename testing_concurrent_btree::value_type v = 0;
    ALWAYS_ASSERT(btr.search(varkey(*it), v));
  }
  for (set<string>::iterator it = remove_keys.begin(); it != remove_keys.end(); ++it) {
    typename testing_concurrent_btree::value_type v = 0;
    ALWAYS_ASSERT(!btr.search(varkey(*it), v));
  }
...
namespace mp_test_long_keys_ns {
  ...
  public:
    ...
    virtual void run() {
      ...
      while (running) {
        if (!reverse_) {
          test_range_scan_helper tester(*btr, varkey(""), NULL, false, ex, test_range_scan_helper::EXPECT_ATLEAST);
          tester.test();
        } else {
          test_range_scan_helper tester(*btr, varkey(mkey), NULL, true, ex, test_range_scan_helper::EXPECT_ATLEAST);
          tester.test();
        }
      }
    }
...
static void mp_test_long_keys() {
  ...
  retry0:
    ...
    if (existing_keys.count(k) == 1) goto retry0;
  ...
  for (size_t i = 0; i < nthreads / 2; i++) {
    ...
    retry:
      string k = r.next_string((r.next() % 200) + 9);
      if (insert_keys.count(k) == 1 || existing_keys.count(k) == 1) goto retry;
      insert_keys.insert(k);
      inp.push_back(k);
    }
    inps.push_back(inp);
  ...
  for (size_t i = nthreads / 2; i < nthreads; i++) {
    ...
    for (size_t j = 0; j < nremovekeys_perthread;) {
      string k = r.next_string((r.next() % 200) + 9);
      if (insert_keys.count(k) == 1 || existing_keys.count(k) == 1 || remove_keys.count(k) == 1) continue;
      ALWAYS_ASSERT(btr.insert(varkey(k), (typename testing_concurrent_btree::value_type) k.data()));
      remove_keys.insert(k);
      inp.push_back(k);
      j++;
  ...
  for (size_t i = 0; i < nthreads / 2; i++) workers.push_back(new insert_worker(inps[i], btr));
  for (size_t i = nthreads / 2; i < nthreads; i++) workers.push_back(new remove_worker(inps[i], btr));
  for (size_t i = 0; i < 4; i++) running_workers.push_back(new scan_worker(existing_keys, btr, (i % 2)));
  for (size_t i = 0; i < nthreads; i++) workers[i]->start();
  for (size_t i = 0; i < running_workers.size(); i++) running_workers[i]->start();
  for (size_t i = 0; i < nthreads; i++) workers[i]->join();
  running = false;
  for (size_t i = 0; i < running_workers.size(); i++) running_workers[i]->join();
  ...
  for (size_t i = 0; i < nthreads; i++) delete workers[i];
  for (size_t i = 0; i < running_workers.size(); i++) delete running_workers[i];
}
...
static void perf_test() {
...

  {
    ...
    map<uint64_t, uint64_t> m;
    {
      ...
      for (size_t i = 0; i < nrecs; i++) m[i] = i;
    }
    {
      ...
      for (size_t i = 0; i < nlookups; i++) {
        ...
        uint64_t key = i;
        map<uint64_t, uint64_t>::iterator it = m.find(key);
...
namespace read_only_perf_test_ns {
  ...
  unsigned long seeds[] = {
      9576455804445224191ULL,  3303315688255411629ULL,  3116364238170296072ULL,  641702699332002535ULL,
      17755947590284612420ULL, 13349066465957081273ULL, 16389054441777092823ULL, 2687412585397891607ULL,
      16665670053534306255ULL, 5166823197462453937ULL,  1252059952779729626ULL,  17962022827457676982ULL,
      940911318964853784ULL,   479878990529143738ULL,   250864516707124695ULL,   8507722621803716653ULL,
  };
...
static void read_only_perf_test() {
  ...
  for (size_t i = 0; i < ARRAY_NELEMS(seeds); i++) workers.push_back(new worker(seeds[i], btr));

  running = true;
  util::timer t;
  COMPILER_MEMORY_FENCE;
  for (size_t i = 0; i < ARRAY_NELEMS(seeds); i++) workers[i]->start();
  sleep(30);
  COMPILER_MEMORY_FENCE;
  running = false;
  COMPILER_MEMORY_FENCE;
...
namespace write_only_perf_test_ns {
  ...
  unsigned long seeds[] = {
      17188055221422272641ULL, 915721317773011804ULL,  11607688859420148202ULL, 16566896965529356730ULL,
      3687473034241167633ULL,  1168118474092824592ULL, 912212972587845337ULL,   890657129662032640ULL,
      7557640044845923769ULL,  9490577770668659131ULL, 14081403972130650060ULL, 14956552848279294368ULL,
      8669268465391111275ULL,  1904251150166743550ULL, 4418832947790992405ULL,  9558684485283258563ULL,
  };
...
static void write_only_perf_test() {
