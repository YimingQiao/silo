...
#define MYREC_KEY_FIELDS(x, y) x(int32_t, k0) y(int32_t, k1)
#define MYREC_VALUE_FIELDS(x, y) x(int32_t, v0) y(int16_t, v1)
DO_STRUCT(myrec, MYREC_KEY_FIELDS, MYREC_VALUE_FIELDS)

#define CURSORREC_KEY_FIELDS(x, y) x(int32_t, k0) y(int32_t, k1)
#define CURSORREC_VALUE_FIELDS(x, y)                                                                              \
  x(int32_t, v0) y(int16_t, v1) y(int32_t, v2) y(inline_str_8<25>, v3) y(inline_str_fixed<25>, v4) y(int32_t, v5) \
      y(int8_t, v6)
...
namespace pxqueuetest {
  ...
  static inline void *N2P(uint64_t n) { return (void *) n; }

  static inline rcu::deleter_t N2D(uint64_t n) { return (rcu::deleter_t) n; }

  struct sorter {
    bool operator()(const rcu::delete_entry &a, const rcu::delete_entry &b) const {
  ...
  void Test() {
    ...
    {
      px_queue q0;
      for (size_t i = 0; i < 14; i++) q0.enqueue(rcu::delete_entry(N2P(i), N2D(i)), 1);
...
void CircbufTest() {
  ...
  for (size_t i = 0; i < ARRAY_NELEMS(values); i++) { b.enq(&values[i]); }
  vector<int *> pxs;
  b.peekall(pxs);
  ALWAYS_ASSERT(pxs.size() == ARRAY_NELEMS(values));
  ALWAYS_ASSERT(set<int *>(pxs.begin(), pxs.end()).size() == pxs.size());
  for (size_t i = 0; i < ARRAY_NELEMS(values); i++) ALWAYS_ASSERT(pxs[i] == &values[i]);
...
namespace small_vector_ns {
  ...
  static void init_vec0(VecType &v) {
  ...
  template<typename VecType>
  static void init_vec1(VecType &v) {
    ALWAYS_ASSERT(v.empty());
    ALWAYS_ASSERT(v.size() == 0);
  ...
  static void assert_vecs_equal(VecA &v, const VecB &stl_v) {
  ...
  struct PComp {
    inline bool operator()(const pair<uint32_t, uint32_t> &a, const pair<uint32_t, uint32_t> &b) const {
  ...
  void Test() {
...

    {
      ...
      assert_vecs_equal(v, stl_v);
      for (size_t i = 0; i < 48; i++) ALWAYS_ASSERT(v[i].first + 1 == v[i].second);
    }
...
namespace small_map_ns {
  ...
  static void assert_map_contains(MapType &m, const string &k, const string &v) {
  ...
  template<typename MapType>
  static void assert_map_equals(MapType &m, const stl_map_type &stl_m) {
    ...
    // const version prefix
    {
      const MapType &const_m = m;
      stl_map_type test;
      for (typename MapType::const_iterator it = const_m.begin(); it != const_m.end(); ++it) {
        ALWAYS_ASSERT(test.find(it->first) == test.end());
        test[it->first] = it->second;
      }
      ALWAYS_ASSERT(test == stl_m);
  ...
  void Test() {
    {
      ...
      for (stl_map_type::iterator it = stl_m.begin(); it != stl_m.end(); ++it) {
...
namespace recordtest {
  ...
  ostream &operator<<(ostream &o, const myrec::key &k) {
    o << "[k0=" << k.k0 << ", k1=" << k.k1 << "]" << endl;
    return o;
  }

  ostream &operator<<(ostream &o, const myrec::value &v) {
  ...
  void TestCursor() {
    ...
    for (size_t i = rc.field(); i < cursorrec::value_descriptor::nfields(); i = rc.field())
    ...
    const size_t v2_newsz = cursorrec::value_descriptor::nbytes_fn(2)((const uint8_t *) &v0.v2);
    const size_t v4_newsz = cursorrec::value_descriptor::nbytes_fn(4)((const uint8_t *) &v0.v4);
    ...
    cursorrec::value v2;
    for (size_t i = rc1.field(); i < cursorrec::value_descriptor::nfields(); i = rc1.field())
...
public:
  main_thread(int argc, char **argv) : ndb_thread(false, string("main")), argc(argc), argv(argv), ret(0) {}
  ...
  inline int retval() const { return ret; }
