...
template<typename P>
void btree<P>::node::base_invariant_checker(const key_slice *min_key, const key_slice *max_key, bool is_root) const {
  ...
  if (is_root) {
    if (is_internal_node()) ALWAYS_ASSERT(n >= 1);
  } else {
    if (is_internal_node()) ALWAYS_ASSERT(n >= NMinKeysPerNode);
    else
...
template<typename P>
void btree<P>::node::invariant_checker(const key_slice *min_key, const key_slice *max_key, const node *left_sibling,
                                       const node *right_sibling, bool is_root) const {
  is_leaf_node() ? AsLeaf(this)->invariant_checker_impl(min_key, max_key, left_sibling, right_sibling, is_root)
...
template<typename P>
btree<P>::leaf_node::leaf_node() : node(),
                                   min_key_(0),
                                   prev_(NULL),
                                   next_(NULL),
                                   suffixes_(NULL) {
...
btree<P>::leaf_node::~leaf_node() {
  if (suffixes_) delete[] suffixes_;
  // suffixes_ = NULL;
  //++evt_btree_leaf_node_deletes;
}

template<typename P>
void btree<P>::leaf_node::invariant_checker_impl(const key_slice *min_key, const key_slice *max_key,
  ...
  size_t n = this->key_slots_used();
  for (size_t i = 0; i < n; i++)
    if (this->value_is_layer(i)) this->values_[i].n_->invariant_checker(NULL, NULL, NULL, NULL, true);
}
...
template<typename P>
void btree<P>::internal_node::invariant_checker_impl(const key_slice *min_key, const key_slice *max_key,
                                                     const node *left_sibling, const node *right_sibling,
                                                     bool is_root) const {
  this->base_invariant_checker(min_key, max_key, is_root);
  size_t n = this->key_slots_used();
  for (size_t i = 0; i <= n; i++) {
    ...
    if (i == 0) {
      ...
      if (left_sibling) left_child_sibling = AsInternal(left_sibling)->children_[left_sibling->key_slots_used()];
      this->children_[0]->invariant_checker(min_key, &this->keys_[0], left_child_sibling, this->children_[i + 1],
                                            false);
    } else if (i == n) {
      const node *right_child_sibling = NULL;
      if (right_sibling) right_child_sibling = AsInternal(right_sibling)->children_[0];
      this->children_[n]->invariant_checker(&this->keys_[n - 1], max_key, this->children_[i - 1], right_child_sibling,
                                            false);
    } else {
      this->children_[i]->invariant_checker(&this->keys_[i - 1], &this->keys_[i], this->children_[i - 1],
                                            this->children_[i + 1], false);
    }
  }
  if (!n || this->children_[0]->is_internal_node()) return;
  for (size_t i = 0; i <= n; i++) {
    const node *left_child_sibling = NULL;
    const node *right_child_sibling = NULL;
    if (left_sibling) left_child_sibling = AsInternal(left_sibling)->children_[left_sibling->key_slots_used()];
...
template<typename P>
void btree<P>::recursive_delete(node *n) {
  if (leaf_node *leaf = AsLeafCheck(n)) {
    ...
    for (size_t i = 0; i < n; i++)
      if (leaf->value_is_layer(i)) recursive_delete(leaf->values_[i].n_);
    leaf_node::deleter(leaf);
  } else {
    internal_node *internal = AsInternal(n);
    size_t n = internal->key_slots_used();
    for (size_t i = 0; i < n + 1; i++) recursive_delete(internal->children_[i]);
#ifdef CHECK_INVARIANTS
    internal->lock();
...
template<typename P>
bool btree<P>::search_impl(const key_type &k, value_type &v, typename util::vec<leaf_node *>::type &leaf_nodes,
  ...
  process:
    ...
    if (leaf_node *leaf = AsLeafCheck(cur, version)) {
      ...
      if (ret != -1) {
        ...
        INVARIANT(!value_is_layer || kslicelen == 9);
        varkey suffix(leaf->suffix(ret));
        if (unlikely(!leaf->check_version(version))) goto process;
        leaf_nodes.push_back(leaf);

        if (!is_layer) {
          // check suffixes
          if (kslicelen == 9 && suffix != kcur.shift()) return false;
          v = vn.v_;
          return true;
        }
      ...
      if (unlikely(kslice < leaf->min_key_)) {
        ...
        if (unlikely(!right_sibling->check_version(right_version))) goto process;
      ...
      ssize_t ret = kret.first;
      if (ret != -1) cur = internal->children_[ret + 1];
      else
        cur = internal->children_[0];
      if (unlikely(!internal->check_version(version))) goto process;
...
template<typename P>
bool btree<P>::search_range_at_layer(leaf_node *leaf, string_type &prefix, const key_type &lower, bool inc_lower,
  ...
  while (!upper || next_key <= upper_slice) {
    ...
    for (size_t i = 0; i < leaf->key_slots_used(); i++) {
      ...
      if ((leaf->keys_[i] > lower_slice ||
           (leaf->keys_[i] == lower_slice && leaf->keyslice_length(i) >= std::min(lower.size(), size_t(9)))) &&
          (!upper || leaf->keys_[i] <= upper_slice))
        buf.emplace_back(leaf->keys_[i], leaf->values_[i], leaf->value_is_layer(i), leaf->keyslice_length(i),
                         leaf->suffix(i));
    }
    ...
    if (unlikely(!leaf->check_version(version))) continue;
    ...
    for (size_t i = 0; i < buf.size(); i++) {
      ...
      if (emitted_last_keyslice &&
          ((buf[i].key_ < last_keyslice) || (buf[i].key_ == last_keyslice && buf[i].length_ <= last_keyslice_len)))
        continue;
      ...
      if (buf[i].layer_) {
        ...
        if (!search_range_at_layer(next_layer, prefix, zerokey, false, NULL, callback)) return false;
      } else {
        ...
        if (upper && buf[i].key_ == upper_slice) {
          if (buf[i].length_ == 9) {
            if (upper->size() <= 8) break;
            if (buf[i].suffix_ >= upper->shift()) break;
          } else if (buf[i].length_ >= upper->size()) {
            break;
          }
        }
        if (buf[i].length_ == 9) prefix.append((const char *) buf[i].suffix_.data(), buf[i].suffix_.size());
        ...
        if (!callback.invoke(prefix, buf[i].vn_.v_, leaf, RawVersionManip::Version(version))) return false;
      }
...
void btree<P>::search_range_call(const key_type &lower, const key_type *upper,
                                 low_level_search_range_callback &callback, string_type *buf) const {
  rcu_region guard;
  INVARIANT(rcu::s_instance.in_rcu_region());
...
#endif
    if (!search_range_at_layer(cur, prefix, lower.shift_many(leaf_nodes.size()), first,
                               layer_has_upper ? &layer_upper : NULL, callback))
      return;
#ifdef CHECK_INVARIANTS
...
template<typename P>
bool btree<P>::remove_stable_location(node **root_location, const key_type &k, value_type *old_v) {
  INVARIANT(rcu::s_instance.in_rcu_region());
retry:
  key_slice new_key;
...
typename btree<P>::leaf_node *btree<P>::leftmost_descend_layer(node *n) const {
  node *cur = n;
  while (true) {
    if (leaf_node *leaf = AsLeafCheck(cur)) return leaf;
    internal_node *internal = AsInternal(cur);
    uint64_t version = cur->stable_version();
    node *child = internal->children_[0];
    if (unlikely(!internal->check_version(version))) continue;
    cur = child;
  }
}
...
void btree<P>::tree_walk(tree_walk_callback &callback) const {
  ...
  while (!q.empty()) {
    ...
    process:
      ...
      for (size_t i = 0; i < n; i++)
        if (leaf->value_is_layer(i)) layers.push_back(leaf->values_[i].n_);
...
void btree<P>::size_walk_callback::on_node_begin(const node_opaque_t *n) {
  ...
  for (size_t i = 0; i < sz; i++)
    if (!leaf->value_is_layer(i)) spec_size_++;
}

template<typename P>
void btree<P>::size_walk_callback::on_node_success() {
  size_ += spec_size_;
  spec_size_ = 0;
}

template<typename P>
void btree<P>::size_walk_callback::on_node_failure() {
  spec_size_ = 0;
}

template<typename P>
typename btree<P>::leaf_node *btree<P>::FindRespLeafNode(leaf_node *leaf, uint64_t kslice, uint64_t &version) {
retry:
...
template<typename P>
typename btree<P>::leaf_node *btree<P>::FindRespLeafLowerBound(leaf_node *leaf, uint64_t kslice, size_t kslicelen,
  ...
  for (size_t i = (ret == -1 ? 0 : ret); i < n; i++) {
    if (leaf->keys_[i] < kslice) {
      ...
      if (kslicelen0 <= kslicelen) {
        ...
        idxlowerbound = i;
        if (kslicelen0 == kslicelen) idxmatch = i;
      }
    } else {
      break;
    }
...
template<typename P>
typename btree<P>::leaf_node *btree<P>::FindRespLeafExact(leaf_node *leaf, uint64_t kslice, size_t kslicelen,
                                                          uint64_t &version, size_t &n, ssize_t &idxmatch) {
  leaf = FindRespLeafNode(leaf, kslice, version);
  key_search_ret kret = leaf->key_search(kslice, kslicelen);
  idxmatch = kret.first;
...
template<typename P>
typename btree<P>::insert_status btree<P>::insert0(node *np, const key_type &k, value_type v, bool only_if_absent,
  ...
  retry_cur_leaf:
    ...
    leaf_node *resp_leaf = FindRespLeafLowerBound(leaf, kslice, kslicelen, version, n, lenmatch, lenlowerbound);

    // len match case
    if (lenmatch != -1) {
      // exact match case
      if (kslicelen <= 8 || (!resp_leaf->value_is_layer(lenmatch) && resp_leaf->suffix(lenmatch) == k.shift())) {
        ...
        if (old_v) *old_v = resp_leaf->values_[lenmatch].v_;
        if (!only_if_absent) resp_leaf->values_[lenmatch].v_ = v;
        if (insert_info) insert_info->node = 0;
      ...
      if (resp_leaf->value_is_layer(lenmatch)) {
        ...
        INVARIANT(subroot);
        if (unlikely(!resp_leaf->check_version(version))) goto retry_cur_leaf;
        key_slice mk;
        node *ret;
        ...
        const insert_status status =
            insert0(subroot, k.shift(), v, only_if_absent, old_v, insert_info, mk, ret, subparents, sub_locked_nodes);
        ...
        switch (status) {
          ...
          case I_SPLIT:
            ...
            for (;;) {
              resp_leaf = FindRespLeafLowerBound(resp_leaf, kslice, kslicelen, version, n, lenmatch, lenlowerbound);
              const uint64_t locked_version = resp_leaf->lock();
        ...
        typename util::vec<node *>::type sub_locked_nodes;
        const insert_status status =
            insert0(new_root, k.shift(), v, only_if_absent, old_v, insert_info, mk, ret, subparents, sub_locked_nodes);
        if (status != I_NONE_MOD) INVARIANT(false);
        INVARIANT(sub_locked_nodes.empty());
        return UnlockAndReturn(locked_nodes, I_NONE_MOD);
      }
    }
    ...
    if (n < NKeysPerNode) {
      ...
      if (resp_leaf->suffixes_) sift_swap_right(resp_leaf->suffixes_, lenlowerbound + 1, n);
      if (kslicelen == 9) {
      ...
      if (parents.empty()) {
        if (unlikely(!resp_leaf->is_root())) return UnlockAndReturn(locked_nodes, I_RETRY);
        ...
        for (auto rit = parents.rbegin(); rit != parents.rend(); ++rit) {
          ...
          if ((rit + 1) == parents.rend()) {
            // did the root change?
            if (unlikely(!p->is_root())) return UnlockAndReturn(locked_nodes, I_RETRY);
            // INVARIANT(p == root);
          }
      ...
      if (!resp_leaf->next_ && resp_leaf->keys_[n - 1] < kslice) {
        ...
        for (ssize_t i = left_split_point - 1; i >= 0; i--) {
          if (likely(resp_leaf->keys_[i] != resp_leaf->keys_[left_split_point])) break;
          left_split_point--;
        }
        INVARIANT(left_split_point <= NKeysPerNode);
        INVARIANT(left_split_point == 0 ||
                  resp_leaf->keys_[left_split_point - 1] != resp_leaf->keys_[left_split_point]);

        right_split_point = NKeysPerNode / 2;
        for (ssize_t i = right_split_point - 1; i >= 0 && i < ssize_t(NKeysPerNode) - 1; i++) {
          if (likely(resp_leaf->keys_[i] != resp_leaf->keys_[right_split_point])) break;
          right_split_point++;
        }
        INVARIANT(right_split_point <= NKeysPerNode);
        INVARIANT(right_split_point == 0 ||
                  resp_leaf->keys_[right_split_point - 1] != resp_leaf->keys_[right_split_point]);
        ...
        if (split_point <= size_t(lenlowerbound + 1) && resp_leaf->keys_[split_point - 1] != kslice) {
          ...
          resp_leaf->keyslice_set_length(lenlowerbound + 1, kslicelen, false);
          if (resp_leaf->suffixes_) sift_swap_right(resp_leaf->suffixes_, lenlowerbound + 1, split_point);
          if (kslicelen == 9) {
            resp_leaf->ensure_suffixes();
            rcu_imstring i(k.data() + 8, k.size() - 8);
            resp_leaf->suffixes_[lenlowerbound + 1].swap(i);
      ...
      if (resp_leaf->next_) resp_leaf->next_->prev_ = new_leaf;
    ...
    if (unlikely(RawVersionManip::IsDeleting(version))) return UnlockAndReturn(locked_nodes, I_RETRY);
    key_search_ret kret = internal->key_lower_bound_search(kslice);
    ssize_t ret = kret.first;
    size_t n = kret.second;
    size_t child_idx = (ret == -1) ? 0 : ret + 1;
    node *child_ptr = internal->children_[child_idx];
    if (unlikely(!internal->check_version(version))) return UnlockAndReturn(locked_nodes, I_RETRY);
    parents.push_back(insert_parent_entry(internal, version));
    key_slice mk = 0;
    node *new_child = NULL;
    insert_status status =
        insert0(child_ptr, k, v, only_if_absent, old_v, insert_info, mk, new_child, parents, locked_nodes);
    if (status != I_SPLIT) {
      INVARIANT(locked_nodes.empty());
      return status;
    }
...
template<typename P>
bool btree<P>::insert_stable_location(node **root_location, const key_type &k, value_type v, bool only_if_absent,
...
retry:
  ...
  node *local_root = *root_location;
  const insert_status status =
      insert0(local_root, k, v, only_if_absent, old_v, insert_info, mk, ret, parents, locked_nodes);
 ...
 */
template<typename P>
typename btree<P>::remove_status btree<P>::remove0(node *np, key_slice *min_key, key_slice *max_key, const key_type &k,
                                                   value_type *old_v, node *left_node, node *right_node,
                                                   key_slice &new_key, node *&replace_node,
                                                   typename util::vec<remove_parent_entry>::type &parents,
                                                   typename util::vec<node *>::type &locked_nodes) {
  uint64_t kslice = k.slice();
  size_t kslicelen = std::min(k.size(), size_t(9));
  ...
  retry_cur_leaf:
    ...
    size_t n;
    ssize_t ret;
    leaf_node *resp_leaf = FindRespLeafExact(leaf, kslice, kslicelen, version, n, ret);
    ...
    if (kslicelen == 9) {
      if (resp_leaf->value_is_layer(ret)) {
        ...
        typename util::vec<node *>::type sub_locked_nodes;
        remove_status status = remove0(subroot, NULL,          /* min_key */
                                       NULL,                   /* max_key */
        ...
        switch (status) {
          ...
          case R_REPLACE_NODE:
            ...
            for (;;) {
              resp_leaf = FindRespLeafExact(resp_leaf, kslice, kslicelen, version, n, ret);
              const uint64_t locked_version = resp_leaf->lock();
              if (likely(btree::CheckVersion(version, locked_version))) {
                locked_nodes.push_back(resp_leaf);
                break;
    ...
    if (n > NMinKeysPerNode) {
      ...
      locked_nodes.push_back(resp_leaf);
      if (old_v) *old_v = resp_leaf->values_[ret].v_;
      resp_leaf->mark_modifying();
      remove_pos_from_leaf_node(resp_leaf, ret, n);
      return UnlockAndReturn(locked_nodes, R_NONE_MOD);
    } else {

      if (unlikely(resp_leaf != leaf)) return UnlockAndReturn(locked_nodes, R_RETRY);
      ...
      if (old_v) *old_v = leaf->values_[ret].v_;
      ...
      if (right_sibling) {
        ...
        leaf->lock();
        if (unlikely(!leaf->check_version(leaf_version))) return UnlockAndReturn(locked_nodes, R_RETRY);
      } else {
        INVARIANT(parents.empty());
        if (unlikely(!leaf->is_root())) return UnlockAndReturn(locked_nodes, R_RETRY);
        // INVARIANT(leaf == root);
      }
      ...
      for (typename util::vec<remove_parent_entry>::type::reverse_iterator rit = parents.rbegin();
        ...
        p->lock();
        locked_nodes.push_back(p);
        if (unlikely(!p->check_version(p_version))) return UnlockAndReturn(locked_nodes, R_RETRY);
        size_t p_n = p->key_slots_used();
        if (p_n > NMinKeysPerNode) break;
        if (r) {
          r->lock();
          ...
          if (unlikely(!p->is_root())) return UnlockAndReturn(locked_nodes, R_RETRY);
          // INVARIANT(p == root);
        }
      }
      ...
      if (right_sibling) {
        ...
        if (right_n > NMinKeysPerNode) {
          ...
          for (size_t i = 0; i < right_n - 1; i++, steal_point++)
            if (likely(right_sibling->keys_[i] != right_sibling->keys_[steal_point])) break;
          ...
          if ((n - 1 + steal_point) <= NKeysPerNode && steal_point < right_n) {
            ...
            if (leaf->suffixes_) sift_swap_left(leaf->suffixes_, ret, n);
            ...
            if (right_sibling->suffixes_) sift_swap_left(right_sibling->suffixes_, 0, right_n, steal_point);
        ...
        if (right_sibling->next_) right_sibling->next_->prev_ = leaf;
      ...
      if (left_sibling) {
        ...
        if (left_n > NMinKeysPerNode) {
          ...
          for (ssize_t i = steal_point - 1; i >= 0; i--, steal_point--)
            if (likely(left_sibling->keys_[i] != left_sibling->keys_[steal_point])) break;
        ...
        if (leaf->next_) leaf->next_->prev_ = left_sibling;
        ...
        SINGLE_THREADED_INVARIANT(!left_sibling->prev_ || left_sibling->prev_->next_ == left_sibling);
    ...
    internal_node *internal = AsInternal(np);
    uint64_t version = internal->stable_version();
    if (unlikely(RawVersionManip::IsDeleting(version))) return UnlockAndReturn(locked_nodes, R_RETRY);
    key_search_ret kret = internal->key_lower_bound_search(kslice);
    ssize_t ret = kret.first;
    size_t n = kret.second;
    size_t child_idx = (ret == -1) ? 0 : ret + 1;
    ...
    if (unlikely(!internal->check_version(version))) return UnlockAndReturn(locked_nodes, R_RETRY);
    parents.push_back(remove_parent_entry(internal, left_node, right_node, version));
    INVARIANT(n > 0);
    key_slice nk;
    node *rn;
    remove_status status = remove0(child_ptr, child_min_key, child_max_key, k, old_v, child_left_sibling,
...
std::string btree<P>::NodeStringify(const node_opaque_t *n) {
  ...
  for (size_t i = 0; i < n->key_slots_used(); i++) keys.push_back(std::string("0x") + util::hexify(n->keys_[i]));

  std::ostringstream b;
  b << "node[v=" << n->version_info_str() << ", keys=" << util::format_list(keys.begin(), keys.end());
  ...
  if (n->is_leaf_node()) {
    ...
    for (size_t i = 0; i < leaf->key_slots_used(); i++) {
      ...
      inf << "<l=" << leaf->keyslice_length(i) << ",value_is_layer=" << leaf->value_is_layer(i) << ">";
...
template<typename P>
std::vector<std::pair<typename btree<P>::value_type, bool>> btree<P>::ExtractValues(const node_opaque_t *n) {
  std::vector<std::pair<value_type, bool>> ret;
  if (!n->is_leaf_node()) return ret;
  const leaf_node *leaf = (const leaf_node *) n;
  const size_t sz = leaf->key_slots_used();
  for (size_t i = 0; i < sz; i++)
    if (!leaf->value_is_layer(i)) ret.emplace_back(leaf->values_[i].v_, leaf->keyslice_length(i) > 8);
  return ret;
}
