...
template <typename T> static inline std::string Encode(const T &t) {
  const encoder<T> enc;
  return enc.write(&t);
}
...
template <typename T> static inline const T *Decode(const char *buf, T &obj) {
  const encoder<T> enc;
...
template <typename T> static inline size_t Size(const T &t) {
  const encoder<T> enc;
  return enc.nbytes(&t);
}
...
#define DO_STRUCT(name, keyfields, valuefields)                                \
  ...
  namespace private_ {                                                         \
  template <> struct is_trivially_destructible<name::key> {                    \
    static const bool value = true;                                            \
  };                                                                           \
  }                                                                            \
  template <> struct encoder<name::key> {                                      \
  ...
  namespace private_ {                                                         \
  template <> struct is_trivially_destructible<name::value> {                  \
    static const bool value = true;                                            \
  };                                                                           \
  }                                                                            \
  template <> struct encoder<name::value> {                                    \
    inline void encode_write(uint8_t *buf,                                     \
