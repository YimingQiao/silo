...
typedef const uint8_t *(*generic_failsafe_read_fn)(const uint8_t *, size_t,
                                                   uint8_t *);
...
template <typename Serializer> struct generic_serializer {
  ...
  static inline const uint8_t *failsafe_read(const uint8_t *buf, size_t nbytes,
                                             uint8_t *obj) {
    return Serializer::failsafe_read(buf, nbytes,
                                     reinterpret_cast<obj_type *>(obj));
  }
...
template <typename T, bool Compress> struct serializer {
  typedef T obj_type;
...
template <> struct serializer<uint32_t, true> {
  typedef uint32_t obj_type;

  static inline uint8_t *write(uint8_t *buf, uint32_t obj) {
    return write_uvint32(buf, obj);
  }

  static inline const uint8_t *read(const uint8_t *buf, uint32_t *obj) {
  ...
  static inline size_t nbytes(const uint32_t *obj) {
...
template <> struct serializer<int32_t, true> {
