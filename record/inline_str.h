...
public:
  ...
  inline_str_base(const char *s) { assign(s); }

  inline_str_base(const char *s, size_t n) { assign(s, n); }

  inline_str_base(const std::string &s) { assign(s); }

  inline_str_base(const inline_str_base &that) : sz(that.sz) { NDB_MEMCPY(&buf[0], &that.buf[0], sz); }
  ...
  inline ALWAYS_INLINE void assign(const std::string &s) { assign(s.data(), s.size()); }
  ...
  inline bool operator==(const inline_str_base &other) const { return memcmp(buf, other.buf, sz) == 0; }

  inline bool operator!=(const inline_str_base &other) const { return !operator==(other); }
...
template<typename IntSizeType, unsigned int N>
inline std::ostream &operator<<(std::ostream &o, const inline_str_base<IntSizeType, N> &s) {
...
public:
  ...
  inline_str_fixed(const inline_str_fixed &that) { NDB_MEMCPY(&buf[0], &that.buf[0], N); }
  ...
  inline ALWAYS_INLINE std::string str() const { return std::string(&buf[0], N); }
  ...
  inline bool operator==(const inline_str_fixed &other) const { return memcmp(buf, other.buf, N) == 0; }

  inline bool operator!=(const inline_str_fixed &other) const { return !operator==(other); }
...
template<unsigned int N, char FillChar>
inline std::ostream &operator<<(std::ostream &o, const inline_str_fixed<N, FillChar> &s) {
  o << std::string(s.data(), s.size());
  return o;
}
...
struct serializer<inline_str_base<IntSizeType, N>, Compress> {
  ...
  static const uint8_t *failsafe_read(const uint8_t *buf, size_t nbytes, obj_type *obj) {
    const uint8_t *const hdrbuf = serializer<IntSizeType, Compress>::failsafe_read(buf, nbytes, &obj->sz);
    if (unlikely(!hdrbuf)) return nullptr;
    nbytes -= (hdrbuf - buf);
  ...
  static inline size_t failsafe_skip(const uint8_t *stream, size_t nbytes, uint8_t *oldv) {
    IntSizeType sz = 0;
    const uint8_t *const body = serializer<IntSizeType, Compress>::failsafe_read(stream, nbytes, &sz);
    if (unlikely(!body)) return 0;
    nbytes -= (body - stream);
  ...
  static inline constexpr size_t max_nbytes() { return serializer<IntSizeType, Compress>::max_bytes() + N; }
};
