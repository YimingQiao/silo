...
template <typename T> struct read_record_cursor {
public:
...
template <typename T> struct write_record_cursor {
public:
  typedef typename T::value value_type;
  typedef typename T::value_descriptor value_descriptor_type;
  ...
  inline void write_current_and_advance(const value_type *v,
                                        uint8_t *old_v = nullptr) {
    static event_counter evt_write_memmove(util::cxx_typename<T>::value() +
                                           std::string("_write_memmove"));
