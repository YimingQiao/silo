...
public:
  ...
  read_record_cursor(const uint8_t *px, size_t nbytes) : px_begin(px), nbytes(nbytes), px_cur(px), n(0) {}

  // returns true on success, false on fail
  inline bool skip_to(size_t i) {
    INVARIANT(i >= n);
    INVARIANT(i < value_descriptor_type::nfields());
    while (n < i) {
      const size_t sz = value_descriptor_type::failsafe_skip_fn(n++)(px_cur, nbytes, nullptr);
      if (unlikely(!sz)) return false;
      px_cur += sz;
      nbytes -= sz;
    }
    return true;
  ...
  inline size_t field() const { return n; }

  // returns 0 on failure
  inline size_t read_current_and_advance(value_type *v) {
    INVARIANT(n < value_descriptor_type::nfields());
    uint8_t *const buf = reinterpret_cast<uint8_t *>(v) + value_descriptor_type::cstruct_offsetof(n);
    const uint8_t *const px_skip = value_descriptor_type::failsafe_read_fn(n++)(px_cur, nbytes, buf);
    if (unlikely(!px_skip)) return 0;
  ...
  // returns 0 on failure
  inline size_t read_current_raw_size_and_advance() {
    INVARIANT(n < value_descriptor_type::nfields());
    const size_t rawsz = value_descriptor_type::failsafe_skip_fn(n++)(px_cur, nbytes, nullptr);
...
public:
  ...
  write_record_cursor(uint8_t *px) : px_begin(px), px_cur(px), px_end(nullptr), n(0) { INVARIANT(px); }

  inline void skip_to(size_t i) {
    INVARIANT(i >= n);
    INVARIANT(i < value_descriptor_type::nfields());
    while (n < i) px_cur += value_descriptor_type::skip_fn(n++)(px_cur, nullptr);
  }
  ...
  inline void write_current_and_advance(const value_type *v, uint8_t *old_v = nullptr) {
    static event_counter evt_write_memmove(util::cxx_typename<T>::value() + std::string("_write_memmove"));
    const uint8_t *const buf = reinterpret_cast<const uint8_t *>(v) + value_descriptor_type::cstruct_offsetof(n);
...
private:
  inline void compute_end() {
    ...
    uint8_t *px = px_cur;
    size_t i = n;
    while (i < value_descriptor_type::nfields()) px += value_descriptor_type::skip_fn(i++)(px, nullptr);
    px_end = px;
  }
