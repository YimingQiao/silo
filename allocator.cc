...
static event_counter
    evt_allocator_total_region_usage("allocator_total_region_usage_bytes");
...
void allocator::Initialize(size_t ncpus, size_t maxpercore) {
  ...
  void *const x = mmap(nullptr, g_ncpus * g_maxpercore + hugepgsize, PROT_NONE,
  ...
  void *const endpx =
      (void *)((uintptr_t)x + g_ncpus * g_maxpercore + hugepgsize);
  ...
  ALWAYS_ASSERT(
      reinterpret_cast<uintptr_t>(g_memend) <=
...
void allocator::DumpStats() {
  ...
  for (size_t i = 0; i < g_ncpus; i++) {
    ...
    const size_t remaining =
        intptr_t(g_regions[i].region_end) - intptr_t(g_regions[i].region_begin);
    std::cerr << "[allocator] cpu=" << i << " fully_faulted?=" << f
              << " remaining=" << remaining << " bytes" << std::endl;
  }
}

static void *initialize_page(void *page, const size_t pagesize,
                             const size_t unit) {
...
void allocator::FaultRegion(size_t cpu) {
  ...
  if (reinterpret_cast<uintptr_t>(pc.region_begin) % hugepgsize)
    ALWAYS_ASSERT(false);
  const size_t sz = reinterpret_cast<uintptr_t>(pc.region_end) -
                    reinterpret_cast<uintptr_t>(pc.region_begin);
  void *const x = mmap(pc.region_begin, sz, PROT_READ | PROT_WRITE,
                       MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
  if (unlikely(x == MAP_FAILED)) {
    perror("mmap");
    std::cerr << "  cpu" << cpu << " [" << pc.region_begin << ", "
              << pc.region_end << ")" << std::endl;
    ALWAYS_ASSERT(false);
  }
  ...
  numa_hint_memory_placement(
      pc.region_begin, (uintptr_t)pc.region_end - (uintptr_t)pc.region_begin,
  ...
  timer t;
  for (char *px = (char *)pc.region_begin; px < (char *)pc.region_end;
       px += CACHELINE_SIZE)
    *px = 0xDE;
  std::cerr << "cpu" << cpu << " finished faulting region in " << t.lap_ms()
            << " ms" << std::endl;
