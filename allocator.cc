...
static event_counter evt_allocator_total_region_usage("allocator_total_region_usage_bytes");

// page+alloc routines taken from masstree

#ifdef MEMCHECK_MAGIC
const allocator::pgmetadata *allocator::PointerToPgMetadata(const void *p) {
  static const size_t hugepgsize = GetHugepageSize();
  if (unlikely(!ManagesPointer(p))) return nullptr;
  const size_t cpu = PointerToCpu(p);
  const regionctx &pc = g_regions[cpu];
  if (p >= pc.region_begin) return nullptr;
...
size_t allocator::GetHugepageSizeImpl() {
...
size_t allocator::GetPageSizeImpl() { return sysconf(_SC_PAGESIZE); }
...
void allocator::Initialize(size_t ncpus, size_t maxpercore) {
  static spinlock s_lock;
  static bool s_init = false;
  if (likely(s_init)) return;
  ...
  void *const x = mmap(nullptr, g_ncpus * g_maxpercore + hugepgsize, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (x == MAP_FAILED) {
    perror("mmap");
    ALWAYS_ASSERT(false);
  }
  ...
  g_memstart = reinterpret_cast<void *>(util::iceil_(uintptr_t(x), hugepgsize));
  ...
  for (size_t i = 0; i < g_ncpus; i++) {
    ...
    std::cerr << "cpu" << i << " owns [" << g_regions[i].region_begin << ", " << g_regions[i].region_end << ")"
              << std::endl;
...
void allocator::DumpStats() {
  std::cerr << "[allocator] ncpus=" << g_ncpus << std::endl;
  for (size_t i = 0; i < g_ncpus; i++) {
    const bool f = g_regions[i].region_faulted;
    const size_t remaining = intptr_t(g_regions[i].region_end) - intptr_t(g_regions[i].region_begin);
    std::cerr << "[allocator] cpu=" << i << " fully_faulted?=" << f << " remaining=" << remaining << " bytes"
              << std::endl;
  }
}

static void *initialize_page(void *page, const size_t pagesize, const size_t unit) {
  ...
  void *first = (void *) util::iceil_((uintptr_t) page, (uintptr_t) unit);
  ...
  while ((uintptr_t) next + unit <= (uintptr_t) page + pagesize) {
    ...
    *p = next;
#ifdef MEMCHECK_MAGIC
    NDB_MEMSET((char *) p + sizeof(void **), MEMCHECK_MAGIC, unit - sizeof(void **));
#endif
    p = (void **) next;
...
void *allocator::AllocateArenas(size_t cpu, size_t arena) {
...
void *allocator::AllocateUnmanaged(size_t cpu, size_t nhugepgs) {
  regionctx &pc = g_regions[cpu];
  pc.lock.lock();
  return AllocateUnmanagedWithLock(pc, nhugepgs);// releases lock
}

void *allocator::AllocateUnmanagedWithLock(regionctx &pc, size_t nhugepgs) {
  static const size_t hugepgsize = GetHugepageSize();

  void *const mypx = pc.region_begin;

  // check alignment
  if (reinterpret_cast<uintptr_t>(mypx) % hugepgsize) ALWAYS_ASSERT(false);

  void *const mynewpx = reinterpret_cast<char *>(mypx) + nhugepgs * hugepgsize;
  ...
  if (needs_mmap) {
    void *const x = mmap(mypx, hugepgsize, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
    if (unlikely(x == MAP_FAILED)) {
      perror("mmap");
      ALWAYS_ASSERT(false);
    }
    INVARIANT(x == mypx);
    const int advice = UseMAdvWillNeed() ? MADV_HUGEPAGE | MADV_WILLNEED : MADV_HUGEPAGE;
...
void allocator::ReleaseArenas(void **arenas) {
  // cpu -> [(head, tail)]
  ...
  for (auto &p: m) {
    ...
    for (size_t arena = 0; arena < MAX_ARENAS; arena++) {
      ...
      if (!p.second[arena].first) continue;
...
void allocator::FaultRegion(size_t cpu) {
  static const size_t hugepgsize = GetHugepageSize();
  ALWAYS_ASSERT(cpu < g_ncpus);
  regionctx &pc = g_regions[cpu];
  if (pc.region_faulted) return;
  ...
  if (reinterpret_cast<uintptr_t>(pc.region_begin) % hugepgsize) ALWAYS_ASSERT(false);
  const size_t sz = reinterpret_cast<uintptr_t>(pc.region_end) - reinterpret_cast<uintptr_t>(pc.region_begin);
  void *const x = mmap(pc.region_begin, sz, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
  if (unlikely(x == MAP_FAILED)) {
    perror("mmap");
    std::cerr << "  cpu" << cpu << " [" << pc.region_begin << ", " << pc.region_end << ")" << std::endl;
    ALWAYS_ASSERT(false);
  }
  ...
  numa_hint_memory_placement(pc.region_begin, (uintptr_t) pc.region_end - (uintptr_t) pc.region_begin,
                             numa_node_of_cpu(cpu));
  const size_t nfaults = ((uintptr_t) pc.region_end - (uintptr_t) pc.region_begin) / hugepgsize;
  std::cerr << "cpu" << cpu << " starting faulting region (" << intptr_t(pc.region_end) - intptr_t(pc.region_begin)
            << " bytes / " << nfaults << " hugepgs)" << std::endl;
  timer t;
  for (char *px = (char *) pc.region_begin; px < (char *) pc.region_end; px += CACHELINE_SIZE) *px = 0xDE;
  std::cerr << "cpu" << cpu << " finished faulting region in " << t.lap_ms() << " ms" << std::endl;
