...
namespace private_ {
  ...
  struct is_trivially_destructible<std::pair<K, V>> {
    static const bool value = is_trivially_destructible<K>::value && is_trivially_destructible<V>::value;
  };
  ...
  template<typename T>
  struct typeutil {
    typedef const T &func_param_type;
  };

  template<typename T>
  struct primitive_typeutil {
    typedef T func_param_type;
  };
...
#define SPECIALIZE_PRIM_TYPEUTIL(tpe) \
  template<>                          \
  struct typeutil<tpe> : public primitive_typeutil<tpe> {};
