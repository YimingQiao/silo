...
failsafe_read_uvint32_slow(const uint8_t *buf, size_t nbytes, uint32_t *value) {
  const uint8_t *p;
  uint32_t b, result;
...
failsafe_read_uvint32(const uint8_t *stream, size_t nbytes, uint32_t *value) {
  if (unlikely(!nbytes))
    return nullptr;
...
inline ALWAYS_INLINE size_t skip_uvint32(const uint8_t *stream, uint8_t *rawv) {
  if (rawv) {
    if (likely((rawv[0] = stream[0]) < 0x80))
      return 1;
    if (likely((rawv[1] = stream[1]) < 0x80))
      return 2;
    if (likely((rawv[2] = stream[2]) < 0x80))
      return 3;
    if (likely((rawv[3] = stream[3]) < 0x80))
      return 4;
    if (likely((rawv[4] = stream[4]) < 0x80))
      return 5;
  } else {
    if (likely(stream[0] < 0x80))
      return 1;
    if (likely(stream[1] < 0x80))
      return 2;
    if (likely(stream[2] < 0x80))
      return 3;
    if (likely(stream[3] < 0x80))
      return 4;
    if (likely(stream[4] < 0x80))
      return 5;
  }
  ALWAYS_ASSERT(false);
  return 0;
...
inline size_t size_uvint32(uint32_t value) {
  if (likely(value <= 0x7F))
    return 1;
  if (likely(value <= ((0x7F << 7) | 0x7F)))
    return 2;
  if (likely(value <= ((0x7F << 14) | (0x7F << 7) | 0x7F)))
    return 3;
