 ...
 */
inline ALWAYS_INLINE const uint8_t *read_uvint32(const uint8_t *buf, uint32_t *value) {
  if (likely(*buf < 0x80)) {
    *value = *buf;
    return buf + 1;
  }
...
inline const uint8_t *failsafe_read_uvint32_slow(const uint8_t *buf, size_t nbytes, uint32_t *value) {
  const uint8_t *p;
  uint32_t b, result;
...
inline ALWAYS_INLINE const uint8_t *failsafe_read_uvint32(const uint8_t *stream, size_t nbytes, uint32_t *value) {
  if (unlikely(!nbytes)) return nullptr;
...
inline ALWAYS_INLINE size_t failsafe_skip_uvint32(const uint8_t *stream, size_t nbytes, uint8_t *rawv) {
  if (rawv) {
