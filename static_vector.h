...
public:
  ...
  inline static_vector(const static_vector &that) : n(0) { assignFrom(that); }
  ...
  inline void resize(size_t n, value_type val = value_type()) {
    INVARIANT(n <= StaticSize);
    if (n > this->n) {
      // expand
...
private:
  ...
  public:
    ...
    inline bool operator==(const iterator_ &o) const { return p == o.p; }
    ...
    inline intptr_t operator-(const iterator_ &o) const { return p - o.p; }
...
public:
  ...
  inline iterator end() { return iterator(endptr()); }

  inline const_iterator end() const { return const_iterator(endptr()); }
...
private:
  ...
  inline ALWAYS_INLINE const T *endptr() const { return ptr() + n; }
