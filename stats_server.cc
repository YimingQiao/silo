...
void stats_server::serve_forever() {
  ...
  if (fd < 0) throw system_error(errno, system_category(), "creating UNIX domain socket");

  struct sockaddr_un addr;
  memset(&addr, 0, sizeof(addr));
  addr.sun_family = AF_UNIX;
  if (sockfile_.length() + 1 >= sizeof(addr.sun_path)) throw range_error("UNIX domain socket path too long");
  ...
  if (listen(fd, 5) < 0) throw system_error(errno, system_category(), "listening on " + sockfile_);

  for (;;) {
    int cfd = accept(fd, nullptr, 0);
    if (cfd < 0) throw system_error(errno, system_category(), "accept failed");
    thread(&stats_server::serve_client, this, cfd).detach();
  }
}

bool stats_server::handle_cmd_get_counter_value(const string &name, packet &pkt) {
  get_counter_value_t ret;
  ret.timestamp_us_ = timer::cur_usec();
  if (!event_counter::stat(name, ret.d_)) cerr << "could not find counter " << name << endl;
