...
if KNOB_ENABLE_TPCC_MULTIPART:
    ...
    grids += [
        ...
        },
        {
            ...
            'benchmarks': ['tpcc'],
            'bench_opts':
                ['--enable-separate-tree-per-partition --workload-mix 100,0,0,0,0 --new-order-remote-item-pct %d' % d
                 for d in D_RANGE],
            'par_load': [False],
            'retry': [False],
            'persist': [PERSIST_NONE],
        ...
        },
        {
            ...
            'bench_opts':
                [
                    '--workload-mix 100,0,0,0,0 --enable-separate-tree-per-partition --enable-partition-locks --new-order-remote-item-pct %d' % d
                    for d in D_RANGE],
            'par_load': [False],
            'retry': [False],
            'persist': [PERSIST_NONE],
...
if KNOB_ENABLE_TPCC_RO_SNAPSHOTS:
    ...
    grids += [
        ...
        },
        {
            ...
            'scale_factors': [8],
            'benchmarks': ['tpcc'],
            'bench_opts': ['--disable-read-only-snapshots --workload-mix 50,0,0,0,50 --new-order-remote-item-pct %d' % d
                           for d in RO_DRANGE],
            'par_load': [False],
            'retry': [True],
            'persist': [PERSIST_NONE],
...
def run_configuration(
    ...
    args = [
           ...
           + ([] if not assignments else list(
        it.chain.from_iterable([['--assignment', ','.join(map(str, x))] for x in assignments]))) \
    ...
    print >> sys.stderr, ('DISABLE_MADV_WILLNEED=1' if disable_madv_willneed else ''), ' '.join(
        [x.replace(' ', r'\ ') for x in args])
    if not DRYRUN:
    ...
    if len(toks) != 5:
        print
        'Failure: retcode=', retcode, ', stdout=', r
        import shutil
    ...
    if failed:
        ...
        for binary in failed:
            ...
            if not m:
                ...
                print >> sys.stderr, 'MASSTREE=%d MODE=%s make -j dbtest' % (
                    1 if m.group(2) == 'masstree' else 0, m.group(1))
        sys.exit(1)
