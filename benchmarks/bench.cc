...
void bench_worker::run() {
  ...
  barrier_b->wait_for();
  while (running &&
         (run_mode != RUNMODE_OPS || ntxn_commits < ops_per_worker)) {
...
void bench_runner::run() {
  ...
  const unsigned long elapsed =
      t.lap(); // lap() must come after do_txn_finish(),
  ...
  if (verbose) {
    ...
    cerr << "txn breakdown: "
         << format_list(agg_txn_counts.begin(), agg_txn_counts.end()) << endl;
  ...
  cout << agg_throughput << " " << agg_persist_throughput << " "
       << avg_latency_ms << " " << avg_persist_latency_ms << " "
       << agg_abort_rate << endl;
  cout.flush();
