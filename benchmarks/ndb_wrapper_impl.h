...
#define TXN_PROFILE_HINT_OP(x)                                                 \
  x(abstract_db::HINT_DEFAULT, hint_default_traits) x(                         \
      abstract_db::HINT_KV_GET_PUT,                                            \
      hint_kv_get_put_traits) x(abstract_db::HINT_KV_RMW, hint_kv_rmw_traits)  \
      x(abstract_db::HINT_KV_SCAN, hint_kv_scan_traits)                        \
          x(abstract_db::HINT_TPCC_NEW_ORDER, hint_tpcc_new_order_traits)      \
              x(abstract_db::HINT_TPCC_PAYMENT, hint_tpcc_payment_traits) x(   \
                  abstract_db::HINT_TPCC_DELIVERY, hint_tpcc_delivery_traits)  \
                  x(abstract_db::HINT_TPCC_ORDER_STATUS,                       \
                    hint_tpcc_order_status_traits)                             \
                      x(abstract_db::HINT_TPCC_ORDER_STATUS_READ_ONLY,         \
                        hint_tpcc_order_status_read_only_traits)               \
                          x(abstract_db::HINT_TPCC_STOCK_LEVEL,                \
                            hint_tpcc_stock_level_traits)                      \
                              x(abstract_db::HINT_TPCC_STOCK_LEVEL_READ_ONLY,  \
                                hint_tpcc_stock_level_read_only_traits)
...
template <template <typename> class Transaction>
void *ndb_wrapper<Transaction>::new_txn(uint64_t txn_flags, str_arena &arena,
...
template <typename T> static inline ALWAYS_INLINE void Destroy(T *t) {
  PERF_DECL(static std::string probe1_name(std::string(__PRETTY_FUNCTION__) +
                                           std::string(":total:")));
...
template <template <typename> class Transaction>
abstract_ordered_index *ndb_wrapper<Transaction>::open_index(
    const std::string &name, size_t value_size_hint, bool mostly_append) {
  return new ndb_ordered_index<Transaction>(name, value_size_hint,
                                            mostly_append);
}
...
template <template <typename> class Transaction>
ndb_ordered_index<Transaction>::ndb_ordered_index(const std::string &name,
...
template <template <typename> class Transaction>
bool ndb_ordered_index<Transaction>::get(void *txn, const std::string &key,
                                         std::string &value,
...
template <template <typename> class Transaction>
const char *ndb_ordered_index<Transaction>::put(void *txn,
...
template <template <typename> class Transaction>
const char *ndb_ordered_index<Transaction>::put(void *txn, std::string &&key,
                                                std::string &&value) {
  ndbtxn *const p = reinterpret_cast<ndbtxn *>(txn);
  try {
#define MY_OP_X(a, b)                                                          \
...
template <template <typename> class Transaction>
const char *ndb_ordered_index<Transaction>::insert(void *txn,
                                                   const std::string &key,
                                                   const std::string &value) {
  PERF_DECL(static std::string probe1_name(std::string(__PRETTY_FUNCTION__) +
                                           std::string(":total:")));
...
template <template <typename> class Transaction>
const char *ndb_ordered_index<Transaction>::insert(void *txn, std::string &&key,
                                                   std::string &&value) {
  ndbtxn *const p = reinterpret_cast<ndbtxn *>(txn);
  try {
#define MY_OP_X(a, b)                                                          \
...
template <template <typename> class Transaction>
class ndb_wrapper_search_range_callback
    : public txn_btree<Transaction>::search_range_callback {
public:
  ndb_wrapper_search_range_callback(
...
template <template <typename> class Transaction>
void ndb_ordered_index<Transaction>::scan(void *txn,
...
template <template <typename> class Transaction>
void ndb_ordered_index<Transaction>::rscan(void *txn,
                                           const std::string &start_key,
                                           const std::string *end_key,
                                           scan_callback &callback,
                                           str_arena *arena) {
