...
#define TXN_PROFILE_HINT_OP(x)                                                                                         \
  x(abstract_db::HINT_DEFAULT, hint_default_traits) x(abstract_db::HINT_KV_GET_PUT, hint_kv_get_put_traits)            \
      x(abstract_db::HINT_KV_RMW, hint_kv_rmw_traits) x(abstract_db::HINT_KV_SCAN, hint_kv_scan_traits)                \
              ...
              x(abstract_db::HINT_TPCC_PAYMENT, hint_tpcc_payment_traits)                                              \
                  x(abstract_db::HINT_TPCC_DELIVERY, hint_tpcc_delivery_traits)                                        \
                      x(abstract_db::HINT_TPCC_ORDER_STATUS, hint_tpcc_order_status_traits)                            \
                          x(abstract_db::HINT_TPCC_ORDER_STATUS_READ_ONLY, hint_tpcc_order_status_read_only_traits) x( \
                              abstract_db::HINT_TPCC_STOCK_LEVEL, hint_tpcc_stock_level_traits)                        \
                              x(abstract_db::HINT_TPCC_STOCK_LEVEL_READ_ONLY, hint_tpcc_stock_level_read_only_traits)

template<template<typename> class Transaction>
ndb_wrapper<Transaction>::ndb_wrapper(const std::vector<std::string> &logfiles,
                                      const std::vector<std::vector<unsigned>> &assignments_given, bool call_fsync,
                                      bool use_compression, bool fake_writes) {
  if (logfiles.empty()) return;
  std::vector<std::vector<unsigned>> assignments_used;
  txn_logger::Init(nthreads, logfiles, assignments_given, &assignments_used, call_fsync, use_compression, fake_writes);
  if (verbose) {
...
template<template<typename> class Transaction>
size_t ndb_wrapper<Transaction>::sizeof_txn_object(uint64_t txn_flags) const {
#define MY_OP_X(a, b) sizeof(typename cast<b>::type),
  const size_t xs[] = {TXN_PROFILE_HINT_OP(MY_OP_X)};
#undef MY_OP_X
  size_t xmax = 0;
  for (size_t i = 0; i < ARRAY_NELEMS(xs); i++) xmax = std::max(xmax, xs[i]);
  return xmax;
}

template<template<typename> class Transaction>
void *ndb_wrapper<Transaction>::new_txn(uint64_t txn_flags, str_arena &arena, void *buf, TxnProfileHint hint) {
...
template<typename T>
static inline ALWAYS_INLINE void Destroy(T *t) {
...
template<template<typename> class Transaction>
bool ndb_wrapper<Transaction>::commit_txn(void *txn) {
...
template<template<typename> class Transaction>
void ndb_wrapper<Transaction>::abort_txn(void *txn) {
  ndbtxn *const p = reinterpret_cast<ndbtxn *>(txn);
#define MY_OP_X(a, b)      \
  case a: {                \
    auto t = cast<b>()(p); \
...
template<template<typename> class Transaction>
void ndb_wrapper<Transaction>::print_txn_debug(void *txn) const {
  ndbtxn *const p = reinterpret_cast<ndbtxn *>(txn);
#define MY_OP_X(a, b)      \
  case a: {                \
    auto t = cast<b>()(p); \
...
template<template<typename> class Transaction>
std::map<std::string, uint64_t> ndb_wrapper<Transaction>::get_txn_counters(void *txn) const {
  ndbtxn *const p = reinterpret_cast<ndbtxn *>(txn);
#define MY_OP_X(a, b)             \
  case a: {                       \
    auto t = cast<b>()(p);        \
...
template<template<typename> class Transaction>
abstract_ordered_index *ndb_wrapper<Transaction>::open_index(const std::string &name, size_t value_size_hint,
                                                             bool mostly_append) {
  return new ndb_ordered_index<Transaction>(name, value_size_hint, mostly_append);
}

template<template<typename> class Transaction>
void ndb_wrapper<Transaction>::close_index(abstract_ordered_index *idx) {
  delete idx;
}
...
ndb_ordered_index<Transaction>::ndb_ordered_index(const std::string &name, size_t value_size_hint, bool mostly_append)
    : name(name),
      btr(value_size_hint, mostly_append, name) {
  // for debugging
  // std::cerr << name << " : btree= "
...
template<template<typename> class Transaction>
bool ndb_ordered_index<Transaction>::get(void *txn, const std::string &key, std::string &value, size_t max_bytes_read) {
  PERF_DECL(static std::string probe1_name(std::string(__PRETTY_FUNCTION__) + std::string(":total:")));
...
#define MY_OP_X(a, b)                                              \
  case a: {                                                        \
    auto t = cast<b>()(p);                                         \
    if (!btr.search(*t, key, value, max_bytes_read)) return false; \
    return true;                                                   \
  }
    switch (p->hint) {
      TXN_PROFILE_HINT_OP(MY_OP_X)
...
#undef MY_OP_X
    ...
    return true;
  } catch (transaction_abort_exception &ex) { throw abstract_db::abstract_abort_exception(); }
}

// XXX: find way to remove code duplication below using C++ templates!

template<template<typename> class Transaction>
const char *ndb_ordered_index<Transaction>::put(void *txn, const std::string &key, const std::string &value) {
  PERF_DECL(static std::string probe1_name(std::string(__PRETTY_FUNCTION__) + std::string(":total:")));
...
template<template<typename> class Transaction>
const char *ndb_ordered_index<Transaction>::put(void *txn, std::string &&key, std::string &&value) {
  ndbtxn *const p = reinterpret_cast<ndbtxn *>(txn);
  try {
#define MY_OP_X(a, b)                              \
  case a: {                                        \
...
template<template<typename> class Transaction>
const char *ndb_ordered_index<Transaction>::insert(void *txn, const std::string &key, const std::string &value) {
  PERF_DECL(static std::string probe1_name(std::string(__PRETTY_FUNCTION__) + std::string(":total:")));
...
template<template<typename> class Transaction>
const char *ndb_ordered_index<Transaction>::insert(void *txn, std::string &&key, std::string &&value) {
  ndbtxn *const p = reinterpret_cast<ndbtxn *>(txn);
  try {
#define MY_OP_X(a, b)                                 \
  case a: {                                           \
...
public:
  ndb_wrapper_search_range_callback(abstract_ordered_index::scan_callback &upcall) : upcall(&upcall) {}

  virtual bool invoke(const typename txn_btree<Transaction>::keystring_type &k,
...
template<template<typename> class Transaction>
void ndb_ordered_index<Transaction>::scan(void *txn, const std::string &start_key, const std::string *end_key,
...
template<template<typename> class Transaction>
void ndb_ordered_index<Transaction>::rscan(void *txn, const std::string &start_key, const std::string *end_key,
                                           scan_callback &callback, str_arena *arena) {
  ndbtxn *const p = reinterpret_cast<ndbtxn *>(txn);
  ndb_wrapper_search_range_callback<Transaction> c(callback);
  try {
...
template<template<typename> class Transaction>
void ndb_ordered_index<Transaction>::remove(void *txn, const std::string &key) {
  PERF_DECL(static std::string probe1_name(std::string(__PRETTY_FUNCTION__) + std::string(":total:")));
...
template<template<typename> class Transaction>
void ndb_ordered_index<Transaction>::remove(void *txn, std::string &&key) {
  ndbtxn *const p = reinterpret_cast<ndbtxn *>(txn);
  try {
#define MY_OP_X(a, b)               \
  case a: {                         \
...
template<template<typename> class Transaction>
size_t ndb_ordered_index<Transaction>::size() const {
  return btr.size_estimate();
}

template<template<typename> class Transaction>
std::map<std::string, uint64_t> ndb_ordered_index<Transaction>::clear() {
