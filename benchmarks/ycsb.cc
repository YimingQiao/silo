...
public:
  ycsb_worker(unsigned int worker_id, unsigned long seed, abstract_db *db,
      ...
      : bench_worker(worker_id, true, seed, db, open_tables, barrier_a,
                     barrier_b),
        tbl(open_tables.at("USERTABLE")), computation_n(0) {
  ...
  txn_result txn_read() {
    void *const txn =
        db->new_txn(txn_flags, arena, txn_buf(), abstract_db::HINT_KV_GET_PUT);
  ...
  txn_result txn_write() {
    ...
    scoped_str_arena s_arena(arena);
    try {
      tbl->put(txn, u64_varkey(r.next() % nkeys).str(str()),
               str().assign(YCSBRecordSize, 'b'));
  ...
  static txn_result TxnWrite(bench_worker *w) {
    return static_cast<ycsb_worker *>(w)->txn_write();
  }

  txn_result txn_rmw() {
    void *const txn =
        db->new_txn(txn_flags, arena, txn_buf(), abstract_db::HINT_KV_RMW);
    scoped_str_arena s_arena(arena);
    try {
  ...
  static txn_result TxnRmw(bench_worker *w) {
  ...
  txn_result txn_scan() {
    void *const txn =
        db->new_txn(txn_flags, arena, txn_buf(), abstract_db::HINT_KV_SCAN);
    scoped_str_arena s_arena(arena);
  ...
  static txn_result TxnScan(bench_worker *w) {
  ...
  virtual workload_desc_vec get_workload() const {
    ...
    if (g_txn_workload_mix[0])
      w.push_back(workload_desc("Read", double(g_txn_workload_mix[0]) / 100.0,
                                TxnRead));
    if (g_txn_workload_mix[1])
      w.push_back(workload_desc("Write", double(g_txn_workload_mix[1]) / 100.0,
                                TxnWrite));
    if (g_txn_workload_mix[2])
      w.push_back(workload_desc("ReadModifyWrite",
                                double(g_txn_workload_mix[2]) / 100.0, TxnRmw));
    if (g_txn_workload_mix[3])
      w.push_back(workload_desc("Scan", double(g_txn_workload_mix[3]) / 100.0,
                                TxnScan));
    return w;
  }
...
static void ycsb_load_keyrange(uint64_t keystart, uint64_t keyend,
                               unsigned int pinid, abstract_db *db,
                               abstract_ordered_index *tbl, str_arena &arena,
                               uint64_t txn_flags, void *txn_buf) {
  if (pin_cpus) {
  ...
  if (verbose)
    cerr << "[INFO] finished loading USERTABLE range [kstart=" << keystart
         << ", kend=" << keyend << ") - nkeys: " << nkeys << endl;
}
...
protected:
  virtual void load() {
    ...
    for (size_t i = 0; i < nthreads; i++) {
      ...
      ycsb_load_keyrange(keystart, keyend, i, db, tbl, arena, txn_flags,
                         txn_buf());
    }
  }
};
...
public:
  ycsb_parallel_usertable_loader(
      unsigned long seed, abstract_db *db,
      const map<string, abstract_ordered_index *> &open_tables,
      unsigned int pinid, uint64_t keystart, uint64_t keyend)
      : bench_loader(seed, db, open_tables), pinid(pinid), keystart(keystart),
        keyend(keyend) {
    INVARIANT(keyend > keystart);
    if (verbose)
      cerr << "[INFO] YCSB par loader cpu " << pinid << " [" << keystart << ", "
           << keyend << ")" << endl;
  }
...
protected:
  virtual void load() {
    abstract_ordered_index *tbl = open_tables.at("USERTABLE");
    ycsb_load_keyrange(keystart, keyend, pinid, db, tbl, arena, txn_flags,
                       txn_buf());
  }
...
protected:
    ...
    if (enable_parallel_loading && nkeys >= nthreads) {
      ...
      if (nthreads > ncpus) {
        for (size_t i = 0; i < ncpus; i++) {
          const uint64_t kend =
              (i + 1 == ncpus) ? nkeys : (i + 1) * nkeysperloader;
          ret.push_back(new ycsb_parallel_usertable_loader(
              0, db, open_tables, i, i * nkeysperloader, kend));
        }
      } else {
        ...
        vector<unsigned> node_allocations(numa_nodes_used.size(),
                                          loaders_per_node);
        // RR the remaining
        for (unsigned i = 0;
             i < (ncpus - loaders_per_node * numa_nodes_used.size()); i++)
          node_allocations[i]++;
  ...
  virtual vector<bench_worker *> make_workers() {
    ...
    for (size_t i = 0; i < nthreads; i++)
      ret.push_back(new ycsb_worker(blockstart + i, r.next(), db, open_tables,
                                    &barrier_a, &barrier_b));
    return ret;
  }
...
private:
  ...
  static vector<unsigned> exclude(const vector<unsigned> &cpus,
                                  unsigned nthds) {
    vector<unsigned> ret;
    for (auto n : cpus)
      if (n < nthds)
...
void ycsb_do_test(abstract_db *db, int argc, char **argv) {
  ...
  while (1) {
    static struct option long_options[] = {
        {"workload-mix", required_argument, 0, 'w'}, {0, 0, 0, 0}};
    int option_index = 0;
