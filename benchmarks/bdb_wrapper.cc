...
bdb_wrapper::bdb_wrapper(const string &envdir, const string &dbfile) : env(0) {
  ...
  ALWAYS_ASSERT(env->open(envdir.c_str(),
                          DB_INIT_LOCK | DB_INIT_LOG | DB_INIT_MPOOL |
                              DB_INIT_TXN | DB_PRIVATE | DB_THREAD | DB_CREATE,
                          0) == 0);
}

bdb_wrapper::~bdb_wrapper() { delete env; }

void *bdb_wrapper::new_txn(uint64_t txn_flags, str_arena &arena, void *buf,
                           TxnProfileHint hint) {
  DbTxn *txn = NULL;
...
void bdb_wrapper::abort_txn(void *p) {
  ALWAYS_ASSERT(((DbTxn *)p)->abort() == 0);
}

abstract_ordered_index *bdb_wrapper::open_index(const string &name,
...
bool bdb_ordered_index::get(void *txn, const string &key, string &value,
  ...
  value.assign((char *)vdbt.get_data(),
               min(static_cast<size_t>(vdbt.get_size()), max_bytes_read));
  return retno == 0;
}

const char *bdb_ordered_index::put(void *txn, const string &key,
                                   const string &value) {
  Dbt kdbt((void *)key.data(), key.size());
