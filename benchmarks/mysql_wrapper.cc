...
static inline void print_error_and_bail(MYSQL *conn) {
...
mysql_wrapper::mysql_wrapper(const string &dir, const string &db) : db(db) {
  struct stat st;
...
void mysql_wrapper::abort_txn(void *p) {
  ALWAYS_ASSERT(tl_conn == p);
...
abstract_ordered_index *mysql_wrapper::open_index(const string &name, size_t value_size_hint, bool mostly_append) {
  ...
  b_create << "CREATE TABLE IF NOT EXISTS " << name
           << " ("
              ...
              ") ENGINE=InnoDB;";
  b_truncate << "TRUNCATE TABLE " << name << ";";
...
void mysql_wrapper::close_index(abstract_ordered_index *idx) { delete idx; }

static inline string my_escape(MYSQL *conn, const char *p, size_t l) {
  char buf[2 * l + 1];
