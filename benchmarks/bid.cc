...
#define BID_REC_KEY_FIELDS(x, y) x(uint32_t, uid) y(uint32_t, bid)
#define BID_REC_VALUE_FIELDS(x, y) x(uint32_t, pid) y(float, amount)
DO_STRUCT(bid_rec, BID_REC_KEY_FIELDS, BID_REC_VALUE_FIELDS)

#define BIDMAX_REC_KEY_FIELDS(x, y) x(uint32_t, pid)
#define BIDMAX_REC_VALUE_FIELDS(x, y) x(float, amount)
...
public:
  bid_bench_runner(abstract_db *db) : bench_runner(db) {
...
void bid_do_test(abstract_db *db, int argc, char **argv) {
