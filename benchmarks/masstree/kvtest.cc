...
template <typename T> class kvtest_worker : public ndb_thread {
public:
  kvtest_worker(btree &btr, spin_barrier &b, unsigned int id,
                const volatile bool *phases)
      : ndb_thread(false, string("kvtest-worker")), btr(&btr), b(&b), id(id),
        phases(phases) {}
...
template <typename T> class kvtest_runner {
public:
...
struct kvtest_rw1 {
  ...
  void operator()(btree &btr, unsigned int id,
                  const volatile bool *phases) const {
    ...
    cout << "puts: " << n << ", rate: " << (double(n) / (put_ms / 1000.0))
         << " ops/sec/core" << endl;
    cout << "gets: " << g << ", rate: " << (double(g) / (get_ms / 1000.0))
         << " ops/sec/core" << endl;
