...
public:
  inline quick_istr() { set(0); }

  inline quick_istr(unsigned long x, int minlen = 0) { set(x, minlen); }

  inline void set(unsigned long x, int minlen = 0) {
  ...
  inline const char *data() const { return bbuf_; }

  inline size_t size() const { return (buf_ + sizeof(buf_) - 1) - bbuf_; }

  inline varkey key() const { return varkey((const uint8_t *) bbuf_, size()); }
};

template<typename T>
class kvtest_worker : public ndb_thread {
public:
  kvtest_worker(btree &btr, spin_barrier &b, unsigned int id, const volatile bool *phases)
...
class kvtest_runner {
public:
  kvtest_runner(unsigned int nthreads) : nthreads(nthreads) {}
  ...
  void go() {
    ...
    for (unsigned int i = 0; i < nthreads; i++) workers.push_back(new kvtest_worker<T>(btr, barrier, i, phases));
    for (unsigned int i = 0; i < nthreads; i++) workers[i]->start();
...
struct kvtest_rw1 {
  ...
  void operator()(btree &btr, unsigned int id, const volatile bool *phases) const {
    ...
    r.reset(seed);
    for (unsigned i = 0; i < n; i++) a[i] = (int32_t) r.next();
    for (unsigned i = 0; i < n; ++i) swap(a[i], a[r.next() % n]);
