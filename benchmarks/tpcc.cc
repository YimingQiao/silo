...
public:
    // ZSTD
    ZSTD<stock::value> *stock_zstd;
    ZSTD<stock_data::value> *stock_data_zstd;
    ZSTD<order_line::value> *order_line_zstd;
    ZSTD<oorder::value> *order_zstd;
    ZSTD<customer::value> *customer_zstd;
...
public:
    tpcc_stock_loader(unsigned long seed, abstract_db *db, const map<string, abstract_ordered_index *> &open_tables,
            ...
            : bench_loader(seed, db, open_tables),
              ...
              stock_zstd_(new ZSTD<stock::value>()),
              stock_data_zstd_(new ZSTD<stock_data::value>()) {
        ALWAYS_ASSERT(
    ...
    ZSTDTable<stock::value> tmp_db_stock_;
    ZSTD<stock::value> *stock_zstd_;
    ZSTDTable<stock_data::value> tmp_db_stock_data_;
    ZSTD<stock_data::value> *stock_data_zstd_;
...
protected:
    void BlitzTraining() {
        ...
        stock_zstd_->Train(tmp_db_stock_);
        stock_data_zstd_->Train(tmp_db_stock_data_);
...
public:
    tpcc_customer_loader(unsigned long seed, abstract_db *db, const map<string, abstract_ordered_index *> &open_tables,
            ...
            : bench_loader(seed, db, open_tables),
              tpcc_worker_mixin(partitions),
              warehouse_id(warehouse_id),
              customer_zstd_(new ZSTD<customer::value>()) {
        ALWAYS_ASSERT(
                warehouse_id == -1 || (warehouse_id >= 1 && static_cast<size_t>(warehouse_id) <= NumWarehouses()));
    }
    ...
    ZSTDTable<customer::value> tmp_db_customer_;
    ZSTD<customer::value> *customer_zstd_;
...
protected:
    void ZSTDTraining() {
        ...
        customer_zstd_->Train(tmp_db_customer_);
        avg_customer_sz =
...
public:
    tpcc_order_loader(unsigned long seed, abstract_db *db, const map<string, abstract_ordered_index *> &open_tables,
            ...
            : bench_loader(seed, db, open_tables),
              tpcc_worker_mixin(partitions),
              warehouse_id(warehouse_id),
              order_line_zstd_(new ZSTD<order_line::value>()),
              order_zstd_(new ZSTD<oorder::value>()) {
        ALWAYS_ASSERT(
                warehouse_id == -1 || (warehouse_id >= 1 && static_cast<size_t>(warehouse_id) <= NumWarehouses()));
    }
    ...
    ZSTDTable<order_line::value> tmp_db_order_line_;
    ZSTDTable<oorder::value> tmp_db_order_;
    ZSTD<order_line::value> *order_line_zstd_;
    ZSTD<oorder::value> *order_zstd_;
    ...
    virtual void load() {
        ...
        try {
            for (size_t i = 0; i < order_keys.size(); ++i) {
                ...
                order_total_sz += InsertOrder(txn, k_oo, tmp_db_order_.GetTuple(i), k_oo.o_w_id, obj_buf);
                n_order++;
            }

            for (size_t i = 0; i < order_line_keys.size(); ++i) {
                order_line::key &k_ol = order_line_keys[i];
                order_line_total_sz += InsertOrderLine(txn, k_ol, tmp_db_order_line_.GetTuple(i), k_ol.ol_w_id,
                                                       obj_buf1);
                n_order_lines++;
            }
            if (db->commit_txn(txn)) {
...
tpcc_worker::txn_result tpcc_worker::txn_delivery() {
    ...
    try {
        ...
        for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
            ...
            FindOrder(txn, k_oo, warehouse_id, order_buffer);
            ...
            for (size_t i = 0; i < c.size(); i++) {
                FindOrderLineInternal(*c.values[i].second, order_line_buffer, ol_tuple, order_line_zstd);
                ...
                InsertOrderLine(txn, *c.values[i].first, v_ol_new, warehouse_id);
            }
            ...
            customer_buffer.c_balance += ol_total;
            InsertCustomer(txn, k_c, customer_buffer, warehouse_id);
        }
...
tpcc_worker::txn_result tpcc_worker::txn_payment() {
    ...
    try {
        ...
        if (strncmp(v_c.c_credit.data(), "BC", 2) == 0) {
            char buf[501];
            int n = snprintf(buf, sizeof(buf), "%d %d %d %d %d %f | %s", k_c.c_id, k_c.c_d_id, k_c.c_w_id,
                             districtID,
...
public:
    ...
    virtual bool invoke(const char *keyp, size_t keylen, const string &value) {
        INVARIANT(keylen == sizeof(order_line::key));
        tpcc_worker::FindOrderLineInternal(value, v_ol, ol_tuple, order_line_zstd);
...
tpcc_worker::txn_result tpcc_worker::txn_stock_level() {
    ...
    try {
        ...
        c.order_line_zstd = order_line_zstd;
...
protected:
    virtual vector<bench_loader *> make_loaders() {
        ...
        // stock
        {
            ...
            stock_zstd = loader->stock_zstd_;
            stock_data_zstd = loader->stock_data_zstd_;
        }
        ...
        // customer
        {
            ...
            customer_zstd = loader->customer_zstd_;
        }
        ...
        // order, new order, and orderline
        {
            ...
            order_line_zstd = loader->order_line_zstd_;
            order_zstd = loader->order_zstd_;
        }
    ...
    vector<bench_worker *> make_workers() override {
        ...
        for (auto *worker: workers) {
            ...
            ((tpcc_worker *) worker)->stock_zstd = stock_zstd->Copy();
            ((tpcc_worker *) worker)->stock_data_zstd = stock_data_zstd->Copy();
            ((tpcc_worker *) worker)->order_line_zstd = order_line_zstd->Copy();
            ((tpcc_worker *) worker)->order_zstd = order_zstd->Copy();
            ((tpcc_worker *) worker)->customer_zstd = customer_zstd->Copy();
