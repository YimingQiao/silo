...
tpcc_worker::txn_result tpcc_worker::txn_order_status() {
  ...
  const abstract_db::TxnProfileHint hint =
      g_disable_read_only_scans ? abstract_db::HINT_TPCC_ORDER_STATUS : abstract_db::HINT_TPCC_ORDER_STATUS_READ_ONLY;
    ...
    if (g_order_status_scan_hack) {
      ...
      const oorder_c_id_idx::key k_oo_idx_1(warehouse_id, districtID, k_c.c_id, numeric_limits<int32_t>::max());
      {
        ...
        tbl_oorder_c_id_idx(warehouse_id)
            ->scan(txn, Encode(obj_key0, k_oo_idx_0), &Encode(obj_key1, k_oo_idx_1), c_oorder, s_arena.get());
      }
...
tpcc_worker::txn_result tpcc_worker::txn_stock_level() {
  ...
  const uint64_t read_only_mask = g_disable_read_only_scans ? 0 : transaction_base::TXN_FLAG_READ_ONLY;
  const abstract_db::TxnProfileHint hint =
      g_disable_read_only_scans ? abstract_db::HINT_TPCC_STOCK_LEVEL : abstract_db::HINT_TPCC_STOCK_LEVEL_READ_ONLY;
  void *txn = db->new_txn(txn_flags | read_only_mask, arena, txn_buf(), hint);
  scoped_str_arena s_arena(arena);
    ...
    if (g_enable_separate_tree_per_partition && !is_read_only) {
      if (NumWarehouses() <= nthreads) {
        ...
        for (size_t partid = 0; partid < nthreads; partid++) {
          ...
          abstract_ordered_index *idx = db->open_index(s_name + "_" + to_string(partid), expected_size, is_append_only);
