...
public:
  ...
  tpcc_worker(unsigned int worker_id, unsigned long seed, abstract_db *db,
      ...
      : bench_worker(worker_id, true, seed, db, open_tables, barrier_a,
                     barrier_b),
        tpcc_worker_mixin(partitions), warehouse_id_start(warehouse_id_start),
        ...
        for (uint batch = 0; batch < nbatches;) {
          ...
          try {
            for (uint cidx0 = cstart; cidx0 < cend; cidx0++) {
              ...
              tbl_history(w)->insert(txn, Encode(k_hist),
...
tpcc_worker::txn_result tpcc_worker::txn_new_order() {
  ...
  //   num_txn_contexts : 9
  void *txn = db->new_txn(txn_flags, arena, txn_buf(),
                          abstract_db::HINT_TPCC_NEW_ORDER);
  scoped_str_arena s_arena(arena);
  ...
  try {
    ...
    ALWAYS_ASSERT(
        tbl_customer(warehouse_id)->get(txn, Encode(obj_key0, k_c), obj_v));
    ...
    tbl_oorder(warehouse_id)
        ->insert(txn, Encode(str(), k_oo), Encode(str(), v_oo));
    ret += oorder_sz;
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      ALWAYS_ASSERT(
          tbl_stock(ol_supply_w_id)->get(txn, Encode(obj_key0, k_s), obj_v));
      ...
      tbl_stock(ol_supply_w_id)
          ->put(txn, Encode(str(), k_s), Encode(str(), v_s_new));
...
tpcc_worker::txn_result tpcc_worker::txn_delivery() {
  ...
  //   num_txn_contexts : 4
  void *txn =
      db->new_txn(txn_flags, arena, txn_buf(), abstract_db::HINT_TPCC_DELIVERY);
  scoped_str_arena s_arena(arena);
  ...
  try {
    ...
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      ...
      if (unlikely(!tbl_oorder(warehouse_id)
                        ->get(txn, Encode(obj_key0, k_oo), obj_v))) {
      ...
      tbl_oorder(warehouse_id)
          ->put(txn, Encode(str(), k_oo), Encode(str(), v_oo_new));
...
tpcc_worker::txn_result tpcc_worker::txn_payment() {
  ...
  //   num_txn_contexts : 5
  void *txn =
      db->new_txn(txn_flags, arena, txn_buf(), abstract_db::HINT_TPCC_PAYMENT);
  scoped_str_arena s_arena(arena);
  scoped_multilock<spinlock> mlock;
  if (g_enable_partition_locks) {
    mlock.enq(LockForPartition(warehouse_id));
  ...
  try {
    ...
    if (RandomNumber(r, 1, 100) <= 60) {
      ...
      ALWAYS_ASSERT(tbl_customer(customerWarehouseID)
                        ->get(txn, Encode(obj_key0, k_c), obj_v));
      Decode(obj_v, v_c);
    ...
    tbl_history(warehouse_id)
        ->insert(txn, Encode(str(), k_h), Encode(str(), v_h));
    ret += history_sz;
        ...
        INVARIANT(p.first >= 1 && p.first <= NumItems());
        {
          ...
          ALWAYS_ASSERT(
              tbl_stock(warehouse_id)
                  ->get(txn, Encode(obj_key0, k_s), obj_v, nbytesread));
        }
    ...
    if (g_enable_separate_tree_per_partition && !is_read_only) {
      if (NumWarehouses() <= nthreads) {
        for (size_t i = 0; i < NumWarehouses(); i++)
          ret[i] = db->open_index(s_name + "_" + to_string(i), expected_size,
                                  is_append_only);
      } else {
        const unsigned nwhse_per_partition = NumWarehouses() / nthreads;
      ...
      abstract_ordered_index *idx =
          db->open_index(s_name, expected_size, is_append_only);
      for (size_t i = 0; i < NumWarehouses(); i++)
        ret[i] = idx;
    }
    return ret;
    ...
    if (NumWarehouses() <= nthreads) {
      for (size_t i = 0; i < nthreads; i++)
        ret.push_back(new tpcc_worker(
            blockstart + i, r.next(), db, open_tables, partitions, &barrier_a,
      ...
      for (size_t i = 0; i < nthreads; i++) {
        ...
        ret.push_back(new tpcc_worker(blockstart + i, r.next(), db, open_tables,
