...
NumItems() { return 100000; }

static constexpr inline ALWAYS_INLINE size_t

NumDistrictsPerWarehouse() { return 10; }

static constexpr inline ALWAYS_INLINE size_t

NumCustomersPerDistrict() { return 3000; }
...
public:
    ...
    inline void multilock() {
        ALWAYS_ASSERT(!did_lock);
        if (locks.size() > 1) sort(locks.begin(), locks.end());
#ifdef CHECK_INVARIANTS
        if (set<T *>(locks.begin(), locks.end()).size() != locks.size()) {
          for (auto &t: locks) cerr << "lock: " << hexify(t) << endl;
          INVARIANT(false && "duplicate locks found");
        }
#endif
        for (auto &t: locks) t->lock();
        did_lock = true;
    }
...
static inline ALWAYS_INLINE unsigned int PartitionId(unsigned int wid) {
    ...
    if (partid >= nthreads) return nthreads - 1;
    return partid;
}
...
static inline atomic <uint64_t> &NewOrderIdHolder(unsigned warehouse, unsigned district) {
    INVARIANT(warehouse >= 1 && warehouse <= NumWarehouses());
    INVARIANT(district >= 1 && district <= NumDistrictsPerWarehouse());
    const unsigned idx = (warehouse - 1) * NumDistrictsPerWarehouse() + (district - 1);
    return g_district_ids[idx].elem;
}

static inline uint64_t FastNewOrderIdGen(unsigned warehouse, unsigned district) {
...
struct checker {
    ...
    static inline ALWAYS_INLINE void SanityCheckOOrder(const oorder::key *k, const oorder::value *v) {
...
struct _dummy {
};// exists so we can inherit from it, so we can use a macro in
// an init list...
...
#undef DEFN_TBL_ACCESSOR_X
    ...
    static void PinToWarehouseId(unsigned int wid) {
        ...
        if (verbose)
            cerr << "PinToWarehouseId(): coreid=" << coreid::core_id() << " pinned to whse=" << wid << " (partid="
                 << partid
                 << ")" << endl;
...
public:
    ...
    static inline ALWAYS_INLINE int NonUniformRandom(fast_random &r, int A, int C, int min, int max) {
    ...
    static inline ALWAYS_INLINE int GetCustomerId(fast_random &r) {
        return CheckBetweenInclusive(NonUniformRandom(r, 1023, 259, 1, NumCustomersPerDistrict()), 1,
                                     NumCustomersPerDistrict());
    }

    // pick a number between [start, end)
    static inline ALWAYS_INLINE unsigned PickWarehouseId(fast_random &r, unsigned start, unsigned end) {
        INVARIANT(start < end);
    ...
    static inline ALWAYS_INLINE size_t GetCustomerLastName(char *buf, fast_random &r, int num) {
    ...
    static inline ALWAYS_INLINE string GetNonUniformCustomerLastNameLoad(fast_random &r) {
        return GetCustomerLastName(r, NonUniformRandom(r, 255, 157, 0, 999));
    }

    static inline ALWAYS_INLINE size_t GetNonUniformCustomerLastNameRun(uint8_t *buf, fast_random &r) {
        return GetCustomerLastName(buf, r, NonUniformRandom(r, 255, 223, 0, 999));
    }

    static inline ALWAYS_INLINE size_t GetNonUniformCustomerLastNameRun(char *buf, fast_random &r) {
        return GetNonUniformCustomerLastNameRun((uint8_t *) buf, r);
    }

    static inline ALWAYS_INLINE string GetNonUniformCustomerLastNameRun(fast_random &r) {
...
string tpcc_worker_mixin::NameTokens[] = {
        string("BAR"), string("OUGHT"), string("ABLE"), string("PRI"), string("PRES"),
        string("ESE"), string("ANTI"), string("CALLY"), string("ATION"), string("EING"),
};
...
public:
    ...
    tpcc_worker(unsigned int worker_id, unsigned long seed, abstract_db *db,
                ...
                const map <string, vector<abstract_ordered_index *>> &partitions, spin_barrier *barrier_a,
                spin_barrier *barrier_b, uint warehouse_id_start, uint warehouse_id_end)
...
public:
    ...
    InsertOrder(void *txn, const oorder::key &k, const oorder::value &v, size_t warehouse_id, bool update = true) {
        ...
        if (in_mem) {
            o_tuple.Set(codes);
            if (update) tbl_oorder(warehouse_id)->put(txn, Encode(str(), k), Serialize(str(), o_tuple));
            else {
                stat.Insert(codes.size(), true, "order");
                tbl_oorder(warehouse_id)->insert(txn, Encode(str(), k), Serialize(str(), o_tuple));
            }
        }
        return codes.size();
        // bool in_mem = stat.ToMemory(codes.size());
...
//            o_tuple.Set(v);
//            if (update)
//                tbl_oorder(warehouse_id)->put(txn, Encode(str(), k), Tuple<oorder::value>::Serialize(str(), o_tuple));
//            else {
//                stat.Insert(sz, true, "order");
//                tbl_oorder(warehouse_id)->insert(txn, Encode(str(), k),
//                                                 Tuple<oorder::value>::Serialize(str(), o_tuple));
//            }
//        }
//        return sz;
    ...
    inline ALWAYS_INLINE bool FindOrder(void *txn, const oorder::key &k, size_t warehouse_id, oorder::value &v) {
        bool success = tbl_oorder(warehouse_id)->get(txn, Encode(obj_key0, k), obj_v);
        if (likely(success)) {
            Deserialize(obj_v, o_tuple);
            order_zstd->ZstdDecompress(o_tuple.data_, &v);
//            Tuple<oorder::value>::Deserialize(obj_v, o_tuple);
//            v = o_tuple.data_;
        }
        return success;
    }
    ...
    inline ALWAYS_INLINE bool FindHistory(void *txn, const history::key &k, size_t warehouse_id, history::value &v) {
    ...
    InsertOrderLine(void *txn, const std::string &k_encoded, const order_line::value &v, size_t w_id,
                    bool update = true) {
        std::string codes = order_line_zstd->ZstdCompress(v);
        bool in_mem = stat.ToMemory(codes.size());
        if (likely(in_mem)) {
    ...
    inline ALWAYS_INLINE bool FindOrderLine(void *txn, const order_line::key &k, size_t w_id, order_line::value &v) {
        ...
        if (success) {
            FindOrderLineInternal(obj_v, v, ol_tuple, order_line_zstd);
    ...
    FindOrderLineInternal(const std::string &obj, order_line::value &v, Tuple <std::string> &tuple,
    ...
    inline ALWAYS_INLINE size_t InsertStock(void *txn, const stock::key &k, const stock::value &v, size_t w_id) {
        std::string codes = stock_zstd->ZstdCompress(v);
//        bool in_mem = stat.ToMemory(codes.size());
        bool in_mem = true;
        if (likely(in_mem)) {
            s_tuple.Set(codes);
            tbl_stock(w_id)->put(txn, Encode(str(), k), Serialize(str(), s_tuple));
        } else {
            FileDescriptor &s_disk = FileManager::GetInstance().GetDescriptor(worker_id, 1);
            s_disk.SeqDiskTupleWrite(&v);

            s_tuple.Set(codes, false, stat.n_s_disk++, worker_id);
            tbl_stock(w_id)->put(txn, Encode(str(), k), Serialize(str(), s_tuple));
        }
        return codes.size();
    }

    inline ALWAYS_INLINE bool FindStock(void *txn, const stock::key &k, size_t w_id, stock::value &v) {
    ...
    inline ALWAYS_INLINE bool FindStock(void *txn, const stock::key &k, size_t w_id, std::string &v_data,
    ...
    InsertCustomer(void *txn, const customer::key &k, const customer::value &v, size_t w_id) {
        std::string codes = customer_zstd->ZstdCompress(v);
//        bool in_mem = stat.ToMemory(codes.size());
        bool in_mem = true;
        if (likely(in_mem)) {
            ...
            c_tuple.Set(codes, false, stat.n_c_disk++, worker_id);
            std::string &serial_str = Serialize(str(), c_tuple);
            tbl_customer(w_id)->put(txn, Encode(str(), k), serial_str);
        }
        return codes.size();
    ...
    inline ALWAYS_INLINE bool FindCustomer(void *txn, const customer::key &k, size_t w_id, customer::value &v) {
...
protected:
    ...
    using vector_tuple = std::vector<tuple_raman::value>;

    inline ALWAYS_INLINE vector_tuple
    RamanInsertOrderBlock(const oorder::key &k, oorder::value &v) {
        ...
        if (block.Insert(k, v)) {
            ...
            for (size_t i = 0; i < block.n_tuple; ++i) {
                ...
                tuple.data = std::move(compressed_codes[i]);
    ...
    inline ALWAYS_INLINE vector_tuple RamanInsertOrderLineBlock(const order_line::key &k, order_line::value &v) {
    ...
    inline ALWAYS_INLINE vector_tuple
    RamanInsertStockBlock(const stock::key &k, stock::value &v) {
    ...
    inline ALWAYS_INLINE vector_tuple
    RamanInsertCustomerBlock(const customer::key &k, customer::value &v) {
...
private:
    ...
    Tuple <string> ol_tuple;
    Tuple <string> c_tuple;
    Tuple <new_order::value> no_tuple;
    // Tuple<oorder::value> o_tuple;
    Tuple <string> o_tuple;
...
public:
    tpcc_warehouse_loader(unsigned long seed, abstract_db *db, const map<string, abstract_ordered_index *> &open_tables,
...
public:
    tpcc_item_loader(unsigned long seed, abstract_db *db, const map<string, abstract_ordered_index *> &open_tables,
                     const map <string, vector<abstract_ordered_index *>> &partitions)
            : bench_loader(seed, db, open_tables),
              tpcc_worker_mixin(partitions) {}
...
public:
    tpcc_stock_loader(unsigned long seed, abstract_db *db, const map<string, abstract_ordered_index *> &open_tables,
    ...
    Tuple <std::string> s_tuple;
    Tuple <std::string> s_data_tuple;
    ...
    InsertStock(void *txn, const stock::key &k, const stock::value &v, size_t warehouse_id, string obj_buf) {
        s_tuple.Set(stock_zstd_->ZstdCompress(v));
        tbl_stock(warehouse_id)->insert(txn, Encode(k), Serialize(obj_buf, s_tuple));
        return s_tuple.data_.size();
    }

    inline ALWAYS_INLINE size_t
    InsertStockData(void *txn, const stock_data::key &k, const stock_data::value &v, size_t warehouse_id,
                    string obj_buf1) {
        s_data_tuple.Set(stock_data_zstd_->ZstdCompress(v));
        tbl_stock_data(warehouse_id)->insert(txn, Encode(k), Serialize(obj_buf1, s_data_tuple));
        return s_data_tuple.data_.size();
    }
            ...
            for (uint b = 0; b < nbatches;) {
                ...
                try {
                    ...
                    for (uint i = (b * batchsize + 1); i <= iend; i++) {
                        ...
                        tbl_stock(w)->insert(txn, Encode(k), Encode(obj_buf, CreateTuple(v_codes, 0, true)));
                        tbl_stock_data(w)->insert(txn, Encode(k_data),
                                                  Encode(obj_buf1, CreateTuple(v_data_codes, 0, true)));
                    }
...
public:
    tpcc_district_loader(unsigned long seed, abstract_db *db, const map<string, abstract_ordered_index *> &open_tables,
                         const map <string, vector<abstract_ordered_index *>> &partitions)
            : bench_loader(seed, db, open_tables),
              tpcc_worker_mixin(partitions) {}
...
public:
    tpcc_customer_loader(unsigned long seed, abstract_db *db, const map<string, abstract_ordered_index *> &open_tables,
                         const map <string, vector<abstract_ordered_index *>> &partitions, ssize_t warehouse_id)
            : bench_loader(seed, db, open_tables),
              tpcc_worker_mixin(partitions),
              warehouse_id(warehouse_id),
    ...
    InsertCustomer(void *txn, const customer::key &k, const customer::value &v, size_t warehouse_id, string obj_buf) {
        std::string str;
        c_tuple.Set(customer_zstd_->ZstdCompress(v));
        tbl_customer(warehouse_id)->insert(txn, Encode(k), Serialize(obj_buf, c_tuple));
        return c_tuple.data_.size();
    }
    ...
    void load() override {
        ...
        try {
            for (size_t i = 0; i < customer_keys.size(); ++i) {
                ...
                tbl_customer(k.c_w_id)->insert(txn, Encode(k), Encode(obj_buf, CreateTuple(v_codes, 0, true)));
            }
            if (db->commit_txn(txn)) b++;
            else {
                db->abort_txn(txn);
...
public:
    tpcc_order_loader(unsigned long seed, abstract_db *db, const map<string, abstract_ordered_index *> &open_tables,
                      const map <string, vector<abstract_ordered_index *>> &partitions, ssize_t warehouse_id)
            : bench_loader(seed, db, open_tables),
              tpcc_worker_mixin(partitions),
              warehouse_id(warehouse_id),
    ...
    Tuple <std::string> ol_tuple;
    Tuple <std::string> o_tuple;
    // Tuple<oorder::value> o_tuple;
    //
    ...
    InsertOrderLine(void *txn, const order_line::key &k, const order_line::value &v, size_t warehouse_id,
                    string obj_buf) {
        ol_tuple.Set(order_line_zstd_->ZstdCompress(v));
        tbl_order_line(warehouse_id)->insert(txn, Encode(k), Serialize(obj_buf, ol_tuple));
        return ol_tuple.data_.size();
    }

    // Zstandard has very weak compression ability for short tuples like oorder::value. So we do not compress it,
    // along with other tables which has the short tuple.
    inline ALWAYS_INLINE size_t
    InsertOrder(void *txn, const oorder::key &k, const oorder::value &v, size_t warehouse_id, string obj_buf) {
        o_tuple.Set(order_zstd_->ZstdCompress(v));
        tbl_oorder(warehouse_id)->put(txn, Encode(k), Serialize(obj_buf, o_tuple));
        return o_tuple.data_.size();
//        o_tuple.Set(v);
//        tbl_oorder(warehouse_id)->insert(txn, Encode(k), Tuple<oorder::value>::Serialize(obj_buf, o_tuple));
//        return Size(v);
    }
...
protected:
    void CprTraining() {
            ...
            for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
                ...
                for (uint c = 1; c <= NumCustomersPerDistrict();) {
                    ...
                    try {
                        ...
                        if (k_oo.o_id < 2101) v_oo.o_carrier_id = RandomNumber(r, 1, 10);
                        else
                            v_oo.o_carrier_id = 0;
        ...
        order_line_zstd_->Train(tmp_db_order_line_);
        order_zstd_->Train(tmp_db_order_);
    ...
    void load() override {
        ...
        try {
            for (size_t i = 0; i < order_keys.size(); ++i) {
                ...
                tbl_oorder(k_oo.o_w_id)->insert(txn, Encode(k_oo), Encode(obj_buf, CreateTuple(v_codes, 0, true)));
            }
            ...
            for (size_t i = 0; i < order_line_keys.size(); ++i) {
                ...
                tbl_order_line(k_ol.ol_w_id)->insert(txn, Encode(k_ol), Encode(obj_buf, CreateTuple(v_codes, 0, true)));
            }
            if (db->commit_txn(txn)) {
                b++;
            } else {
                db->abort_txn(txn);
                ALWAYS_ASSERT(warehouse_id != -1);
                cerr << "[WARNING] order line loader loading abort" << endl;
            }
        } catch (abstract_db::abstract_abort_exception &ex) {
            db->abort_txn(txn);
            ALWAYS_ASSERT(warehouse_id != -1);
...
tpcc_worker::txn_result tpcc_worker::txn_new_order() {
    ...
    if (!allLocal) ++evt_tpcc_cross_partition_new_order_txns;
    ...
    try {
        ...
        const uint64_t my_next_o_id =
                g_new_order_fast_id_gen ? FastNewOrderIdGen(warehouse_id, districtID) : v_d->d_next_o_id;
        ...
        auto tuple_oo = CreateTuple(Encode(obj_tuple, v_oo), order_block.CprNum(), false, order_block.id);
        tbl_oorder(warehouse_id)->insert(txn, Encode(str(), k_oo), Encode(str(), tuple_oo));
        auto tuples = RamanInsertOrderBlock(k_oo, v_oo);
//        if (!tuples.empty()) {
...
//                tbl_oorder(w_id)->put(txn, Encode(str(), key), Encode(str(), tuple));
//            }
//        }
        ...
        for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
            ...
            auto tuple_s = CreateTuple(Encode(obj_tuple, v_s), stock_block.CprNum(), false, stock_block.id);
            tbl_stock(ol_supply_w_id)->put(txn, Encode(str(), k_s), Encode(str(), tuple_s));
            tuples = RamanInsertStockBlock(k_s, v_s);
//            if (!tuples.empty()) {
//                size_t n = tuples.size();
//                for (size_t i = 0; i < n; ++i) {
...
//                    auto &tuple = tuples[i];
//                    uint32_t w_id = GetWarehouseID(key);
//                    tbl_stock(w_id)->put(txn, Encode(str(), key), Encode(str(), tuple));
//                }
//            }
            ...
            auto tuple_ol = CreateTuple(Encode(obj_tuple, v_ol), order_line_block.CprNum(), false, order_line_block.id);
            tbl_order_line(warehouse_id)->insert(txn, Encode(str(), k_ol), Encode(str(), tuple_ol));
            tuples = RamanInsertOrderLineBlock(k_ol, v_ol);
//            if (!tuples.empty()) {
//                size_t n = tuples.size();
//                for (size_t i = 0; i < n; ++i) {
...
//                    auto &tuple = tuples[i];
//                    uint32_t w_id = GetWarehouseID(key);
//                    tbl_order_line(w_id)->put(txn, Encode(str(), key), Encode(str(), tuple));
//                }
//            }
        }
...
tpcc_worker::txn_result tpcc_worker::txn_delivery() {
    ...
    scoped_str_arena s_arena(arena);
    scoped_lock_guard<spinlock> slock(g_enable_partition_locks ? &LockForPartition(warehouse_id) : nullptr);
    try {
        ssize_t ret = 0;
        for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
            ...
            new_order_scan_callback new_order_c;
            {
                ...
                tbl_new_order(warehouse_id)
                        ->scan(txn, Encode(obj_key0, k_no_0), &Encode(obj_key1, k_no_1), new_order_c,
                               s_arena.get());
            }
            ...
            if (unlikely(!FindOrder(txn, k_oo, warehouse_id, order_buffer))) {
            ...
            auto tuple_o = DecodeTuple(obj_v);
            order_block.Decompress(tuple_o, order_buffer);
            ...
            for (size_t i = 0; i < c.size(); i++) {
                auto tuple_ol = DecodeTuple(*c.values[i].second);
                order_line_block.Decompress(tuple_ol, order_line_buffer);
                const order_line::value *v_ol = &order_line_buffer;
                sum += v_ol->ol_amount;
                ...
                order_line::key k_ol;
                Decode(*c.values[i].first, k_ol);

                tuple_ol = CreateTuple(Encode(obj_tuple, v_ol_new), order_line_block.CprNum(), false,
                                       order_line_block.id);
                tbl_order_line(warehouse_id)->insert(txn, Encode(str(), k_ol), Encode(str(), tuple_ol));
                auto tuples = RamanInsertOrderLineBlock(k_ol, v_ol_new);
//                if (!tuples.empty()) {
//                    size_t n = tuples.size();
//                    for (size_t i = 0; i < n; ++i) {
//                        auto &key = order_line_block.keys_[i];
...
//                }
            ...
            auto tuple_oo = CreateTuple(Encode(obj_tuple, v_oo_new), order_block.CprNum(), false, order_block.id);
            tbl_oorder(warehouse_id)->insert(txn, Encode(str(), k_oo), Encode(str(), tuple_oo));
            auto tuples = RamanInsertOrderBlock(k_oo, v_oo_new);
//            if (!tuples.empty()) {
//                size_t n = tuples.size();
//                for (size_t i = 0; i < n; ++i) {
//                    auto &key = order_block.keys_[i];
...
//            }
            ...
            ALWAYS_ASSERT(tbl_customer(warehouse_id)->get(txn, Encode(obj_key0, k_c), obj_v));
            auto tuple = DecodeTuple(obj_v);
            customer_block.Decompress(tuple, customer_buffer);
            customer::value &v_c = customer_buffer;
            v_c.c_balance += ol_total;

            auto tuple_c = CreateTuple(Encode(obj_tuple, v_c), customer_block.CprNum(), false, customer_block.id);
            tbl_customer(warehouse_id)->put(txn, Encode(str(), k_c), Encode(str(), tuple_c));
            tuples = RamanInsertCustomerBlock(k_c, v_c);
//            if (!tuples.empty()) {
//                size_t n = tuples.size();
//                for (size_t i = 0; i < n; ++i) {
...
//                    auto &tuple = tuples[i];
//                    uint32_t w_id = GetWarehouseID(key);
//                    tbl_customer(w_id)->put(txn, Encode(str(), key), Encode(str(), tuple));
//                }
//            }
        }
...
tpcc_worker::txn_result tpcc_worker::txn_payment() {
    ...
    if (likely(g_disable_xpartition_txn || NumWarehouses() == 1 || RandomNumber(r, 1, 100) <= 85)) {
        ...
        do { customerWarehouseID = RandomNumber(r, 1, NumWarehouses()); }
    ...
    if (customerWarehouseID != warehouse_id) ++evt_tpcc_cross_partition_payment_txns;
    try {
        ssize_t ret = 0;
        ...
        if (RandomNumber(r, 1, 100) <= 60) {
            ...
            tbl_customer_name_idx(customerWarehouseID)
                    ->scan(txn, Encode(obj_key0, k_c_idx_0), &Encode(obj_key1, k_c_idx_1), c, s_arena.get());
        ...
        auto tuple_c = CreateTuple(Encode(obj_tuple, v_c_new), customer_block.CprNum(), false, customer_block.id);
        tbl_customer(warehouse_id)->put(txn, Encode(str(), k_c), Encode(str(), tuple_c));
        auto tuples = RamanInsertCustomerBlock(k_c, v_c_new);
//        if (!tuples.empty()) {
//            size_t n = tuples.size();
//            for (size_t i = 0; i < n; ++i) {
//                auto &key = customer_block.keys_[i];
...
//        }
        ...
        int n = snprintf((char *) v_h.h_data.data(), v_h.h_data.max_size() + 1, "%.10s    %.10s",
                         v_w->w_name.c_str(),
                         v_d->d_name.c_str());
...
tpcc_worker::txn_result tpcc_worker::txn_order_status() {
    ...
    //   num_txn_contexts : 4
    const uint64_t read_only_mask = g_disable_read_only_scans ? 0 : transaction_base::TXN_FLAG_READ_ONLY;
    ...
    try {
        ...
        if (RandomNumber(r, 1, 100) <= 60) {
            ...
            k_c.c_id = v_c_idx->c_id;
            ALWAYS_ASSERT(tbl_customer(warehouse_id)->get(txn, Encode(obj_key0, k_c), obj_v));
            auto tuple_c = DecodeTuple(obj_v);
            customer_block.Decompress(tuple_c, customer_buffer);
            v_c = &customer_buffer;
        } else {
            // cust by ID
            const uint customerID = GetCustomerId(r);
        ...
        if (g_order_status_scan_hack) {
                                                  ...
                                                  numeric_limits<int32_t>::max());
            {
                ...
                tbl_oorder_c_id_idx(warehouse_id)
                        ->scan(txn, Encode(obj_key0, k_oo_idx_0), &Encode(obj_key1, k_oo_idx_1), c_oorder,
                               s_arena.get());
            }
...
tpcc_worker::txn_result tpcc_worker::txn_stock_level() {
    ...
    try {
        ...
        const order_line::key k_ol_1(warehouse_id, districtID, cur_next_o_id, 0);
        {
            ...
            tbl_order_line(warehouse_id)->scan(txn, Encode(obj_key0, k_ol_0), &Encode(obj_key1, k_ol_1), c,
                                               s_arena.get());
        }
        {
...
template<typename T>
static vector <T> unique_filter(const vector <T> &v) {
    set <T> seen;
    vector <T> ret;
    for (auto &e: v)
        if (!seen.count(e)) {
...
private:
    static bool IsTableReadOnly(const char *name) { return strcmp("item", name) == 0; }

    static bool IsTableAppendOnly(const char *name) {
        return strcmp("history", name) == 0 || strcmp("oorder_c_id_idx", name) == 0;
    }

    static vector<abstract_ordered_index *> OpenTablesForTablespace(abstract_db *db, const char *name,
        ...
        if (g_enable_separate_tree_per_partition && !is_read_only) {
            if (NumWarehouses() <= nthreads) {
                ...
                for (size_t partid = 0; partid < nthreads; partid++) {
                    ...
                    const unsigned wend = (partid + 1 == nthreads) ? NumWarehouses() : (partid + 1) *
                                                                                       nwhse_per_partition;
                    abstract_ordered_index *idx = db->open_index(s_name + "_" + to_string(partid), expected_size,
                                                                 is_append_only);
...
public:
    tpcc_bench_runner(abstract_db *db) : bench_runner(db) {

#define OPEN_TABLESPACE_X(x) partitions[#x] = OpenTablesForTablespace(db, #x, sizeof(x));
...
#undef OPEN_TABLESPACE_X
        ...
        for (auto &t: partitions) {
            ...
            for (size_t i = 0; i < v.size(); i++) open_tables[t.first + "_" + to_string(i)] = v[i];
        }
        ...
        if (g_new_order_fast_id_gen) {
            ...
            CACHELINE_SIZE, sizeof(aligned_padded_elem<atomic < uint64_t>>) *NumWarehouses() *
...
protected:
    ...
    vector<bench_worker *> make_workers() override {
        ...
        if (NumWarehouses() <= nthreads) {
            for (size_t i = 0; i < nthreads; i++)
                workers.push_back(
                        new tpcc_worker(blockstart + i, r.next(), db, open_tables, partitions, &barrier_a,
                                        &barrier_b,
