...
static constexpr inline ALWAYS_INLINE size_t NumItems() {
    return 100000;
}
...
public:
    ...
    tpcc_worker(unsigned int worker_id, unsigned long seed, abstract_db *db,
                ...
                uint warehouse_id_start, uint warehouse_id_end) : bench_worker(worker_id, true, seed, db,
...
protected:
    ...
    inline ALWAYS_INLINE string &str() {
        return *arena.next();
    }
        ...
        if (verbose) {
            if (warehouse_id == -1) {
                ...
                cerr << "[INFO]   * average customer record length: "
                     << (double(total_sz) /
                         double(NumWarehouses() * NumDistrictsPerWarehouse() * NumCustomersPerDistrict()))
                     << " bytes " << endl;
            } else {
...
public:
    ...
    inline const new_order::key *get_key() const {
        return k_no;
    }
...
tpcc_worker::txn_result tpcc_worker::txn_payment() {
    ...
    try {
        ...
        if (RandomNumber(r, 1, 100) <= 60) {
            ...
            tbl_customer_name_idx(customerWarehouseID)->scan(txn, Encode(obj_key0, k_c_idx_0),
...
tpcc_worker::txn_result tpcc_worker::txn_order_status() {
    ...
    try {
        ...
        if (RandomNumber(r, 1, 100) <= 60) {
            ...
            static_limit_callback<NMaxCustomerIdxScanElems> c(s_arena.get(), true); // probably a safe bet for now
            tbl_customer_name_idx(warehouse_id)->scan(txn, Encode(obj_key0, k_c_idx_0), &Encode(obj_key1, k_c_idx_1), c,
                                                      s_arena.get());
            ALWAYS_ASSERT(c.size() > 0);
            INVARIANT(c.size() < NMaxCustomerIdxScanElems); // we should detect this
            int index = c.size() / 2;
        ...
        if (g_order_status_scan_hack) {
            ...
            const oorder_c_id_idx::key k_oo_idx_1(warehouse_id, districtID, k_c.c_id, numeric_limits<int32_t>::max());
            {
                ...
                tbl_oorder_c_id_idx(warehouse_id)->scan(txn, Encode(obj_key0, k_oo_idx_0),
        ...
        tbl_order_line(warehouse_id)->scan(txn, Encode(obj_key0, k_ol_0), &Encode(obj_key1, k_ol_1), c_order_line,
                                           s_arena.get());
...
#undef OPEN_TABLESPACE_X
        ...
        if (g_new_order_fast_id_gen) {
            void *const px = memalign(CACHELINE_SIZE,
            ...
            for (size_t i = 0; i < nthreads; i++)
                ret.push_back(
                        new tpcc_worker(blockstart + i, r.next(), db, open_tables, partitions, &barrier_a,
                                        &barrier_b, (i % NumWarehouses()) + 1, (i % NumWarehouses()) + 2));
        } else {
            const unsigned nwhse_per_partition = NumWarehouses() / nthreads;
            for (size_t i = 0; i < nthreads; i++) {
