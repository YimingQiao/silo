...
void transaction<Protocol, Traits>::cleanup_inserted_tuple_marker(
  ...
  if (unlikely(!did_remove)) {
    ...
    std::cerr << " *** could not remove key: " << util::hexify(key)
              << std::endl;
#ifdef TUPLE_CHECK_KEY
    std::cerr << " *** original key        : " << util::hexify(marker->key)
              << std::endl;
#endif
#endif
    ALWAYS_ASSERT(false);
...
bool transaction<Protocol, Traits>::commit(bool doThrow) {
  ...
  try {
    ...
    PERF_DECL(static std::string probe0_name(std::string(__PRETTY_FUNCTION__) +
                                             std::string(":total:")));
    ...
    if (!write_set.empty()) {
      PERF_DECL(
          static std::string probe1_name(std::string(__PRETTY_FUNCTION__) +
                                         std::string(":lock_write_nodes:")));
      ...
      for (size_t pos = 0; it != it_end; ++it, ++pos) {
        ...
        write_dbtuples.emplace_back(it->get_tuple(), &(*it), it->is_insert(),
                                    pos);
      }
    }
    ...
    if (!is_snapshot()) {
      ...
      if (!write_dbtuples.empty()) {
        PERF_DECL(
            static std::string probe2_name(std::string(__PRETTY_FUNCTION__) +
                                           std::string(":lock_write_nodes:")));
        ANON_REGION(probe2_name.c_str(),
                    &transaction_base::g_txn_commit_probe2_cg);
        // lock the logical nodes in sort order
        {
          PERF_DECL(static std::string probe6_name(
              std::string(__PRETTY_FUNCTION__) +
              std::string(":sort_write_nodes:")));
        ...
        for (; it != it_end; last_px = &(*it), ++it) {
          if (likely(last_px && last_px->tuple != it->tuple)) {
            // on boundary
            if (unlikely(
                    !handle_last_tuple_in_group(*last_px, inserted_last_run))) {
        ...
        if (likely(last_px) && unlikely(!handle_last_tuple_in_group(
                                   *last_px, inserted_last_run))) {
          abort_trap((reason = ABORT_REASON_WRITE_NODE_INTERFERENCE));
          goto do_abort;
        }
        commit_tid.first = true;
        PERF_DECL(
            static std::string probe5_name(std::string(__PRETTY_FUNCTION__) +
                                           std::string(":gen_commit_tid:")));
        ANON_REGION(probe5_name.c_str(),
                    &transaction_base::g_txn_commit_probe5_cg);
        commit_tid.second = cast()->gen_commit_tid(write_dbtuples);
        VERBOSE(std::cerr << "commit tid: "
                          << g_proto_version_str(commit_tid.second)
                          << std::endl);
      } else {
      ...
      {
        PERF_DECL(
            static std::string probe3_name(std::string(__PRETTY_FUNCTION__) +
                                           std::string(":read_validation:")));
        ...
        if (!read_set.empty()) {
          ...
          for (; it != it_end; ++it) {
            VERBOSE(std::cerr << "validating dbtuple "
                              << util::hexify(it->get_tuple())
                              << " at snapshot_tid "
                              << g_proto_version_str(cast()->snapshot_tid())
                              << std::endl);
            const bool found =
                sorted_dbtuples_contains(write_dbtuples, it->get_tuple());
            ...
            VERBOSE(std::cerr
                    << "validating dbtuple " << util::hexify(it->get_tuple())
                    << " at snapshot_tid "
                    << g_proto_version_str(cast()->snapshot_tid()) << " FAILED"
                    << std::endl
                    << "  txn read version: "
                    << g_proto_version_str(it->get_tid()) << std::endl
        ...
        if (!absent_set.empty()) {
          ...
          for (; it != it_end; ++it) {
            const uint64_t v =
                concurrent_btree::ExtractVersionNumber(it->first);
            if (unlikely(v != it->second.version)) {
              VERBOSE(std::cerr << "expected node " << util::hexify(it->first)
                                << " at v=" << it->second.version
                                << ", got v=" << v << std::endl);
              abort_trap(
                  (reason = ABORT_REASON_NODE_SCAN_READ_VERSION_CHANGED));
              goto do_abort;
            }
          }
        }
      ...
      if (!write_dbtuples.empty()) {
        PERF_DECL(static std::string probe4_name(
        ...
        for (; it != it_end; ++it) {
          ...
          if (it->is_insert()) {
            ...
            tuple->version =
                commit_tid.second; // allows write_record_ret() to succeed
            ...
            if (unlikely(ret.head_ != tuple)) {
              ...
              if (it->get_btree()->insert(
                      varkey(it->get_key()),
                      (typename concurrent_btree::value_type)ret.head_, &old_v,
                      NULL))
          ...
          VERBOSE(std::cerr << "dbtuple " << util::hexify(tuple)
                            << " is_locked? " << tuple->is_locked()
                            << std::endl);
        }
        // unlock
  ...
  do_abort:
    ...
    if (this->is_snapshot())
      VERBOSE(std::cerr << "aborting txn @ snapshot_tid "
                        << cast()->snapshot_tid() << std::endl);
    else
...
std::pair<dbtuple *, bool> transaction<Protocol, Traits>::try_insert_new_tuple(
  ...
  INVARIANT(key);
  const size_t sz =
      value ? writer(dbtuple::TUPLE_WRITER_COMPUTE_NEEDED, value, nullptr, 0)
            : 0;
  ...
  if (unlikely(!btr.insert_if_absent(
          varkey(*key), (typename concurrent_btree::value_type)tuple,
          &insert_info))) {
    VERBOSE(std::cerr << "insert_if_absent failed for key: "
                      << util::hexify(key) << std::endl);
    tuple->clear_latest();
    tuple->unlock();
  ...
  VERBOSE(std::cerr << "insert_if_absent suceeded for key: "
                    << util::hexify(key) << std::endl
  ...
  if (!absent_set.empty()) {
    ...
    if (it != absent_set.end()) {
      ...
      VERBOSE(std::cerr << "bump node=" << util::hexify(it->first)
                        << " from v=" << insert_info.old_version
...
bool transaction<Protocol, Traits>::do_tuple_read(const dbtuple *tuple,
  ...
  const transaction_base::tid_t snapshot_tid =
      is_snapshot_txn ? cast()->snapshot_tid()
