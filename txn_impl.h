...
template<template<typename> class Protocol, typename Traits>
void transaction<Protocol, Traits>::cleanup_inserted_tuple_marker(dbtuple *marker, const std::string &key,
...
namespace {
  inline const char *transaction_state_to_cstr(transaction_base::txn_state state) {
    switch (state) {
  ...
  inline std::string transaction_flags_to_str(uint64_t flags) {
    ...
    if (flags & transaction_base::TXN_FLAG_READ_ONLY) {
      if (first) oss << "TXN_FLAG_READ_ONLY";
      else
...
void transaction<Protocol, Traits>::dump_debug_info() const {
  std::cerr << "Transaction (obj=" << util::hexify(this) << ") -- state " << transaction_state_to_cstr(state)
            << std::endl;
  ...
  // read-set
  for (typename read_set_map::const_iterator rs_it = read_set.begin(); rs_it != read_set.end(); ++rs_it)
    std::cerr << *rs_it << std::endl;

  std::cerr << "      === Write Set ===" << std::endl;
  // write-set
  for (typename write_set_map::const_iterator ws_it = write_set.begin(); ws_it != write_set.end(); ++ws_it)
    std::cerr << *ws_it << std::endl;

  std::cerr << "      === Absent Set ===" << std::endl;
  // absent-set
  for (typename absent_set_map::const_iterator as_it = absent_set.begin(); as_it != absent_set.end(); ++as_it)
...
template<template<typename> class Protocol, typename Traits>
bool transaction<Protocol, Traits>::handle_last_tuple_in_group(dbtuple_write_info &last, bool did_group_insert) {
  if (did_group_insert) {
    // don't need to lock
    if (!last.is_insert())
    ...
    last.mark_locked();
    if (unlikely(!dbtuple::IsLatest(v) || !cast()->can_read_tid(tuple->version))) {
  ...
  try {
    ...
    switch (state) {
      ...
      case TXN_COMMITED:
        return true;
      case TXN_ABRT:
        if (doThrow) throw transaction_abort_exception(reason);
        return false;
    }
    ...
    if (!write_set.empty()) {
      PERF_DECL(static std::string probe1_name(std::string(__PRETTY_FUNCTION__) + std::string(":lock_write_nodes:")));
    ...
    if (!is_snapshot()) {
      ...
      if (!write_dbtuples.empty()) {
        ...
        {
          PERF_DECL(
              static std::string probe6_name(std::string(__PRETTY_FUNCTION__) + std::string(":sort_write_nodes:")));
        ...
        if (likely(last_px) && unlikely(!handle_last_tuple_in_group(*last_px, inserted_last_run))) {
          abort_trap((reason = ABORT_REASON_WRITE_NODE_INTERFERENCE));
          goto do_abort;
        }
        commit_tid.first = true;
        PERF_DECL(static std::string probe5_name(std::string(__PRETTY_FUNCTION__) + std::string(":gen_commit_tid:")));
        ...
        if (!read_set.empty()) {
          ...
          for (; it != it_end; ++it) {
            VERBOSE(std::cerr << "validating dbtuple " << util::hexify(it->get_tuple()) << " at snapshot_tid "
                              << g_proto_version_str(cast()->snapshot_tid()) << std::endl);
            const bool found = sorted_dbtuples_contains(write_dbtuples, it->get_tuple());
            if (likely(found ? it->get_tuple()->is_latest_version(it->get_tid())
        ...
        if (!absent_set.empty()) {
          ...
          for (; it != it_end; ++it) {
            ...
            if (unlikely(v != it->second.version)) {
              VERBOSE(std::cerr << "expected node " << util::hexify(it->first) << " at v=" << it->second.version
                                << ", got v=" << v << std::endl);
      ...
      if (!write_dbtuples.empty()) {
        ...
        typename write_set_map::iterator it_end = write_set.end();
        for (; it != it_end; ++it) {
          if (unlikely(!it->do_write())) continue;
          dbtuple *const tuple = it->get_tuple();
          INVARIANT(tuple->is_locked());
          VERBOSE(std::cerr << "writing dbtuple " << util::hexify(tuple) << " at commit_tid "
                            << g_proto_version_str(commit_tid.second) << std::endl);
          if (it->is_insert()) {
            ...
            const dbtuple::write_record_ret ret =
                tuple->write_record_at(cast(), commit_tid.second, it->get_value(), it->get_writer());
            ...
            if (unlikely(ret.head_ != tuple)) {
              ...
              if (it->get_btree()->insert(varkey(it->get_key()), (typename concurrent_btree::value_type) ret.head_,
                                          &old_v, NULL))
            ...
            if (unlikely(unlock_head)) ret.head_->unlock();
          }
        ...
        for (typename dbtuple_write_info_vec::iterator it = write_dbtuples.begin(); it != write_dbtuples.end(); ++it) {
          if (it->is_locked()) it->tuple->unlock();
          else
    ...
    state = TXN_COMMITED;
    if (commit_tid.first) cast()->on_tid_finish(commit_tid.second);
    clear();
    return true;
  ...
  do_abort:
    ...
    for (typename dbtuple_write_info_vec::iterator it = write_dbtuples.begin(); it != write_dbtuples.end(); ++it) {
      if (it->is_locked()) {
        if (it->is_insert()) {
          INVARIANT(it->entry);
          this->cleanup_inserted_tuple_marker(it->tuple.get(), it->entry->get_key(), it->entry->get_btree());
        }
...
template<template<typename> class Protocol, typename Traits>
std::pair<dbtuple *, bool> transaction<Protocol, Traits>::try_insert_new_tuple(concurrent_btree &btr,
                                                                               const std::string *key,
                                                                               const void *value,
                                                                               dbtuple::tuple_writer_t writer) {
  INVARIANT(key);
  const size_t sz = value ? writer(dbtuple::TUPLE_WRITER_COMPUTE_NEEDED, value, nullptr, 0) : 0;

  // perf: ~900 tsc/alloc on istc11.csail.mit.edu
  dbtuple *const tuple = dbtuple::alloc_first(sz, true);
  if (value) writer(dbtuple::TUPLE_WRITER_DO_WRITE, value, tuple->get_value_start(), 0);
  ...
  if (unlikely(!btr.insert_if_absent(varkey(*key), (typename concurrent_btree::value_type) tuple, &insert_info))) {
...
bool transaction<Protocol, Traits>::do_tuple_read(const dbtuple *tuple, ValueReader &value_reader) {
  INVARIANT(tuple);
  ++evt_local_search_lookups;

  const bool is_snapshot_txn = is_snapshot();
  const transaction_base::tid_t snapshot_tid =
      is_snapshot_txn ? cast()->snapshot_tid() : static_cast<transaction_base::tid_t>(dbtuple::MAX_TID);
  ...
  if (Traits::read_own_writes) {
    ...
    if (unlikely(write_set_it != write_set.end())) {
      ...
      if (!write_set_it->get_value()) return false;
  ...
  if (v_empty) ++transaction_base::g_evt_read_logical_deleted_node_search;
  if (!is_snapshot_txn)
...
template<template<typename> class Protocol, typename Traits>
void transaction<Protocol, Traits>::do_node_read(const typename concurrent_btree::node_opaque_t *n, uint64_t v) {
  INVARIANT(n);
