 ...
 */
struct dbtuple {
  friend std::ostream &operator<<(std::ostream &o, const dbtuple &tuple);
...
private:
  static inline void AddTupleToLockRegion(const dbtuple *n) {
...
private:
  ...
  static inline ALWAYS_INLINE node_size_type CheckBounds(size_type s) {
...
public:
  ...
  inline version_t lock(bool write_intent) {
  ...
  inline bool is_deleting() const { return IsDeleting(hdr); }

  static inline bool IsDeleting(version_t v) { return v & HDR_DELETING_MASK; }

  inline void mark_deleting() {
    CheckMagic();
  ...
  inline bool is_modifying() const { return IsModifying(hdr); }

  inline void mark_modifying() {
    CheckMagic();
    version_t v = hdr;
    INVARIANT(IsLocked(v));
    INVARIANT(is_lock_owner());
  ...
  static inline bool IsModifying(version_t v) { return v & HDR_MODIFYING_MASK; }
  ...
  static inline bool IsLatest(version_t v) { return v & HDR_LATEST_MASK; }
  ...
  inline version_t unstable_version() const { return hdr; }
  ...
  inline ALWAYS_INLINE struct dbtuple *get_next() { return next; }

  inline const struct dbtuple *get_next() const { return next; }

  inline ALWAYS_INLINE void set_next(struct dbtuple *next) {
    CheckMagic();
    this->next = next;
  }
  ...
  inline ALWAYS_INLINE uint8_t *get_value_start() {
    CheckMagic();
...
private:
  ...
  struct scoped_recorder {
    ...
    ~scoped_recorder() { g_evt_avg_dbtuple_read_retries.offer(*n); }
...
public:
  ...
  inline bool latest_value_is_nil() const { return is_latest() && size == 0; }
...
private:
  static inline void destruct_and_free(dbtuple *n) {
