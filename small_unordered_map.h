...
template <typename T> struct is_eq_expensive {
...
static event_avg_counter evt_avg_max_unordered_map_chain_length
    CACHE_ALIGNED("avg_max_unordered_map_chain_length");

template <typename T> struct fast_func_param {
  typedef
...
template <typename Tp> struct myhash<Tp *> {
  inline ALWAYS_INLINE size_t operator()(Tp *t) const {
    ...
    size_t key =
        reinterpret_cast<size_t>(t) >> 3; // shift out 8-byte pointer alignment
...
static inline constexpr size_t TableSize(size_t small_size) {
 ...
 */
template <typename Key, typename T, size_t SmallSize = SMALL_SIZE_MAP,
...
public:
  ...
  typedef iterator_<const bucket, large_table_const_iterator, const value_type>
      const_iterator;
...
#else
        large_elems->operator[](std::move(b.ref().first)) =
            std::move(b.ref().second);
#endif
        b.destroy();
      }
      n = 0;
