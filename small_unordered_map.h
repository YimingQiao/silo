...
namespace private_ {
  ...
  static event_avg_counter evt_avg_max_unordered_map_chain_length CACHE_ALIGNED("avg_max_unordered_map_chain_length");
  ...
  template<typename T>
  struct myhash {
    inline ALWAYS_INLINE size_t operator()(typename fast_func_param<T>::type t) const { return std::hash<T>()(t); }
  };

  template<typename Tp>
  struct myhash<Tp *> {
    inline ALWAYS_INLINE size_t operator()(Tp *t) const {
  ...
  static inline constexpr size_t TableSize(size_t small_size) { return round_up_to_pow2_const(small_size); }
}// namespace private_
 ...
 */
template<typename Key, typename T, size_t SmallSize = SMALL_SIZE_MAP, typename Hash = private_::myhash<Key>>
...
private:
  ...
  static const bool is_trivially_destructible = private_::is_trivially_destructible<bucket_value_type>::value;
  ...
  struct bucket {
    inline ALWAYS_INLINE bucket_value_type *ptr() { return reinterpret_cast<bucket_value_type *>(&buf[0]); }
    ...
    inline ALWAYS_INLINE void destroy() {
      if (!is_trivially_destructible) ref().~bucket_value_type();
    }
  ...
  template<typename SmallIterType, typename LargeIterType, typename ValueType>
  ...
  public:
    ...
    inline iterator_(const iterator_<S, L, V> &other) : large(other.large),
                                                        b(other.b) {}

    inline ValueType &operator*() const {
      if (unlikely(large)) return *large_it;
  ...
  protected:
    inline iterator_(SmallIterType *b) : large(false), b(b) {}
    inline iterator_(LargeIterType large_it) : large(true), large_it(large_it) {}
...
public:
  typedef iterator_<bucket, large_table_iterator, value_type> iterator;

  typedef iterator_<const bucket, large_table_const_iterator, const value_type> const_iterator;

  small_unordered_map() : n(0), large_elems(0) { NDB_MEMSET(&table[0], 0, sizeof(table)); }
  ...
  small_unordered_map &operator=(const small_unordered_map &other) {
    // self assignment
...
private:
  bucket *find_bucket(const key_type &k, size_t *hash_value) {
    ...
    if (hash_value) *hash_value = h;
    const size_t i = h % TableSize;
    bucket *b = table[i];
    while (b) {
      const bool check_hash = private_::is_eq_expensive<key_type>::value;
      if ((!check_hash || b->h == h) && b->ref().first == k) return b;
      b = b->bnext;
    }
    return 0;
  }

  inline ALWAYS_INLINE const bucket *find_bucket(const key_type &k, size_t *hash_value) const {
...
public:
  ...
  mapped_type &operator[](const key_type &k) {
    if (unlikely(large_elems)) return large_elems->operator[](k);
    size_t h;
    bucket *b = find_bucket(k, &h);
    if (b) return b->ref().second;
  ...
  mapped_type &operator[](key_type &&k) {
    if (unlikely(large_elems)) return large_elems->operator[](std::move(k));
    size_t h;
    bucket *b = find_bucket(k, &h);
    if (b) return b->ref().second;
    if (unlikely(n == SmallSize)) {
  ...
  inline size_t size() const {
    if (unlikely(large_elems)) return large_elems->size();
    return n;
  }
  ...
  iterator begin() {
    if (unlikely(large_elems)) return iterator(large_elems->begin());
    return iterator(&small_elems[0]);
  }

  const_iterator begin() const {
    if (unlikely(large_elems)) return const_iterator(large_elems->begin());
    return const_iterator(&small_elems[0]);
  }

  inline iterator end() {
    if (unlikely(large_elems)) return iterator(large_elems->end());
    return iterator(&small_elems[n]);
  }

  inline const_iterator end() const {
    if (unlikely(large_elems)) return const_iterator(large_elems->end());
    return const_iterator(&small_elems[n]);
  }

  iterator find(const key_type &k) {
    if (unlikely(large_elems)) return iterator(large_elems->find(k));
    bucket *const b = find_bucket(k, 0);
    if (b) return iterator(b);
    return end();
  }

  const_iterator find(const key_type &k) const {
    if (unlikely(large_elems)) return const_iterator(large_elems->find(k));
    const bucket *const b = find_bucket(k, 0);
    if (b) return const_iterator(b);
    return end();
  }
...
private:
  ...
  inline void assignFrom(const small_unordered_map &that) {
    clear();
    if (that.large_elems) {
      INVARIANT(!that.n);
      INVARIANT(!n);
